...
export default async function ProjectsPage({searchParams}:{searchParams?:Promise<Search>}){
  ...
  return <>
      ...
      <div className="shell">
        ...
        {loadError?<div className="emptyState panel"><h3>Projects are unavailable right now.</h3><p>Please try again later.</p></div>:filtered.length?<PaginatedCardGrid label="projects" className="projectGrid projectBriefGrid">{filtered.map(p=>{
          ...
          return <article className={`projectBriefCard projectType-${p.project_type}`} key={p.id}>
            ...
            <footer className="projectBriefFoot"><div><span>{availabilityCopy}</span>{deadline&&<small>Applications close {deadline.toLocaleDateString('en-GB')}</small>}</div><div className="projectCardActions hasSecondaryAction"><a className="button dark" href={`/projects/${p.id}`}>View project →</a><a className="button ghost" href={interestDestination(p.id)}>Submit interest</a></div></footer>
    ...
    {interestProjects.length>0&&<section className="projectsInterest" id="interest"><div className="shell formShell"><div><div className="eyebrow">REGISTER INTEREST</div><h2>See a project you could contribute to?</h2><p className="lead">Tell us where you could add value. If the project moves forward and your experience fits the brief, we can invite you into the next step.</p></div>{selectedInterestProject?<SubmissionForm formType="project_application" submitLabel="Register my interest →" successMessage="Interest received. We’ll keep it connected to this project and contact you if there is a suitable next step."><div className="selectedPilotProject"><small>SELECTED PROJECT</small><strong>{selectedInterestProject.title}</strong><a href="#projects">Change project</a></div><input type="hidden" name="project_id" value={selectedInterestProject.id}/><input type="hidden" name="project" value={selectedInterestProject.title}/><label htmlFor="project-name">Full name *</label><input id="project-name" required name="name" autoComplete="name"/><label htmlFor="project-email">Email address *</label><input id="project-email" required name="email" type="email" autoComplete="email"/><label htmlFor="project-role">Where could you contribute? *</label><select id="project-role" name="role" required defaultValue=""><option value="" disabled>Select an area</option><option>Data Analysis / BI</option><option>Data Engineering</option><option>AI / ML</option><option>Research / UX</option><option>Project Lead</option><option>QA / Technical Review</option><option>Documentation / Storytelling</option></select><label htmlFor="project-profile">LinkedIn, GitHub or portfolio</label><input id="project-profile" name="profile" type="url" placeholder="https://"/><label htmlFor="project-contribution">Tell us what you could bring to this project *</label><textarea id="project-contribution" name="contribution" required/><label className="consent"><input required type="checkbox" name="consent" value="yes"/><span>I agree that Mettelo can use this information to review my interest in this project.</span></label></SubmissionForm>:<div className="formCard projectSelectionPrompt"><span className="chip">CHOOSE A PROJECT</span><h3>Select a project first.</h3><p>Use Submit interest on the relevant project so your request stays linked to the correct brief.</p><div className="actions"><a className="button dark" href="#projects">Choose a project →</a></div></div>}</div></section>}

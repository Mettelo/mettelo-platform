...
async function seed(){
 ...
 const {error:projectError}=await db.from('projects').insert({id:projectId,slug:'phase7-run-privacy',title:'Phase 7 run privacy fixture',summary:'Disposable Phase 7 run privacy fixture for governed operational visibility.',problem_statement:'Validate safe public run status without exposing Admin operational metadata.',status:'open',visibility:'public',project_type:'open',applications_open:false,team_size_threshold:1,min_team_size:1,target_team_size:1,max_team_size:2,participation_mode:'flexible',admission_mode:'auto'});if(projectError)throw projectError;

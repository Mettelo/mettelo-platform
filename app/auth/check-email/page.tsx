...
export default function CheckEmailPage(){
  ...
  return <section className="section softSection"><div className="shell" style={{maxWidth:760}}><div className="panel" style={{padding:'clamp(24px,5vw,52px)'}}><div className="eyebrow">Account verification</div><h1 style={{fontSize:'clamp(2.35rem,7vw,4.25rem)',margin:'0 0 14px'}}>Check your email.</h1><p className="lead" style={{marginBottom:12}}>We sent a verification link to <strong>{maskEmail(email)}</strong>. Open it to confirm your address and continue setting up My Mettelo.</p><p style={{color:'var(--slate)'}}>Didn&apos;t receive it? Check spam or junk first. You can resend the email below, or change the address and create the account again.</p><div className="actions" style={{alignItems:'stretch',flexWrap:'wrap'}}><button className="button dark" type="button" onClick={openEmailApp}>Open email app</button><button className="button ghost" type="button" onClick={resend} disabled={!email||status==='working'||cooldown>0}>{status==='working'?'Sending…':cooldown>0?`Resend available in ${cooldown}s`:'Resend verification email'}</button><a className="button ghost" href={`/signin?mode=signup&next=${encodeURIComponent(next)}`}>Wrong email? Change it</a></div><div className={`formStatus ${status}`} role="status" aria-live="polite">{message}</div><div style={{marginTop:20,paddingTop:20,borderTop:'1px solid rgba(16,19,29,.12)'}}><strong>Still stuck?</strong><p style={{marginBottom:10,color:'var(--slate)'}}>Check that you entered the address correctly and look in spam or junk. If the message still does not arrive, Mettelo support can help you recover the journey.</p><a className="linkArrow" href="/contact">Contact Mettelo support →</a></div></div></div></section>;
}

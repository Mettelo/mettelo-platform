...
function phraseMatch(a:string,b:string){return Boolean(a&&b&&(a===b||a.includes(b)||b.includes(a)))}
...
export function evaluateMemberProjectFit(input:Input):MemberProjectFit{
 ...
 const roleFits:Record<string,MemberRoleFit>={};for(const role of input.roles){const roleSkills=[...(role.skills||[]),...(role.recommendedSkills||[])].map(item=>item.trim()).filter(Boolean);const matched=roleSkills.filter(item=>memberSkills.has(norm(item)));const review=roleSkills.filter(item=>!memberSkills.has(norm(item))).slice(0,6);const roleKeys=[norm(role.title),norm(role.canonicalRoleKey)].filter(Boolean);const preferredRoleMatch=preferredRoles.some(preferred=>roleKeys.some(roleKey=>phraseMatch(preferred,roleKey)));const status=matched.length>=Math.min(2,roleSkills.length||2)||preferredRoleMatch?'aligned':matched.length>0?'partial':'review';roleFits[role.id]={roleId:role.id,matchedSkills:matched.slice(0,8),skillsToReview:review,preferredRoleMatch,status,summary:status==='aligned'?'Strong signals from your saved profile':status==='partial'?'Some profile signals overlap':'Review this role against your experience before continuing'}}

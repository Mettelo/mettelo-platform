...
test.describe('Public Projects approved discovery filters',()=>{
  test('public catalogue uses the shared governed engine, complete loader and public-only boundary',()=>{
    ...
    expect(page).toContain("from '@/lib/project-catalogue-filtering'");expect(page).toContain("from '@/lib/public-project-catalogue-loader'");expect(page).toContain('filterAndSortProjectCatalogue(catalogueItems,filters)');expect(page).toContain('const PUBLIC_PAGE_SIZE=12');expect(page).toContain("query.set('skill',filters.capability)");expect(page).toContain("query.set('working',filters.workingModel)");expect(page).toContain("query.set('sort',filters.sort)");expect(page).not.toContain('project_members');expect(page).not.toContain('saved_projects');expect(page).not.toContain('applicationReadiness');
    expect(loader).toContain(".eq('visibility','public')");expect(loader).toContain('const BATCH_SIZE=200');expect(loader).toContain('.range(from,to)');expect(loader).toContain('if(batch.length<BATCH_SIZE)break');expect(loader).not.toContain('.limit(500)');expect(engine).toContain('catalogueDurationOptions');expect(engine).toContain("experience:string;");expect(engine).toContain("format:string;");expect(engine).toContain("availability:string;");expect(controls).toContain("mettelo:catalogue-analytics");expect(controls).toContain('More filters');expect(controls).toContain('Filters · {activeCount}');expect(controls).toContain('AbortController');expect(controls).not.toContain('window.location.assign');
  });
  ...
  test('public catalogue preserves enriched to core to scalar fallback without fixed catalogue ceilings',()=>{
    ...
    expect(loader).toContain('const PRIMARY_SELECT=');expect(loader).toContain('const CORE_SELECT=');expect(loader).toContain('const MINIMAL_SELECT=');expect(loader).toContain('team_size_threshold');expect(loader).toContain('return all(db,MINIMAL_SELECT)');expect(loader).toContain("console.warn('public Projects enriched query failed; retrying core facets'");expect(loader).toContain("console.warn('public Projects core query failed; retrying scalar fields'");expect(publicClient).toContain('new Set([429,500,502,503,504])');expect(publicClient).toContain('const PUBLIC_READ_MAX_ATTEMPTS=3');expect(publicClient).toContain("const retryable=method==='GET'||method==='HEAD'");expect(publicClient).toContain('if(!retryable)return fetch(input,init)');
  });
  ...
  test('desktop matches approved quick-filter hierarchy and filter drawer',async({page})=>{
    ...
    await expect(page.getByLabel('Career or role',{exact:true})).toBeVisible();await expect(page.getByLabel('Experience level',{exact:true})).toBeVisible();await expect(page.getByLabel('Work format',{exact:true})).toBeVisible();
  ...
  test('rapid quick filters stay interactive and preserve their combined URL state',async({page})=>{
    ...
    const role=page.getByLabel('Career or role',{exact:true});const experience=page.getByLabel('Experience level',{exact:true});const format=page.getByLabel('Work format',{exact:true});

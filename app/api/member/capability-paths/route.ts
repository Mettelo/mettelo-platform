...
export async function POST(request:Request){
  ...
  try{
    ...
    if(action==='follow'){
      const {data:path}=await db.from('capability_paths').select('id,status').eq('id',pathId).eq('status','published').maybeSingle();if(!path)return NextResponse.json({error:'Only published Capability Paths can be followed.'},{status:409});
      const {data:existing}=await db.from('member_capability_paths').select('path_id,is_primary').eq('user_id',user.id).eq('path_id',pathId).maybeSingle();
      ...
      else{const {error}=await db.from('member_capability_paths').insert({user_id:user.id,path_id:pathId,status:'following',is_primary:false});if(error)throw error;}
      const {count}=await db.from('member_capability_paths').select('path_id',{count:'exact',head:true}).eq('user_id',user.id).eq('is_primary',true);
      if(!count){const {error}=await db.rpc('set_my_primary_capability_path',{target_path:pathId});if(error)throw error;}

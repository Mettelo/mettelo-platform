...
const checks=[
 ...
 ['components/AdminPlatformSettings.tsx',["inputType=setting.value_type==='email'?'email':setting.value_type==='url'?'url':'text'",'audit_recorded===false',"fetch('/api/admin/settings'","fetch('/api/admin/project-role-catalogue'",'aria-live="polite"','@media(max-width:480px)','font-size:16px']],
 ...
 ['components/AdminPlatformAuthStatus.tsx',['platformAuthWorkspace','<h1>Authentication &amp; SSO status</h1>','Read-only deployment status','never exposes API keys','Configured','Missing','Enabled','Disabled','Unknown','Feature flags are not managed here.','No governed runtime feature-flag consumers','@media(max-width:480px)']],
...
if(failed)process.exit(1);console.log(`Admin Platform configuration audit passed: ${passed}/${checks.length} files.`);

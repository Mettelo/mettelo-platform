...
export async function PATCH(request:Request){
 try{
  ...
  const {data:current,error:loadError}=await db.from('projects').select('id,admission_mode,auto_start_delay_minutes,auto_start_paused_at,late_joining_enabled,late_joining_cutoff_at,project_sharing_enabled,member_invites_enabled').eq('id',projectId).maybeSingle();if(loadError||!current)return NextResponse.json({error:'Project not found.'},{status:404});
  ...
  const {data,error}=await db.from('projects').update(patch).eq('id',projectId).select('id,admission_mode,auto_start_delay_minutes,auto_start_paused_at,late_joining_enabled,late_joining_cutoff_at,project_sharing_enabled,member_invites_enabled').single();if(error)throw error;

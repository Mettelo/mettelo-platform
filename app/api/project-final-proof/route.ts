...
export async function POST(request:Request){
 try{
  ...
  if(action==='assess_criterion'){
   const criterionId=clean(body.criterion_id,80),notes=clean(body.assessment_notes,1600),evidenceUrl=clean(body.evidence_url,800),satisfied=body.satisfied===true;if(!criterionId)return NextResponse.json({error:'Choose a success criterion.'},{status:400});if(satisfied&&!notes&&!evidenceUrl)return NextResponse.json({error:'Add assessment notes or an evidence URL before marking this criterion satisfied.'},{status:400});
   const {data:result,error}=await auth.rpc('phase19_assess_success_criterion',{p_run_id:runId,p_criterion_id:criterionId,p_satisfied:satisfied,p_notes:notes||null,p_evidence_url:evidenceUrl||null});
   if(error)return NextResponse.json({error:error.message||'Unable to assess this success criterion.'},{status:rpcStatus(error)});return NextResponse.json(result||{ok:true});
  }

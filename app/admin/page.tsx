...
export default async function AdminDashboard(){
  ...
  if(url&&serviceKey){
    ...
    const [members,projects,opportunities,events,submissions,applications,projectRows,submissionRows,applicationRows]=await Promise.all([
  ...
  return <section className="section softSection"><div className="shell dashboard">
    <aside className="sidebar"><div className="sidebarTop"><h3>Mettelo Admin</h3><small>{user.email}</small></div><a className="active" href="#overview">Overview <span>⌂</span></a><a href="#publisher">Publish <span>＋</span></a><a href="#applications">Applications <span>{counts.applications}</span></a><a href="#projects">Projects <span>{counts.projects}</span></a><a href="#submissions">Intake <span>{counts.submissions}</span></a><a href="#operations">Operations <span>→</span></a></aside>
    <main className="dashboardMain" id="overview"><div className="eyebrow">Operations console</div><h1>Run Mettelo from production records.</h1><p className="lead">Publish Labs briefs, opportunities and events, review applications and move accepted members into project teams from one protected workspace.</p>
      ...
      <div className="dashboardGrid" style={{marginTop:18}}><section className="panel" id="submissions"><div className="panelHead"><h3>Latest intake</h3><span className="chip">PRIVATE</span></div>{recentSubmissions.length?recentSubmissions.map(item=><div className="listRow" key={item.id}><div><strong>{item.form_type.replace('_',' ')}</strong><br/><small>{new Date(item.created_at).toLocaleString('en-GB')}</small></div><span className="chip">{item.status.toUpperCase()}</span></div>):<div className="emptyState"><h3>No submissions yet.</h3><p>Contact, partnership, contributor and feedback submissions appear here.</p></div>}</section><aside className="panel" id="operations"><div className="panelHead"><h3>Operational loop</h3><span className="chip green">LIVE DATA</span></div><div className="listRow"><span>Publish</span><strong>Admin → Supabase</strong></div><div className="listRow"><span>Apply</span><strong>Member account</strong></div><div className="listRow"><span>Review</span><strong>Admin queue</strong></div><div className="listRow"><span>Accept</span><strong>Project membership</strong></div></aside></div>
    </main>
  </div></section>;
}

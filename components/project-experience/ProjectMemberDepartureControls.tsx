...
const hasOperationalContext=(value:Handover)=>[value.completed_work,value.open_work,value.file_references,value.decisions,value.risks,value.recommendations,value.open_responsibilities,value.handover_availability].some(item=>item.trim());
...
export default function ProjectMemberDepartureControls({projectId,projectRunId,initialState}:Props){
 ...
 return <section className={styles.panel} aria-labelledby="member-departure-title">
  ...
  {state==='none'?<div className={styles.form}>
   ...
   <div className={styles.meta}><span>At least one practical handover section is required.</span><button type="button" onClick={()=>void submit('request')} disabled={busy||!handover.reason_category||!hasOperationalContext(handover)}>{busy?'Recording…':'Record handover'}</button></div>

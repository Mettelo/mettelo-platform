...
export async function getPublicProjectExperienceData(projectId:string):Promise<PublicProjectExperienceData>{
  ...
  const detail:ProjectDetailContent={
    ...
    technicalSkills:strings(normalized?.technical_skills),professionalSkills:strings(normalized?.professional_skills),importedTools:strings(normalized?.tools),importedMethods:strings(normalized?.methods),importedDomain:text(normalized?.domain),sourceProjectKey:text(origin?.source_project_key)
  };

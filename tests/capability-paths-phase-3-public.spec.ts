...
test.describe('Capability Paths Phase 3 public journey',()=>{
 test('public users discover only published Paths and reused projects retain one canonical route',async({page},testInfo)=>{
  ...
  await page.goto(`/projects/${projectId}?path=${encodeURIComponent(pathA.slug)}`,{waitUntil:'networkidle'});await expect(page.getByRole('heading',{name:'Where this project fits.'})).toBeVisible();await expect(page.getByText(pathA.name,{exact:true}).first()).toBeVisible();await expect(page.getByText(pathB.name,{exact:true}).first()).toBeVisible();await expect(page.getByRole('link',{name:`← Back to ${pathA.name} Capability Path`})).toBeVisible();

...
import {isValidTimeZone,zonedLocalToIso} from '@/lib/zoned-date-time';
...
export async function PATCH(request:Request){
  try{
    ...
    const interviewTimezone=text(body.interview_timezone,100);const interviewFormat=text(body.interview_format,120);const interviewUrl=text(body.interview_url,1000);const interviewer=text(body.interviewer,200);const interviewInstructions=text(body.interview_instructions,4000);const interviewAt=status==='interview'&&interviewTimezone?zonedLocalToIso(String(body.interview_at||''),interviewTimezone):null;
    if(status==='interview'&&!interviewTimezone)return NextResponse.json({error:'Interview timezone is required.'},{status:400});if(status==='interview'&&!isValidTimeZone(interviewTimezone||''))return NextResponse.json({error:'Use a valid IANA timezone such as Europe/London.'},{status:400});if(status==='interview'&&!interviewAt)return NextResponse.json({error:'Enter a valid interview date and time for the selected timezone. Times skipped by daylight-saving changes are not valid.'},{status:400});if(status==='interview'&&!interviewFormat)return NextResponse.json({error:'Interview format is required.'},{status:400});if(!validHttps(interviewUrl))return NextResponse.json({error:'Interview URL must be a valid HTTPS link.'},{status:400});
    ...
    const interviewSummary=status==='interview'?[interviewAt?`Interview: ${new Intl.DateTimeFormat('en-GB',{timeZone:interviewTimezone||'UTC',dateStyle:'medium',timeStyle:'short'}).format(new Date(interviewAt))}`:'',interviewTimezone?`Timezone: ${interviewTimezone}`:'',interviewFormat?`Format: ${interviewFormat}`:'',interviewer?`Interviewer: ${interviewer}`:'',interviewInstructions||'',interviewUrl?`Joining link: ${interviewUrl}`:''].filter(Boolean).join('\n'):null;
    ...
    return NextResponse.json({ok:true,email_sent:sent.sent,email_queued:sent.queued,onboarding_items:onboardingItems,interview_at:interviewAt});

...
export async function PATCH(request:Request){
 try{
  ...
  if(action==='status'){const status=String(body.status||'');if(!['new','in_progress','resolved','duplicate'].includes(status))return NextResponse.json({error:'Invalid triage status.'},{status:400});const patch:Record<string,unknown>={status,updated_at:now};if(status==='in_progress')patch.reviewed_at=now;if(status==='resolved'||status==='duplicate')patch.resolved_at=now;const {data,error}=await db.from('form_submissions').update(patch).in('id',ids).select('id,status,assigned_to_user_id,reviewed_at,resolved_at,duplicate_of_id,converted_application_id,updated_at');if(error)throw error;return NextResponse.json({ok:true,items:data||[]});}
  if(action==='assign'){const assigned=String(body.assigned_to_user_id||'').trim()||null;const {data,error}=await db.from('form_submissions').update({assigned_to_user_id:assigned,updated_at:now}).in('id',ids).select('id,status,assigned_to_user_id,reviewed_at,resolved_at,duplicate_of_id,converted_application_id,updated_at');if(error)throw error;return NextResponse.json({ok:true,items:data||[]});}
  if(action==='duplicate'){const duplicateOf=String(body.duplicate_of_id||'').trim()||null;if(ids.length!==1)return NextResponse.json({error:'Choose one submission when linking a duplicate.'},{status:400});const {data,error}=await db.from('form_submissions').update({status:'duplicate',duplicate_of_id:duplicateOf,resolved_at:now,updated_at:now}).eq('id',ids[0]).select('id,status,assigned_to_user_id,reviewed_at,resolved_at,duplicate_of_id,converted_application_id,updated_at').single();if(error)throw error;return NextResponse.json({ok:true,items:[data]});}
  if(action==='convert'){
    ...
    const payload=(submission.payload&&typeof submission.payload==='object'&&!Array.isArray(submission.payload)?submission.payload:{}) as Record<string,unknown>;const email=text(payload.email,320).toLowerCase(),projectTitle=text(payload.project,240),contribution=text(payload.contribution,4000),requestedRole=text(payload.role,160),portfolio=text(payload.profile,800);if(!email||!projectTitle||!contribution)return NextResponse.json({error:'This legacy submission is missing email, project or contribution details required for conversion.'},{status:409});
    const {data:project}=await db.from('projects').select('id,title').ilike('title',projectTitle).limit(1).maybeSingle();if(!project)return NextResponse.json({error:'No governed project matches the submitted project name. Link/clean the intake record before conversion.'},{status:409});
    const account=await findAuthUserByEmail(db,email);if(!account)return NextResponse.json({error:'The submitter does not have a linked Mettelo account. Ask them to create/sign in to an account before conversion.'},{status:409});
    const {data:existingRows,error:existingError}=await db.from('project_applications').select('id').eq('project_id',project.id).eq('user_id',account.id).neq('status','withdrawn').limit(1);if(existingError)throw existingError;const existing=existingRows?.[0];if(existing){await db.from('form_submissions').update({status:'duplicate',converted_application_id:existing.id,resolved_at:now,updated_at:now}).eq('id',submission.id);return NextResponse.json({ok:true,application_id:existing.id,duplicate:true});}
    const {data:application,error}=await db.from('project_applications').insert({project_id:project.id,user_id:account.id,portfolio_url:portfolio||null,contribution_statement:contribution,requested_role:requestedRole||null,status:'submitted',application_kind:'application'}).select('id').single();if(error)throw error;
    await db.from('project_application_events').insert({application_id:application.id,from_status:null,to_status:'submitted',note:'Converted by Admin from legacy intake submission.',actor_user_id:user.id});
    await db.from('form_submissions').update({status:'resolved',converted_application_id:application.id,reviewed_at:now,resolved_at:now,updated_at:now}).eq('id',submission.id);
    return NextResponse.json({ok:true,application_id:application.id,duplicate:false});
  }

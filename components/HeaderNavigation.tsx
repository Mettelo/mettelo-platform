  ...
  return <><nav ref={navRef} className={`${styles.headerNavigation} primaryNav`} aria-label="Primary navigation">{navigation.map(({label,items})=>{const isOpen=open===label;return <div className={`navDropdown${isOpen?' isOpen':''}`} key={label}><button className="navDropdownTrigger" type="button" aria-expanded={isOpen} onClick={()=>setOpen(isOpen?null:label)}>{label}<span aria-hidden="true">⌄</span></button><div className="navDropdownPanel" aria-hidden={!isOpen}>{items.map(([title,href,copy])=><Link key={`${label}-${href}`} href={href} onClick={()=>setOpen(null)}><strong>{title}</strong><small>{copy}</small></Link>)}</div></div>})}<Link className="primaryNavLink aboutNavLink" href="/about">About Mettelo</Link></nav>
    ...
    <style jsx global>{`
      ...
      @media(max-width:1080px){
        .siteHeader .nav{height:58px;gap:8px}.siteHeader .brandLogo{width:112px!important;max-width:112px!important}.globalSearchPortal,.desktopAccountPortal{display:none}.mobileHeaderSearch{display:grid;order:5;flex:none}.mobileMenu{order:6;margin-left:0!important}.mobileMenu>summary{width:38px!important;height:38px!important;min-width:38px!important;padding:0!important;border-radius:11px!important;box-shadow:none!important}.menuLabel{display:none!important}.mobileMenuPanel{top:calc(34px + 58px + 7px)!important;left:10px!important;right:10px!important;width:auto!important;max-height:calc(100dvh - 110px)!important;padding:10px!important;border-radius:16px!important;overscroll-behavior:contain}.authSignedIn .mobileMenuPanel{top:65px!important;max-height:calc(100dvh - 74px)!important}.mobileMenuPanel>:not(.metteloMobileNav){display:none!important}.metteloMobileNav{display:grid;gap:10px}.mobileGuestActions{display:grid;grid-template-columns:1.15fr .85fr;gap:8px}.mobileGuestActions a{min-height:42px;display:grid!important;place-items:center!important;padding:8px 10px!important;border:1px solid rgba(16,19,29,.1);border-radius:10px!important}.mobileGuestPrimary{background:#10131d!important;color:#fff!important}.mobileAccountCard{padding:10px;border:1px solid rgba(16,19,29,.09);border-radius:13px;background:#fbfaf7}.mobileAccountIdentity{display:flex;align-items:center;gap:10px;padding:2px 3px 9px}.mobileAccountIdentity>div{display:grid}.mobileAccountIdentity strong{font-size:.82rem;line-height:1.2}.mobileAccountIdentity small{font-size:.66rem;color:#7b8390}.mobileAvatar{width:34px;height:34px;display:grid;place-items:center;border-radius:9px;background:#2a2f52;color:#fff;font-weight:800;flex:none}.mobileAccountLinks{display:grid;grid-template-columns:1fr 1fr;gap:6px}.mobileAccountLinks a,.mobileAccountLinks button{min-height:40px;display:flex!important;align-items:center!important;justify-content:space-between!important;padding:8px 10px!important;border:0;border-radius:9px!important;background:#fff;color:#4d5663;font-size:.75rem;font-weight:700;text-align:left}.mobileAccountLinks button{cursor:pointer}.mobileAccountLinks a:first-child{background:#f7efdd}.mobileAccountLinks button{color:#92352f}.mobileNavSections{display:grid}.mobileNavSection{border-top:1px solid rgba(16,19,29,.08)}.mobileNavSection>summary{height:auto!important;min-width:0!important;min-height:44px!important;display:flex!important;align-items:center!important;justify-content:space-between!important;gap:10px!important;padding:10px 8px!important;border:0!important;border-radius:0!important;background:transparent!important;box-shadow:none!important;list-style:none!important;color:#242a35!important;font-size:.82rem!important;font-weight:750!important;cursor:pointer!important}.mobileNavSection>summary::-webkit-details-marker{display:none}.mobileChevron{font-size:.75rem;transition:transform .18s ease}.mobileNavSection[open] .mobileChevron{transform:rotate(180deg)}.mobileNavLinks{display:grid;padding:0 0 7px 6px}.mobileNavLinks a{min-height:38px!important;padding:8px 10px!important;border-radius:8px!important;color:#56606d!important;font-size:.78rem!important;font-weight:650!important}.mobileNavLinks a:hover,.mobileNavLinks a:focus-visible{background:#f7efdd!important}.mobileAboutLink{min-height:44px!important;display:flex!important;align-items:center!important;justify-content:space-between!important;padding:10px 8px!important;border-top:1px solid rgba(16,19,29,.08);border-radius:0!important;font-size:.82rem!important;font-weight:750!important}.footerGrid{grid-template-columns:1fr 1fr 1fr!important}.footerBrandColumn,.footerNewsletterColumn{grid-column:1/-1}
      }

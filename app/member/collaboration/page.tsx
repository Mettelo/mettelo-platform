...
export default async function CollaborationNetworkPage({searchParams}:{searchParams?:Promise<Search>}){
 ...
 let projectContext:null|{projectId:string;runId:string;needId:string;title:string;role:string;occupied:number;maximum:number;openPlaces:number;recruitment:string}=null;
 let contextWarning='';
 if(requestedProjectId||requestedRunId){
  if(!requestedProjectId||!requestedRunId)contextWarning='Project context could not be verified. Open Collaboration Network from Mettelo Lab → Team and try again.';
   ...
   if(project&&run&&membership&&capacity&&!capacityResult.error){
    ...
    projectContext={projectId:requestedProjectId,runId:requestedRunId,needId:verifiedNeedId,title:project.title,role:human(membership.team_role||'Contributor'),occupied:Number(capacity.occupied||0),maximum:Number(capacity.maximum||0),openPlaces:Number(capacity.available||0),recruitment};
 ...
 return <main className="cnRoot">
  ...
  {requestedView==='people'?<MemberCollaboratorDiscovery projectId={projectContext?.projectId} projectRunId={projectContext?.runId} initialNeedId={projectContext?.needId} projectTitle={projectContext?.title}/>:<section className="cnTeams" aria-labelledby="teams-looking-title">
   ...
   <form className="cnFilters" method="get" action="/member/collaboration"><input type="hidden" name="view" value="teams"/><label className="cnSearch">Search<input name="q" defaultValue={one(params.q)} placeholder="Project, role, capability or domain"/></label><label>Role<select name="role" defaultValue={one(params.role)}><option value="">All roles</option>{roleOptions.map(value=><option key={value} value={value}>{value}</option>)}</select></label><label>Capability<select name="capability" defaultValue={one(params.capability)}><option value="">All capabilities</option>{capabilityOptions.map(value=><option key={value} value={value}>{value}</option>)}</select></label><label>Domain<select name="domain" defaultValue={one(params.domain)}><option value="">All domains</option>{domainOptions.map(value=><option key={value} value={value}>{value}</option>)}</select></label><label>Commitment<select name="commitment" defaultValue={one(params.commitment)}><option value="">Any commitment</option>{commitmentOptions.map(value=><option key={value} value={value}>{value}</option>)}</select></label><button className="button dark" type="submit">Apply filters</button></form>
   {teamsError?<div className="cnError" role="alert"><strong>We couldn’t load the Collaboration Network.</strong><p>Try again.</p></div>:<><div className="cnCount" aria-live="polite">{filteredTeams.length} open collaboration need{filteredTeams.length===1?'':'s'}</div><div className="cnGrid">{filteredTeams.map(item=><article className="cnTeamCard" key={item.need.id}><div className="cnTeamTop"><div><span>COLLABORATOR NEEDED</span><h3>{item.project.title}</h3><p>{[item.role||item.need.responsibility,item.domain].filter(Boolean).join(' · ')||'Project collaboration'}</p></div><b>{item.openPlaces} place{item.openPlaces===1?'':'s'} open</b></div>{item.project.summary&&<p className="cnSummary">{item.project.summary}</p>}<div className="cnChips">{item.capabilities.slice(0,4).map(value=><span key={value}>{value}</span>)}</div><dl className="cnStats"><div><dt>Team</dt><dd>{item.occupied} / {item.maximum}</dd></div><div><dt>Weekly</dt><dd>{item.need.weekly_commitment||item.project.weekly_commitment||'See brief'}</dd></div><div><dt>Joining</dt><dd>Open</dd></div></dl><div className="cnActions"><Link className="button dark" href={`/member/discover/${item.project.id}?collaboration_need=${encodeURIComponent(item.need.id)}`}>View collaboration need</Link><Link className="button ghost" href={`/member/discover/${item.project.id}`}>View project</Link></div></article>)}{filteredTeams.length===0&&<div className="cnEmpty"><strong>No teams are currently looking for collaborators</strong><p>Explore projects or check back when new collaboration needs open.</p>{(q||roleFilter||capabilityFilter||domainFilter||commitmentFilter)&&<Link className="button ghost" href="/member/collaboration?view=teams">Clear filters</Link>}</div>}</div></>}
  </section>}
  <style>{`

...
export default async function ProjectWorkspace({params}:{params:Promise<{id:string}>}){
  ...
  const isAdmin=user.app_metadata?.role==='admin';const canLead=isAdmin||['project_lead','reviewer'].includes(membership.team_role);const canReview=isAdmin||['project_lead','reviewer'].includes(membership.team_role);
  ...
  const milestones=(milestoneResult.data||[]) as Milestone[];const tasks=(taskResult.data||[]) as Task[];const discussions=(discussionResult.data||[]) as Discussion[];const resources=(resourceResult.data||[]) as Resource[];const meetings=(meetingResult.data||[]) as Meeting[];const presentation=(presentationResult.data||null) as Presentation|null;const slots=(slotResult.data||[]) as Slot[];const readiness=(readinessResult.data||emptyReadiness) as Readiness;

  const serviceUrl=process.env.NEXT_PUBLIC_SUPABASE_URL;const serviceKey=process.env.SUPABASE_SERVICE_ROLE_KEY;const service=serviceUrl&&serviceKey?createClient(serviceUrl,serviceKey,{auth:{persistSession:false,autoRefreshToken:false}}):null;
  let team:TeamMember[]=[{id:user.id,name:user.user_metadata?.full_name||user.email?.split('@')[0]||'You',headline:null,role:membership.team_role}];let presenterIds:string[]=[];let reviewItems:{id:string;name:string;title:string;type:string;description:string|null;evidence_url:string|null;status:string;task:string|null;created_at:string}[]=[];
  if(service){
    const {data:memberRows}=await service.from('project_members').select('user_id,team_role').eq('project_id',id).order('joined_at');
    const ids=(memberRows||[]).map(row=>row.user_id);const {data:profiles}=ids.length?await service.from('profiles').select('id,full_name,headline').in('id',ids):{data:[]};const profileMap=new Map((profiles||[]).map(profile=>[profile.id,profile]));
    ...
    if(canReview){const {data:proofRows}=await service.from('contributions').select('id,user_id,title,contribution_type,description,evidence_url,verification_status,task_id,created_at').eq('project_id',id).in('verification_status',['pending','needs_changes']).order('created_at');const taskMap=new Map(tasks.map(task=>[task.id,task.title]));const nameMap=new Map(team.map(member=>[member.id,member.name]));reviewItems=((proofRows||[]) as ReviewRow[]).map(row=>({id:row.id,name:nameMap.get(row.user_id)||'Mettelo member',title:row.title,type:row.contribution_type,description:row.description,evidence_url:row.evidence_url,status:row.verification_status,task:row.task_id?taskMap.get(row.task_id)||null:null,created_at:row.created_at}));}
  }
  const teamNames=new Map(team.map(member=>[member.id,member.name]));const userTasks=tasks.filter(task=>task.assignee_user_id===user.id).map(task=>({id:task.id,project_id:id,title:task.title,status:task.status}));

  return <section className="section softSection"><div className="shell"><div className="sectionHead"><div><div className="eyebrow">Mettelo Labs · Private team workspace</div><h1>{project.title}</h1></div><p>{project.summary}</p></div>

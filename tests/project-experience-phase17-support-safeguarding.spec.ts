...
test.describe('Project Experience Phase 17 support, conflict and safeguarding contract',()=>{
 ...
 test('RLS plus column privileges keep cases reporter-private and internal notes unreadable',()=>{
  ...
  expect(safeGrant).not.toContain('internal_notes');
  expect(safeGrant).not.toContain('assigned_admin_user_id');
 ...
 test('member API authenticates exact-run membership, supports secure reporter response and keeps notification copy generic',()=>{
  ...
  const createNotification=route.slice(route.indexOf('await notifyAdmins'),route.indexOf('}catch(notificationError)'));
  for(const privateField of ['description,','body.description','category,description','internal_notes','recovery_plan'])expect(createNotification).not.toContain(privateField);
 });
 test('Admin workflow requires explicit support capability and safeguarding access is narrower',()=>{
  ...
  for(const text of ["hasAdminCapability(user,'projects.support.manage')","hasAdminCapability(user,'projects.safeguarding.manage')",'project_support_case_updates','project_activity_log',"actor_type:'admin'",'support_case_id:caseId','safeguarding_escalated_at'])expect(route).toContain(text);
  expect(capabilities).toContain("'projects.support.manage'");
  expect(capabilities).toContain("'projects.safeguarding.manage'");
 ...
 test('Admin state transitions keep safeguarding detail restricted, reject stale concurrent writes and closing requires resolution',()=>{
  const route=read('app/api/admin/project-support-cases/route.ts');
  const privacy=read('supabase/migrations/20260908011000_project_experience_phase_17_support_privacy_hardening.sql');
  ...
  expect(route).toContain('This support case changed before your action was saved.');
  expect(privacy).toContain('safeguarding_escalated_at timestamptz');
  expect(privacy).toContain('Permanent sensitivity marker');
 });
 test('member Lab surface provides private entry, safe reference, tracker and secure information response',()=>{
  const member=read('components/project-experience/ProjectSupportCaseSection.tsx');
  for(const text of ['PRIVATE SUPPORT','Your Project Lead and teammates do not automatically receive access','Support case created. Reference','Mettelo project support is not an emergency service.',"item.status==='awaiting_member'",'Respond securely','Send secure response',"item.status==='closed'"])expect(member).toContain(text);
 });

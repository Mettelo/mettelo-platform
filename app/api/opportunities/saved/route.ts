...
export async function GET(request:Request){
  ...
  if(opportunityId){
    const {data,error}=await supabase.from('saved_opportunities').select('opportunity_id,saved_at,reminders_enabled').eq('user_id',user.id).eq('opportunity_id',opportunityId).maybeSingle();
  ...
  const {data,error}=await supabase.from('saved_opportunities').select('opportunity_id,saved_at,reminders_enabled').eq('user_id',user.id).order('saved_at',{ascending:false});
...
export async function POST(request:Request){
  ...
  const {error}=await supabase.from('saved_opportunities').upsert({user_id:user.id,opportunity_id:opportunityId,reminders_enabled:true},{onConflict:'user_id,opportunity_id',ignoreDuplicates:true});
...
export async function PATCH(request:Request){
  ...
  const {data,error}=await supabase.from('saved_opportunities').update({reminders_enabled:body.reminders_enabled}).eq('user_id',user.id).eq('opportunity_id',opportunityId).select('opportunity_id,reminders_enabled').maybeSingle();

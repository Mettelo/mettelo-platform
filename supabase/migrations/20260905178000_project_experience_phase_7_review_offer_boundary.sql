...
-- AUTO remains a separate canonical admission path; once a run is ready, Admin gets
-- a six-hour oversight window before the existing durable scheduler may auto-start it.

alter table public.projects
  alter column auto_start_delay_minutes set default 360;

-- Phase 6 introduced 120 as the programme default. Phase 7 changes that programme
-- default to six hours. At this point no production Phase 6 migration has been
-- released, so rows carrying the untouched Phase 6 default are safe to migrate.
...
comment on column public.projects.auto_start_delay_minutes is
  'Delay after AUTO run readiness is reached before durable auto-start processing. Programme default is 360 minutes (6 hours), providing an Admin oversight window. Per-project configuration remains supported.';

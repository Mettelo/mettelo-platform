...
test('Discover and member project detail preserve the approved responsive internal journey',async({page})=>{
  ...
  for(const width of widths){
    ...
    if(width<=480){const mobile=page.getByRole('navigation',{name:'My Mettelo mobile navigation'});await expect(mobile).toBeVisible();await expect(mobile.locator('a[href="/member/discover"]')).toHaveAttribute('aria-current','page');await expect(page.locator('.mdDesktopFilters')).toBeHidden();await expect(page.getByRole('button',{name:/Filters ·/})).toBeVisible()}else{await expect(page.getByRole('complementary',{name:'My Mettelo navigation'})).toBeVisible();await expect(page.locator('.mdDesktopFilters')).toBeVisible()}

...
// failure while providing no stronger protection over the actual metadata write.
const labAnalyticsSql=labAnalytics.replace(/--.*$/gm,'');
...
const cases:[string,boolean][]=[
 ...
 ['completed Lab history is read-only while active runs retain mutation authority',labMutations.includes("pm.membership_status='active'")&&labMutations.includes("pr.status in ('active','review')")&&['project_discussions','project_resources','project_meetings','project_tasks','project_milestones','project_data_sources','project_data_source_versions','project_deliverables'].every(table=>labMutations.includes(table))&&labMutations.includes('phase12_run_live_update')],
 ['project-level canonical definitions remain under existing governance while run rows require live mutation authority',labMutations.includes('project_run_id is null or public.phase12_can_mutate_lab(project_id,project_run_id)')],
 ...
 ['Lab analytics stores only aggregate surface metadata and no private content',labAnalyticsSql.includes("jsonb_build_object('surface','mettelo_lab')")&&!labAnalyticsSql.includes('resource_url')&&!labAnalyticsSql.includes('chat')&&!labAnalyticsSql.includes('task_description')&&!labAnalyticsSql.includes('application')],

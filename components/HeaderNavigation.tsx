...
export default function HeaderNavigation({dropdowns}:{dropdowns:DropdownConfig[]}){
  ...
  useEffect(()=>{
    ...
    const refresh=(container:HTMLElement)=>{
      ...
      const pages=Math.ceil(items.length/PAGE_SIZE);const page=Math.max(1,Math.min(pages,Number(container.dataset.metteloPage||'1')||1));container.dataset.metteloPage=String(page);

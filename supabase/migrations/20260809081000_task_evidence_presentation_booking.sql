alter table public.contributions add column if not exists task_id uuid references public.project_tasks(id) on delete set null;
...
declare
  actor uuid:=auth.uid();
  slot_record public.presentation_slots%rowtype;
...
begin
  ...
  if not found or slot_record.status<>'available' then raise exception 'Presentation slot is no longer available'; end if;
  ...
  foreach presenter in array target_presenters loop
    if not exists(select 1 from public.project_members pm where pm.project_id=target_project and pm.user_id=presenter) then raise exception 'Every presenter must be a project member'; end if;
  end loop;
  insert into public.project_presentations(project_id,slot_id,meeting_url,deck_url,status,booked_by,updated_at)
  values(target_project,target_slot,slot_record.meeting_url,target_deck_url,'booked',actor,now())
  on conflict(project_id) do update set slot_id=excluded.slot_id,meeting_url=excluded.meeting_url,deck_url=excluded.deck_url,status='booked',booked_by=actor,reviewer_notes=null,updated_at=now()
  returning id into presentation_id;
  delete from public.project_presenters where presentation_id=project_presentations.id;
  insert into public.project_presenters(presentation_id,user_id) select presentation_id,x from unnest(target_presenters) x on conflict do nothing;
...
begin
 ...
 if target_status='presented' then
   ...
   if team_role not in ('reviewer','admin') then raise exception 'Reviewer or admin access required'; end if;

...
const base={project:open,applicationReady:true,capacityAvailable:true,capacityKnown:true,now:new Date('2026-08-19T12:00:00Z').getTime()};

test('member project resolver implements the Phase 5 qualification matrix',()=>{
...
test('resolver hands lifecycle ownership to the correct destination',()=>{
 ...
 for(const state of ['closed','ineligible','full','cancelled'] as const)expect(memberProjectPrimaryAction(state,'p1')).toBeNull();

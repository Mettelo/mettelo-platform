...
const iconByStatus:Record<string,string>={draft:'○',pilot:'◇',open:'●',recruiting:'●',forming:'◎',active:'✓',paused:'Ⅱ',blocked:'!',starting_soon:'◷',start_scheduled:'◷',needs_attention:'!',review:'◷',completed:'✓',archived:'—',private:'◒',public:'●',submitted:'◷',in_review:'◷',under_review:'◷',clarification_requested:'?',shortlisted:'★',offered:'→',approved:'✓',declined:'×',failed:'!',inactive:'○',success:'✓',completed_sync:'✓',waiting:'◷',confirmed:'✓'};
...
export default function AdminStatusBadge({status,label}:{status:string;label?:string}){
  ...
  const tone=['active','approved','completed','public','success','confirmed'].includes(key)?'success':['forming','shortlisted','offered','pilot','review','waiting','starting_soon','start_scheduled','clarification_requested'].includes(key)?'warning':['failed','declined','blocked','needs_attention'].includes(key)?'danger':['paused','archived','inactive','private'].includes(key)?'neutral':'info';

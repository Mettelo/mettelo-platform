  ...
  return <>
    <section className="section softSection"><div className="shell publicProfileShell">
      <div className="publicProfileHero">
        ...
        <div><div className="eyebrow">Mettelo professional profile</div><h1>{name}</h1><p className="lead">{headline}</p>{profile.location&&<p className="publicProfileLocation">{profile.location}</p>}<div className="actions">{profile.linkedin_url&&<a className="button ghost" href={profile.linkedin_url} target="_blank" rel="noopener noreferrer">LinkedIn ↗</a>}{profile.github_url&&<a className="button ghost" href={profile.github_url} target="_blank" rel="noopener noreferrer">GitHub ↗</a>}</div></div>
      </div>

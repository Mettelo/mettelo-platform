...
async function experienceReadiness(db:Awaited<ReturnType<typeof architectContext>> extends {db:infer T}?T:never,projectId:string){
...
export async function POST(request:Request){
  ...
  try{
    ...
    if(resourceInputs.length){const {error:resourceError}=await db.from('project_data_sources').insert(resourceInputs.map(({index,invalid_url,...item})=>({...item,project_id:project.id,project_run_id:null,owner_user_id:null,version_label:null,sensitivity:item.sensitivity,access_status:'needs_access',quality_status:'unreviewed',download_policy:'team_only',publish_policy:'not_permitted',retention_policy:'unknown',internal_storage_policy:'unknown',governance_status:'unreviewed',added_by:user.id})));if(resourceError)throw resourceError}
    if(deliverableInputs.length){const {error:deliverableError}=await db.from('project_deliverables').insert(deliverableInputs.map(({index,...item})=>({...item,project_id:project.id,project_run_id:null,workstream_id:null,owner_user_id:null,reviewer_user_id:null,status:'planned',created_by:user.id})));if(deliverableError)throw deliverableError}
    if(successInputs.length){const {error:successError}=await db.from('project_success_criteria').insert(successInputs.map(({index,...item})=>({...item,project_id:project.id,created_by_user_id:user.id})));if(successError)throw successError}
    if(milestoneInputs.length){const {error:milestoneError}=await db.from('project_milestones').insert(milestoneInputs.map(({index,...item})=>({...item,project_id:project.id,status:'planned'})));if(milestoneError)throw milestoneError}

...
export default function MemberProjectDetailV2({model,state,stateLabel,stateCopy,primaryAction,applicationReady,profileCompletion,applicationMissing,saved,roles,contributionAreas,fit,teamState}:Props){
 const project=model.project;const workingModel=project.locationType?.replaceAll('_',' ')||project.location||'Project-specific';
 const incompleteAction=state==='ineligible'&&!applicationReady?{label:'Submit Interest',href:`/member/profile?next=${encodeURIComponent(`/member/discover/${project.id}`)}`}:null;
 const action=primaryAction||incompleteAction;
 return <div className={styles.page}>
  ...
  <header className={styles.hero} aria-labelledby="member-project-title"><div className={styles.heroMain}><div className={styles.eyebrow}>MEMBER PROJECT DETAIL</div><h1 id="member-project-title">{project.title}</h1><p>{project.summary}</p><div className={styles.pills} aria-label="Project characteristics"><span className={`${styles.pill} ${state==='open_eligible'?styles.open:''}`}>{stateLabel}</span>{project.difficultyLevel&&<span className={styles.pill}>{project.difficultyLevel}</span>}{project.durationWeeks&&<span className={styles.pill}>{project.durationWeeks} {project.durationWeeks===1?'week':'weeks'}</span>}{project.weeklyCommitment&&<span className={styles.pill}>{project.weeklyCommitment}</span>}<span className={styles.pill}>{humanMode(project.participationMode)}</span><span className={styles.pill}>{workingModel}</span></div></div>
   <aside className={styles.decision} aria-labelledby="member-decision-title"><div className={styles.eyebrow}>YOUR DECISION</div><h2 id="member-decision-title">{stateLabel}</h2><p>{stateCopy}</p><dl className={styles.meta}><div><dt>Team state</dt><dd>{teamState.stateLabel}</dd></div><div><dt>Confirmed</dt><dd>{teamState.known?teamState.confirmedMembers:'Checking'}</dd></div><div><dt>Minimum to start</dt><dd>{teamState.minTeamSize??'Not published'}</dd></div><div><dt>Target team</dt><dd>{teamState.targetTeamSize??'Not published'}</dd></div><div><dt>Maximum team</dt><dd>{teamState.maxTeamSize??'Not published'}</dd></div><div><dt>Interest closes</dt><dd>{date(project.applicationDeadline)}</dd></div></dl>{teamState.known&&teamState.reservedMembers>0&&<p className={styles.readiness} role="status"><strong>{teamState.reservedMembers} place{teamState.reservedMembers===1?' is':'s are'} currently reserved/offered</strong><span>Reserved places count toward current capacity while team formation is in progress.</span></p>}{state==='ineligible'&&!applicationReady&&<div className={styles.readiness} role="status"><strong>{applicationMissing.length} profile requirement{applicationMissing.length===1?'':'s'} remaining</strong><span>Profile completion: {profileCompletion}%</span></div>}{action&&<Link className={styles.primaryButton} href={action.href}>{action.label}</Link>}<div className="pdv2SaveUtility"><SaveProjectButton projectId={project.id} initialSaved={saved}/></div>{state==='ineligible'&&!applicationReady&&<p className={styles.readiness} role="status">Submit Interest will first take you to complete the missing profile information, then return you to this exact project.</p>}</aside>
  </header>

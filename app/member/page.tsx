...
type Application={id:string;status:string;submitted_at:string;project_id:string;application_kind?:string;requested_role?:string|null;projects:{title:string;status:string}|null;project_roles:{title:string}|null;formation?:Formation|null};
...
export default async function MemberDashboard(){
  ...
  const [profileResult,appsResult,membersResult,proofResult,savedResult,tasksResult]=await Promise.all([
    ...
    supabase.from('project_applications').select('id,status,submitted_at,project_id,application_kind,requested_role,projects(title,status),project_roles(title)',{count:'exact'}).eq('user_id',user.id).order('submitted_at',{ascending:false}).limit(3),
  ...
  return <section className={`${styles.memberScope} section softSection memberWorkspace`}><div className="shell memberDashboardMain">
    <header className="memberHero"><div className="eyebrow">My Mettelo</div><h1>Welcome, {name}.</h1><p className="lead">Your work comes first: continue active projects, track requests, then deal with anything that needs attention.</p></header>
    <div className="metricGrid memberMetricGrid"><a className="metric" href="/member/projects"><strong>{active.length}</strong><span>Active projects</span></a><a className="metric" href="/member/applications"><strong>{appsResult.count||0}</strong><span>Project requests</span></a><a className="metric" href="/member/proof"><strong>{proofResult.count||0}</strong><span>Verified Proof</span></a><a className="metric" href="/member/saved-opportunities"><strong>{savedResult.count||0}</strong><span>Saved opportunities</span></a></div>

    <section className="panel memberSection" style={{marginTop:18}}><div className="panelHead"><div><span className="cardNumber">ACTIVE WORK</span><h3 style={{marginTop:7}}>Continue where you left off</h3></div><a className="linkArrow" href="/member/projects">My Projects →</a></div>{active.length?<div className="memberProjectList">{active.slice(0,4).map(item=><div className="listRow" key={item.id}><div><strong>{item.projects?.title||'Mettelo Labs project'}</strong><br/><small>{item.team_role.replaceAll('_',' ')} · active workspace</small></div><a className="button dark" href={`/member/projects/${item.project_id}`}>Open workspace →</a></div>)}</div>:<div className="emptyState"><h3>No active workspace yet.</h3><p>When an approved project reaches kickoff, your shared team workspace appears here automatically.</p><a className="linkArrow" href="/member/applications">Track project requests →</a></div>}</section>

    <section className="panel memberSection" style={{marginTop:18}}><div className="panelHead"><div><span className="cardNumber">APPLICATION PROGRESS</span><h3 style={{marginTop:7}}>Latest interests and applications</h3></div><a className="linkArrow" href="/member/applications">View all requests →</a></div><MemberApplicationTracker applications={apps}/></section>

    <div className="dashboardGrid" style={{marginTop:18}}><section className="panel"><div className="panelHead"><div><span className="cardNumber">NEXT ACTIONS</span><h3 style={{marginTop:7}}>Needs your attention</h3></div></div>{attention.length?attention.map((item,index)=><div className="listRow" key={index}><div><strong>{item.title}</strong><br/><small>{item.text}</small></div><a className="button ghost" href={item.href}>{item.cta} →</a></div>):<div className="emptyState"><h3>You are up to date.</h3><p>Nothing needs action right now. Explore recommendations when you are ready for something new.</p><a className="linkArrow" href="/member/recommended">See recommendations →</a></div>}</section>
    <aside className="panel"><div className="panelHead"><div><span className="cardNumber">ASSIGNED TO YOU</span><h3 style={{marginTop:7}}>Open tasks</h3></div><a className="linkArrow" href="/member/projects">All projects →</a></div>{tasks.length?tasks.slice(0,4).map(task=><div className="listRow" key={task.id}><div><strong>{task.title}</strong><br/><small>{task.projects?.title||'Mettelo project'}{task.due_at?` · due ${new Date(task.due_at).toLocaleDateString('en-GB')}`:''}</small></div><span className="chip">{task.status.replaceAll('_',' ').toUpperCase()}</span></div>):<div className="emptyState"><h3>No open tasks.</h3><p>Assigned project work will appear here.</p></div>}</aside></div>
  </div></section>;
}

...
test('My Mettelo Home V4 matches the approved hierarchy and responsive visual contract',async({page})=>{
  ...
  for(const viewport of viewports){
    ...
    if(viewport.width<=480){
      ...
      await mobileNav.locator(':scope > details > summary').click();const more=page.locator('#member-more');await expect(more).toBeVisible();for(const name of ['Capability Paths','Applications','Recommended','Opportunities','Saved','Events','Spotlight','Profile'])await expect(more.getByText(name,{exact:true})).toBeVisible();await assertNoHorizontalOverflow(page,`${viewport.name}/more`);await mobileNav.locator(':scope > details > summary').click();await expect(more).toBeHidden();
    }else{
      ...
      const groups=['My Work','Direction & Discovery','Opportunities & Community'];if(viewport.width>=1025){for(const group of groups)await expect(desktopNav.getByRole('heading',{name:group})).toBeVisible()}else{for(const group of groups)await expect(desktopNav.getByRole('heading',{name:group})).toBeHidden()}
    }
...
test('Explore and Grow actions land on the intended launch-ready destinations',async({page})=>{
  ...
  const journeys=[
    ...
    {name:'Opportunities',path:'/opportunities',heading:'Explore relevant routes forward.'},

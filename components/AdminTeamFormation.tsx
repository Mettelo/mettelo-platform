  ...
  return <div className="teamFormationAdmin">
    ...
    {!filtered.length?<div className="adminEmpty"><h3>No teams match these filters</h3><p>Try another project, status, or search term.</p></div>:pageGroups.map(group=><section className="projectTeamGroup" key={group.id}>
      ...
      <div className="runList">{group.teams.map(item=>{
        ...
        const canForceStart=item.status==='forming'&&item.filled>0&&readiness?.system?.ready===true;
        ...
        return <article className="runCard" key={item.run_id}>
          ...
          <div className="teamActions"><button className="button dark" type="button" disabled={!canStart||working!==''} title={canStart?'Canonical readiness passed.':blockers.length?`Blocked: ${blockers.map(words).join(', ')}`:'Readiness has not passed.'} onClick={()=>act(item,'start')}>Start normally</button><button className="button ghost" type="button" disabled={!canForceStart||working!==''||!(reason[item.run_id]||'').trim()||String(reason[item.run_id]||'').trim().length<8} title={canForceStart?'Force start overrides ordinary Team formation blockers only after hard Lab/system readiness passes.':'Force start requires a forming run, at least one confirmed member and green Lab/system readiness.'} onClick={()=>act(item,'force_start')}>Force start</button>{item.status==='paused'?<button className="button dark" type="button" disabled={working!==''} onClick={()=>act(item,'resume')}>Resume team</button>:['forming','active'].includes(item.status)&&<><label>Admin intervention reason<input value={reason[item.run_id]||''} onChange={e=>setReason(current=>({...current,[item.run_id]:e.target.value}))} placeholder="Required reason"/></label><button className="button ghost" type="button" disabled={working!==''||!(reason[item.run_id]||'').trim()} onClick={()=>act(item,'pause')}>Pause team</button></>}<a className="button ghost" href={`/member/projects/${item.id}?run=${item.run_id}`}>Open workspace</a></div>
        </article>
      })}</div>
    </section>)}

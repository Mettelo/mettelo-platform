...
type Item={
  ...
  flexible_preference:string|null;
  effective_participation_mode:'solo'|'team'|'flexible';
  start_threshold:number;
  team_configuration:{minimum:number;target:number;maximum:number};
  leadership_interest:boolean;
  reviewer_notes?:string|null;
  profile_completeness?:number;
  admission_lane:'auto'|'review_required';
...
function canForceStart(item:Item){return item.admission_lane==='review_required'&&Boolean(item.project_id)&&['accepted','waiting_for_team'].includes(item.status)}
function nextActions(item:Item):ReviewStatus[]{
  if(item.admission_lane==='auto')return[];
...
export default function AdminApplicationQueue({initialItems}:{initialItems:Item[]}){
  ...
  const[autoStartPending,setAutoStartPending]=useState<Item|null>(null);
  const[forceStartPending,setForceStartPending]=useState<Item|null>(null);
  ...
  const visible=useMemo(()=>{
    ...
    const rows=items.filter(item=>(projectFilter==='all'||item.project===projectFilter)&&(typeFilter==='all'||(typeFilter==='review_required'&&item.admission_lane==='review_required')||(typeFilter==='auto'&&item.admission_lane==='auto')||(typeFilter==='partner'&&partner(item))||(typeFilter==='open'&&!partner(item)))&&(!allowed.length||allowed.includes(item.status))&&(!dateFrom||new Date(item.submitted_at)>=new Date(`${dateFrom}T00:00:00`))&&(!dateTo||new Date(item.submitted_at)<=new Date(`${dateTo}T23:59:59`)));
  ...
  async function update(item:Item,status:ReviewStatus,override?:string,note?:string){
    const response=await fetch('/api/admin/applications',{method:'PATCH',headers:{'content-type':'application/json'},body:JSON.stringify({id:item.id,status,expected_status:item.status,reviewer_notes:(note||'').trim(),custom_message:(override||'').trim()||defaults[status](item)})});
    ...
    setDetail(current=>current?.id===item.id?{...current,status:payload.application?.status||status,reviewer_notes:(note||current.reviewer_notes||null)}:current);
  }
  async function startReview(item:Item){
    if(item.admission_lane!=='review_required'||item.status!=='submitted'){setMessage('Only a submitted REVIEW_REQUIRED request can start human review.');return}
    setWorking(true);setMessage('');
    try{
      await update(item,'in_review','',item.reviewer_notes||'');
      const reviewed={...item,status:'in_review'};
      setDetail(reviewed);
      setMessage(`${requestLabel(item)} moved to In review. The review workspace is now open.`);
      setSelected(current=>current.filter(id=>id!==item.id));
    }catch(error){setMessage(error instanceof Error?error.message:'Unable to start review.')}finally{setWorking(false)}
  }

  async function startAutoProject(item:Item){
    if(item.admission_lane!=='auto'||item.auto_start?.state!=='READY_TO_START'){setMessage('This AUTO project is not currently ready to start. Refresh the page and review its current status.');return}
    setWorking(true);setMessage('');
    try{
  ...
  async function forceStartProject(item:Item){
    if(!canForceStart(item)||!item.project_id){setMessage('Force start is available only for an accepted REVIEW_REQUIRED project place.');return}
    if(forceStartReason.trim().length<8){setMessage('Enter a clear reason for force-starting this project.');return}
    setWorking(true);setMessage('');
    try{
      const response=await fetch('/api/admin/project-admission',{
        ...
        body:JSON.stringify({
          ...
          action:'force_start_application',
          reason:forceStartReason.trim()
        })
      });
      const body=await response.json().catch(()=>({}));
      if(!response.ok)throw new Error(body.error||'Unable to force start this project.');
      setMessage(`${item.project} force-started successfully. The action and reason were recorded in the audit log.`);
      setForceStartPending(null);setForceStartReason('');setDetail(null);
      setTimeout(()=>window.location.reload(),700);
    }catch(error){setMessage(error instanceof Error?error.message:'Unable to force start this project.')}finally{setWorking(false)}
  }
  ...
  return <div className="adminApplications">
    <div className="applicationToolbar" role="search" aria-label="Filter project requests">
      ...
      <label>Admission lane<select value={typeFilter} onChange={e=>{setTypeFilter(e.target.value);resetPage()}}><option value="all">All submissions</option><option value="review_required">Review required</option><option value="auto">AUTO</option><option value="partner">Partner Projects</option><option value="open">Open Projects</option></select></label>
      <label>Status<select aria-label="Filter project requests by status" value={statusFilter} onChange={e=>{setStatusFilter(e.target.value as StatusFilter);resetPage()}}><option value="all">All statuses</option><option value="current">Current review work</option><option value="submitted">Submitted</option><option value="in_review">In review</option><option value="clarification_requested">Clarification requested</option><option value="shortlisted">Shortlisted</option><option value="offered">Offered</option><option value="declined">Declined</option></select></label>
    ...
    {!items.length?<div className="adminEmpty"><h3>No project submissions found</h3><p>Every project interest and application submission will appear here, including AUTO and REVIEW_REQUIRED lanes.</p></div>:!visible.length?<div className="adminEmpty"><h3>No project requests match these filters</h3><button className="button ghost" type="button" onClick={()=>{setProjectFilter('all');setTypeFilter('all');setStatusFilter('all');setDateFrom('');setDateTo('');setPage(1)}}>Clear filters</button></div>:<>
      <div className="applicationTableWrap"><table className="applicationTable"><thead><tr><th><input type="checkbox" aria-label="Select project requests on this page" checked={pageItems.length>0&&pageItems.every(item=>selected.includes(item.id))} onChange={e=>setSelected(current=>e.target.checked?[...new Set([...current,...pageItems.map(item=>item.id)])]:current.filter(id=>!pageItems.some(item=>item.id===id)))}/></th><th>Member</th><th>Project</th><th>Submitted</th><th>Status</th><th>Formation</th><th><span className="srOnly">Actions</span></th></tr></thead><tbody>{pageItems.map(item=><tr key={item.id}><td><input type="checkbox" checked={selected.includes(item.id)} onChange={()=>toggle(item.id)} aria-label={`Select ${item.name}`}/></td><td><button className="applicantButton" type="button" onClick={()=>setDetail(item)}><span className="miniAvatar" style={item.avatar_url?{backgroundImage:`url(${item.avatar_url})`}:undefined}>{item.avatar_url?'':item.name.slice(0,1).toUpperCase()}</span><span><strong>{item.name}</strong><small>{item.username?`@${item.username}`:item.email||item.member_id||'Member'}</small></span></button></td><td><div className="projectCell"><div className="projectBadges"><span className={partner(item)?'partnerBadge':item.admission_lane==='auto'?'autoBadge':'openBadge'}>{partner(item)?'PARTNER PROJECT':item.admission_lane==='auto'?'OPEN · AUTO':'OPEN · REVIEW REQUIRED'}</span>{partner(item)&&item.partner_name&&<span className="partnerName">{item.partner_name}</span>}</div><a href={item.project_id?`/admin/project-operations/projects/${item.project_id}`:'#'}>{item.project}</a><small>{requestLabel(item)} · {item.participation_preference||item.role}</small>{item.leadership_interest&&<span className="leadershipBadge">Willing to lead</span>}</div></td><td>{new Date(item.submitted_at).toLocaleString('en-GB',{dateStyle:'medium',timeStyle:'short'})}</td><td><AdminStatusBadge status={item.status}/></td><td><span className="capacityLine">{item.start_threshold} member{item.start_threshold===1?'':'s'} to start</span><small>{item.effective_participation_mode==='team'?`Team path · ${item.capacity.confirmed} confirmed · ${item.capacity.reservedOffers||0} offered`:`${item.effective_participation_mode==='solo'?'Solo path':'Flexible path'} · ${item.capacity.confirmed} confirmed`}</small><small>Project team config: Min {item.team_configuration.minimum} · Target {item.team_configuration.target} · Max {item.team_configuration.maximum}</small></td><td><details className="rowMenu"><summary aria-label={`Actions for ${item.name}`}>⋯</summary><div><button type="button" onClick={()=>setDetail(item)}>{item.status==='in_review'?'View review':'View submission'}</button>{canForceStart(item)&&<button className="forceStartAction" type="button" disabled={working} onClick={()=>{setForceStartReason('');setForceStartPending(item)}}>Force start project</button>}{item.admission_lane==='auto'&&item.project_id&&<a className="rowMenuLink" href={`/admin/project-operations/projects/${item.project_id}`}>Manage AUTO start</a>}{item.admission_lane==='auto'&&item.auto_start?.state==='READY_TO_START'&&<button type="button" disabled={working} onClick={()=>setAutoStartPending(item)}>Start project now</button>}{item.admission_lane==='auto'&&item.project_id&&item.auto_start?.state!=='READY_TO_START'&&<a className="rowMenuLink" href={`/admin/project-operations/projects/${item.project_id}`}>{item.auto_start?.state==='NEEDS_ATTENTION'?'View issue':item.auto_start?.state==='STARTED'?'View project':'View status'}</a>}{nextActions(item).map(status=><button className={status==='declined'?'danger':undefined} type="button" key={status} disabled={working} onClick={()=>status==='in_review'&&item.status==='submitted'?void startReview(item):openAction(status,[item.id])}>{actionLabel(status,item.status)}</button>)}</div></details></td></tr>)}</tbody></table></div>
      <div className="applicationMobileList">{pageItems.map(item=><article key={item.id}><div className="mobileAppTop"><input type="checkbox" checked={selected.includes(item.id)} onChange={()=>toggle(item.id)} aria-label={`Select ${item.name}`}/><button type="button" onClick={()=>setDetail(item)}><strong>{item.name}{item.username?` · @${item.username}`:''}</strong><small>{partner(item)?`Partner Project${item.partner_name?` · ${item.partner_name}`:''}`:item.admission_lane==='auto'?'Open · AUTO':'Open · Review required'} · {item.project}</small></button><details className="rowMenu"><summary aria-label={`Actions for ${item.name}`}>⋯</summary><div><button type="button" onClick={()=>setDetail(item)}>{item.status==='in_review'?'View review':'View submission'}</button>{canForceStart(item)&&<button className="forceStartAction" type="button" disabled={working} onClick={()=>{setForceStartReason('');setForceStartPending(item)}}>Force start project</button>}{item.admission_lane==='auto'&&item.project_id&&<a className="rowMenuLink" href={`/admin/project-operations/projects/${item.project_id}`}>Manage AUTO start</a>}{item.admission_lane==='auto'&&item.auto_start?.state==='READY_TO_START'&&<button type="button" disabled={working} onClick={()=>setAutoStartPending(item)}>Start project now</button>}{item.admission_lane==='auto'&&item.project_id&&item.auto_start?.state!=='READY_TO_START'&&<a className="rowMenuLink" href={`/admin/project-operations/projects/${item.project_id}`}>{item.auto_start?.state==='NEEDS_ATTENTION'?'View issue':item.auto_start?.state==='STARTED'?'View project':'View status'}</a>}{nextActions(item).map(status=><button type="button" className={status==='declined'?'danger':undefined} key={status} disabled={working} onClick={()=>status==='in_review'&&item.status==='submitted'?void startReview(item):openAction(status,[item.id])}>{actionLabel(status,item.status)}</button>)}</div></details></div><div className="mobileAppMeta"><AdminStatusBadge status={item.status}/><span>{item.capacity.confirmed} confirmed</span><span>{item.capacity.reservedOffers||0} offered</span><span>{openPlaces(item)} open</span></div></article>)}</div>
    ...
    {detail&&<div className="modalBackdrop" onMouseDown={event=>{if(event.target===event.currentTarget)setDetail(null)}}><section className="detailModal" role="dialog" aria-modal="true" aria-labelledby="application-detail-heading"><div className="modalHead"><div><span className="eyebrow">{partner(detail)?'PARTNER PROJECT REVIEW':detail.admission_lane==='auto'?'OPEN PROJECT · AUTO OVERSIGHT':'OPEN PROJECT REVIEW'}</span><h2 id="application-detail-heading">{detail.name}</h2><p>{detail.username?`@${detail.username} · `:''}{detail.member_id?`${detail.member_id} · `:''}{detail.project}{partner(detail)&&detail.partner_name?` · ${detail.partner_name}`:''}</p></div><button type="button" aria-label={`Close ${requestNoun(detail)} detail`} onClick={()=>setDetail(null)}>×</button></div>
      <div className="contextBanner"><strong>{partner(detail)?'Human review is mandatory':detail.admission_lane==='auto'?'AUTO submission is visible for Admin oversight before start':'This Open Project is configured for human review'}</strong><span>{detail.admission_lane==='auto'?'AUTO admission remains automatic. Formation and start controls are operational actions, not human application review.':'Selection creates an Offer only. It does not create membership or open Lab.'}</span></div>
      <div className="detailGrid">
        ...
        <section><h3>Project context</h3><dl><div><dt>Project type</dt><dd>{partner(detail)?'Partner Project':detail.admission_lane==='auto'?'Open Project · AUTO':'Open Project · REVIEW_REQUIRED'}</dd></div>{partner(detail)&&<div><dt>Partner organisation</dt><dd>{detail.partner_name||'Not supplied'}</dd></div>}<div><dt>Project participation</dt><dd>{detail.participation_mode||'Not supplied'}</dd></div><div><dt>Member participation choice</dt><dd>{detail.participation_preference||'Historical choice not recorded — review submission evidence'}</dd></div><div><dt>Effective formation path</dt><dd>{detail.effective_participation_mode}</dd></div><div><dt>Start threshold</dt><dd>{detail.start_threshold} confirmed member{detail.start_threshold===1?'':'s'}</dd></div><div><dt>Difficulty</dt><dd>{detail.difficulty||'Not supplied'}</dd></div><div><dt>Commitment</dt><dd>{detail.weekly_commitment||'Not supplied'}</dd></div><div><dt>Recruitment</dt><dd>{detail.recruitment_state||'Open for review'}</dd></div><div><dt>Leadership</dt><dd>{detail.leadership_interest?'Willing to be considered for Project Lead.':'Not volunteering for Project Lead.'}</dd></div><div><dt>Current project capacity</dt><dd>{detail.capacity.confirmed} confirmed · {detail.capacity.reservedOffers||0} offered · {openPlaces(detail)} open</dd></div><div><dt>Team configuration</dt><dd>Min {detail.team_configuration.minimum} · Target {detail.team_configuration.target} · Max {detail.team_configuration.maximum}{detail.effective_participation_mode==='solo'?' · does not change this Solo request threshold':''}</dd></div></dl></section>

...
export default async function ProfilePage(){
  ...
  return <section className="section softSection memberWorkspace"><ProfileReturnAfterSave/><div className="shell"><div className="sectionHead"><div><div className="eyebrow">My profile</div><h1>Help Mettelo understand what you can do.</h1></div><p>Add the skills, experience and availability that matter for project matching. You can come back and update this whenever your experience changes.</p></div><div className="profilePurpose" role="note" aria-label="How your profile is used"><div><strong>Better project matches</strong><span>Skills and interests help us show more relevant work.</span></div><div><strong>Faster applications</strong><span>A complete profile means less information to repeat when you apply.</span></div><div><strong>Clearer professional identity</strong><span>Your public profile can show people what you do and the Proof you build.</span></div></div><MemberProfileSection userId={user.id} profile={profile} domains={domains} tools={tools} domainPreferences={domainPreferences} toolPreferences={toolPreferences}/></div><style>{`.profilePurpose{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;margin:0 0 20px}.profilePurpose>div{display:grid;gap:5px;padding:15px 16px;border:1px solid rgba(16,19,29,.08);border-radius:14px;background:#fff}.profilePurpose strong{font-size:.8rem}.profilePurpose span{color:#68717e;font-size:.72rem;line-height:1.5}@media(max-width:760px){.profilePurpose{grid-template-columns:1fr}.profilePurpose>div{padding:14px}}`}</style></section>;
}

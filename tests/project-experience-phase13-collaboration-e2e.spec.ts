...
test.describe('Project Experience Phase 13 collaboration',()=>{
 test('Chat categories, mention preferences and active-team boundaries work end to end',async({page})=>{
  test.slow();const db=service();const senderId=await memberUserId(db);const email=`phase13-mention-${Date.now()}@example.test`;const created=await db.auth.admin.createUser({email,password,email_confirm:true});if(created.error||!created.data.user)throw created.error||new Error('Could not create Phase 13 mention target.');const targetId=created.data.user.id;const messageIds:string[]=[];
  try{
   const senderMembership=await db.from('project_members').select('id,membership_status').eq('project_id',projectId).eq('project_run_id',runId).eq('user_id',senderId).maybeSingle();if(senderMembership.error)throw senderMembership.error;if(!senderMembership.data)throw new Error('Phase 13 requires the seeded member to belong to the Lab run.');await db.from('project_members').update({membership_status:'active'}).eq('id',senderMembership.data.id);
   ...
   await db.from('project_members').update({membership_status:'active'}).eq('id',senderMembership.data.id);
  }finally{await db.from('project_members').update({membership_status:'active'}).eq('project_id',projectId).eq('project_run_id',runId).eq('user_id',senderId);await cleanup(db,targetId,email,messageIds)}
 });
});

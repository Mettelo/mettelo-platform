...
const checks=[
 ...
 ['nested legacy Back actions cannot expose member or admin return buttons',regressions.includes('a[href="/member"]')&&regressions.includes('a[href="/admin/project-operations"]')],
 ['Support is a first-class Lab destination and is removed from Home',nav.includes("{view:'support',label:'Support'")&&view.includes("'support'")&&regressions.includes('[data-lab-view="home"] [data-lab-support-section]')&&regressions.includes('[data-lab-view="support"] #mettelo-lab')],
 ['mobile Chat composer assigns message type input and send to explicit rows',regressions.includes('grid-template-areas:"type type" "input send"')&&regressions.includes('grid-area:type')&&regressions.includes('grid-area:input')&&regressions.includes('grid-area:send')],
 ['mobile More items override inherited bottom-nav centering',regressions.includes('.labMoreDropupPanel a')&&regressions.includes('grid-template-columns:32px minmax(0,1fr) 24px!important')&&regressions.includes('justify-items:stretch!important')],

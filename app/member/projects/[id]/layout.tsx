 ...
 return <div className={styles.workspace}>
  ...
  <div className={styles.shell}>
   <aside className={styles.rail} aria-label="Mettelo Lab workspace">
    <div className={styles.railIdentity}><p className={styles.kicker}>METTELO LAB</p><a className={styles.projectTitle} href="#mettelo-lab">{project.title}</a><div className={styles.context} aria-label="Project status and your context"><span>{humanise(workspaceStatus)}</span><span>{humanise(membership?.team_role||(isAdmin?'admin':'member'))}</span></div></div>
    <div className={styles.navGroup}><p className={styles.navTitle}>Core</p><nav className={styles.nav} aria-label="Mettelo Lab primary navigation">{primaryNavigation.map(([label,href])=><a href={href} key={label}>{label}{label==='Chat'&&unreadCount>0?<span className={styles.count} aria-label={`${unreadCount} unread messages`}>{unreadCount}</span>:null}</a>)}</nav></div>
    <div className={styles.navGroup}><p className={styles.navTitle}>Project tools</p><nav className={styles.nav} aria-label="Mettelo Lab project tools">{projectTools.map(([label,href])=><a href={href} key={label}>{label}</a>)}</nav></div>
   ...
   <div className={styles.stage}>
    ...
    <main id="mettelo-lab-content" className={styles.main} tabIndex={-1}>
     ...
     <div className={styles.content}>{children}</div><ProjectConversationReadMarker projectRunId={runId} lastMessageId={lastMessageId}/>
    </main>
    <nav className={styles.mobileNav} aria-label="Mettelo Lab mobile navigation">{[['Home','#mettelo-lab'],['Tasks','#delivery'],['Chat','#discussion'],['Data','#phase4-data-governance'],['More','#lab-more']] .map(([label,href])=><a href={href} key={label}>{label}{label==='Chat'&&unreadCount>0?<span className={styles.mobileCount}>{unreadCount}</span>:null}</a>)}</nav>
    <section id="lab-more" className={styles.moreMenu} aria-labelledby="lab-more-title"><h2 id="lab-more-title">More</h2><div>{[['Plan','#problem'],['Proof','#phase4-contributions'],['Resources','#resources'],['Events','#meetings'],['Team','#team']].map(([label,href])=><a href={href} key={label}>{label}</a>)}</div></section>
   </div>
  </div>
 </div>;
}

 ...
 return <section className={styles.metteloLab} id="mettelo-lab" aria-labelledby="mettelo-lab-title">
  ...
  <section className={styles.teamRoster} data-lab-team-section id="team" aria-labelledby="team-roster-title">
   <div className={styles.teamHero}>
    <div><span className={styles.cardNumber}>YOUR TEAM · METTELO LAB</span><h3 id="team-roster-title">{current?`Team ${current.run_number}`:'Your team'}</h3><p className={styles.teamIntro}>See the active people working with you on this project, their responsibilities and the current team status.</p></div>
    <div className={styles.teamSnapshot} aria-label="Team status"><span>{current?humanise(current.status):'Forming'}</span><strong>{current?`${visibleMembers.length}/${current.required_team_size??visibleMembers.length}`:'—'}</strong><small>{current?'active members':'team placement'}</small></div>
   </div>
   {current?<>
    ...
    {props.projectRunId?<ProjectGrowTeamSection projectId={props.projectId} projectRunId={props.projectRunId} workspaceRole={props.workspaceRole} activeMemberCount={visibleMembers.length} isAdmin={isAdmin}/>:null}

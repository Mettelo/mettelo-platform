...
export default function MemberProofPortfolio({verifiedItems,pendingItems,rejectedItems,verifiedTotal,pendingTotal,projectsEvidenced,pendingLoadFailed,credential}:Props){
  ...
  return <>
    <section className="mpSummary" aria-label="Proof summary">
      <article className="mpSummaryCard mpSummaryVerified"><strong>{verifiedTotal}</strong><span>Verified Proof</span><small>Evidence Mettelo has verified from your project contributions</small></article>
      {projectsEvidenced!==null&&<article className="mpSummaryCard"><strong>{projectsEvidenced}</strong><span>Projects evidenced</span><small>Distinct projects represented in your verified Proof</small></article>}
      <article className="mpSummaryCard"><strong>{pendingTotal}</strong><span>Pending</span><small>{pendingTotal===1?'One evidence item is still being reviewed':'Evidence still being reviewed is not counted as verified'}</small></article>
    </section>

    {verified.length>0&&<section className="mpFilterbar" aria-label="Proof filters">
      <label><span className="mpSrOnly">Search Proof</span><input className="mpInput" type="search" value={query} onChange={event=>setQuery(event.target.value)} placeholder="Search Proof or projects"/></label>
    ...
    <section className="mpSection" aria-labelledby="verified-proof-title">
      <div className="mpSectionHead"><div><div className="mpEyebrow">YOUR VERIFIED PROOF</div><h2 id="verified-proof-title">Evidence you can stand behind</h2><p>Each item is a real contribution Mettelo has verified. Project completion, task completion and profile claims do not create Proof on their own.</p></div><span className="mpCount">{verifiedTotal} verified {verifiedTotal===1?'item':'items'}</span></div>
      {verified.length===0?<div className="mpEmpty"><h3>Your Proof starts with project work</h3><p>Verified Proof appears here when Mettelo confirms evidence from your project contributions.</p><div className="mpActions"><a className="mpBtn mpPrimary" href="/member/projects">View Projects</a><a className="mpBtn" href="/projects">Discover projects</a></div></div>:
      ...
      <div className="mpEmpty" aria-live="polite"><h3>No Proof matches these filters</h3><p>Try a different project or search term.</p><button className="mpBtn mpPrimary" type="button" onClick={resetFilters}>Clear filters</button></div>}
      {verifiedTotal>verified.length&&<p className="mpLimitNote">Showing the {verified.length} most recently verified items. The portfolio query is deliberately bounded so the first screen does not fetch an unlimited evidence history.</p>}
    </section>

    {pendingLoadFailed?<section className="mpPendingWarning" role="status"><strong>Pending evidence could not be loaded.</strong><span>Your verified Proof above is still available. Refresh to retry the review queue.</span></section>:pending.length>0&&<section className="mpSection mpPendingSection" aria-labelledby="pending-proof-title">
      <div className="mpSectionHead"><div><div className="mpEyebrow">BEING VERIFIED</div><h2 id="pending-proof-title">Evidence still in review</h2><p>Pending evidence is not Verified Proof. Items only move into the portfolio after the real verification state changes to verified.</p></div><span className="mpCount">{pendingTotal} pending {pendingTotal===1?'item':'items'}</span></div>
    ...
    {rejectedItems.length>0&&<details className="mpHistory"><summary>Review history · {rejectedItems.length} not verified</summary><div className="mpHistoryList">{rejectedItems.map(item=><article key={item.id}><div><span className="mpStatus">Not verified</span><strong>{item.title}</strong><small>{item.project_title||'Mettelo project'} · reviewed {formatDate(item.updated_at||item.created_at)}</small></div><button className="mpBtn" type="button" onClick={()=>setSelected(item)} aria-label={`View review outcome for ${item.title}`}>View outcome</button></article>)}</div></details>}

    <section className="mpIdentity" aria-label="Proof connections">
      <article className="mpIdentityCard"><div className="mpEyebrow">YOUR PROFESSIONAL IDENTITY</div><h2>Bring verified evidence into your profile</h2><p>Your Profile can reflect your professional identity and claims, while Proof remains the detailed record of evidence Mettelo has actually verified.</p><div className="mpActions"><a className="mpBtn mpPrimary" href="/member/profile">View profile</a>{credential&&<a className="mpBtn" href={`/credentials/${credential.credential_id}`}>Open credential</a>}</div>{credential&&<small className="mpIdentityNote">Your {credential.status} Project Architect credential remains a separate verification record, issued {formatDate(credential.issued_at)}.</small>}</article>
      <article className="mpIdentityCard mpSpotlight"><div className="mpEyebrow">PROOF ≠ PUBLICATION</div><h2>Spotlight stays separate</h2><p>Verified Proof does not automatically become public recognition. Spotlight uses its own publication flow and preserves member consent.</p><div className="mpActions"><a className="mpBtn" href="/member/spotlight">View Spotlight</a></div></article>
    </section>
...
function VerifiedCard({item,featured,onOpen}:{item:MemberProofItem;featured:boolean;onOpen:(item:MemberProofItem)=>void}){
  return <article className={`mpProofCard${featured?' mpFeatured':''}`}>
    <span className="mpStatus mpVerified">✓ Verified</span>
    <h3>{item.title}</h3>
    {item.description&&<p>{item.description}</p>}
    {item.project_title&&<div className="mpProjectSource"><small>Source project</small><strong>{item.project_title}</strong></div>}
    <div className="mpVerifyRow">{item.project_role&&<div><small>Project role</small><strong>{item.project_role}</strong></div>}<div><small>Verified</small><strong>{formatDate(item.verified_at)}</strong></div></div>
    <div className="mpActions mpCardActions"><button className="mpBtn mpPrimary" type="button" onClick={()=>onOpen(item)} aria-label={`View Proof: ${item.title}`}>View Proof</button>{item.can_view_project&&item.project_id&&<a className="mpBtn" href={`/member/projects/${item.project_id}${item.project_run_id?`?run=${item.project_run_id}`:''}`}>View project</a>}</div>
  </article>;
}
...
function PendingCard({item,onOpen}:{item:MemberProofItem;onOpen:(item:MemberProofItem)=>void}){
  ...
  return <article className="mpPendingCard"><div><span className={`mpStatus ${changes?'mpNeedsChanges':'mpPending'}`}>{changes?'! Changes requested':'◷ Pending verification'}</span><h3>{item.title}</h3><p>{changes?'Mettelo reviewed this evidence and needs an update before verification can continue.':'This evidence is being reviewed before it can appear in your verified Proof portfolio.'}</p>{item.project_title&&<small className="mpPendingProject">Source project · {item.project_title}</small>}</div><div className="mpPendingSide"><small>{changes?'YOUR NEXT STEP':'YOUR NEXT STEP'}</small><strong>{changes?'Update and resubmit':'No action needed'}</strong><p>{changes?(item.review_notes||'Open this evidence record to update the contribution statement or evidence link requested by the reviewer.'):'Mettelo is reviewing this evidence. We’ll update the status when verification is complete.'}</p>{changes&&<button className="mpBtn mpPrimary" type="button" onClick={()=>onOpen(item)}>Review &amp; resubmit</button>}</div></article>;
}
...
function ProofDetail({item,description,evidenceUrl,setDescription,setEvidenceUrl,resubmit,working,statusMessage,close}:{item:MemberProofItem;description:string;evidenceUrl:string;setDescription:(value:string)=>void;setEvidenceUrl:(value:string)=>void;resubmit:(event:React.FormEvent)=>void;working:boolean;statusMessage:string;close:()=>void}){
  ...
  return <>
    <div className="mpDialogHead"><div><div className="mpEyebrow">{verified?'VERIFIED PROOF':changes?'EVIDENCE UPDATE':'PROOF REVIEW'}</div><h2 id="mp-proof-dialog-title">{item.title}</h2></div><button className="mpDialogClose" type="button" aria-label="Close Proof detail" onClick={close}>×</button></div>
    <div className="mpDialogBody">
      <div className="mpDetailGrid"><div className="mpDetail"><small>Verification</small><strong>{verified?'✓ Verified by Mettelo':changes?'Changes requested':item.verification_status==='pending'?'Pending verification':'Not verified'}</strong></div>{item.verified_at&&<div className="mpDetail"><small>Verified date</small><strong>{formatDate(item.verified_at)}</strong></div>}{item.project_title&&<div className="mpDetail"><small>Source project</small><strong>{item.project_title}</strong></div>}{item.project_role&&<div className="mpDetail"><small>Project role</small><strong>{item.project_role}</strong></div>}<div className="mpDetail"><small>Evidence type</small><strong>{humanise(item.contribution_type)}</strong></div></div>
      {item.description&&<section className="mpEvidenceBlock"><h3>What this evidence says</h3><p>{item.description}</p></section>}
      ...
      {changes&&<form className="mpResubmit" onSubmit={resubmit}><h3>Update your evidence</h3>{item.review_notes&&<div className="mpReviewFeedback"><strong>Feedback from this review</strong><p>{item.review_notes}</p></div>}<label>Contribution statement<textarea value={description} minLength={30} maxLength={2000} required onChange={event=>setDescription(event.target.value)}/></label><label>Evidence URL <span>(optional if your existing project-ledger evidence is still valid)</span><input type="url" inputMode="url" value={evidenceUrl} onChange={event=>setEvidenceUrl(event.target.value)} placeholder="https://…"/></label><button className="mpBtn mpPrimary" type="submit" disabled={working}>{working?'Resubmitting…':'Resubmit for verification'}</button><div className="mpFormStatus" role="status" aria-live="polite">{statusMessage}</div></form>}
      {verified&&<section className="mpVisibilityBlock"><h3>Visibility is separate from verification</h3><p>Changing who can see this record does not change whether Mettelo has verified it, and it does not publish you to Spotlight.</p><ProofVisibilityControl id={item.id} initialVisibility={item.visibility}/>{publicProof&&<><SocialShare url={`https://mettelo.com/proof/${item.id}`} text={`Verified contribution Proof on Mettelo: ${item.title}${item.project_title?` · ${item.project_title}`:''}.`} label="Share this Proof"/><a className="mpBtn" href={`/proof/${item.id}`}>Open public Proof</a></>}</section>}
    </div>

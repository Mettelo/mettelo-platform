...
import {isWebsiteCmsPageKey,validateWebsiteCmsPagePayload} from '@/lib/website-pages-cms';
...
export async function GET(request:Request){
 try{
  ...
  const items=(data||[]).map(row=>{const validated=validateWebsiteCmsPagePayload(ctx.page,row.payload);return{...row,payload:validated.ok?validated.payload:null,valid:validated.ok,actor_name:row.created_by?(names.get(row.created_by)||'Admin user'):'System baseline'}});
...
export async function POST(request:Request){
 try{
  ...
  const validated=validateWebsiteCmsPagePayload(ctx.page,revision.payload);if(!validated.ok)return NextResponse.json({error:'This revision is not valid for the current page schema.'},{status:409});

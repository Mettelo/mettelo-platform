...
export type ProjectTeamOverviewMember={id:string;name:string;headline:string|null;avatar_url:string|null;role:string;status:string;can_submit_final_proof:boolean};
...
export async function resolveProjectTeamOverview({db,projectId,userId,isAdmin,currentRunId=null}:{db:Db;projectId:string;userId:string;isAdmin:boolean;currentRunId?:string|null}):Promise<ProjectTeamOverview|null>{
 ...
 const teams=visibleRuns.map(run=>{
  ...
  return{id:isMember?run.id:`cohort-${run.run_number}`,run_number:run.run_number,status:run.status,required_team_size:isMember?run.required_team_size:null,has_started:isMember?run.has_started:null,is_member:isMember,members:isMember?(members||[]).filter(member=>member.project_run_id===run.id).map(member=>({id:member.user_id,name:profileMap.get(member.user_id)?.full_name||'Mettelo member',headline:profileMap.get(member.user_id)?.headline||null,avatar_url:profileMap.get(member.user_id)?.avatar_url||null,role:member.team_role,status:member.membership_status,can_submit_final_proof:(permissions||[]).some(permission=>permission.project_run_id===run.id&&permission.user_id===member.user_id)})):[]};
 });

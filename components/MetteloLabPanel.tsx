...
export default async function MetteloLabPanel(props:Props){
 ...
 return <section className={styles.metteloLab} id="mettelo-lab" aria-labelledby="mettelo-lab-title">
  ...
  <section className={styles.teamRoster} data-lab-team-section id="team" aria-labelledby="team-roster-title">
   ...
   {current?<><div className={styles.teamMeta}><span><strong>Your role</strong><b>{humanise(props.workspaceRole)}</b></span><span><strong>Project Lead</strong><b>{lead?.name||'Not assigned yet'}</b></span><span><strong>Project Architect</strong><b>{architect||'Not assigned yet'}</b></span></div><div className={styles.teamGrid}>{visibleMembers.length?visibleMembers.map(member=><RosterMember key={member.id} member={member} currentUserId={props.currentUserId} canManageSubmissionPermissions={props.canManageSubmissionPermissions} completionHref={viewHref('proof')}/>):<div className={styles.labEmpty}><strong>No active team members are available.</strong><p>Only active participants in this run appear in the delivery roster.</p></div>}</div>{props.projectRunId?<ProjectGrowTeamSection projectId={props.projectId} projectRunId={props.projectRunId} workspaceRole={props.workspaceRole} activeMemberCount={visibleMembers.length} isAdmin={isAdmin}/>:null}</>:<div className={styles.labEmpty}><strong>Your team is still being formed.</strong><p>Your working team will appear here once placement is complete.</p></div>}
  </section>

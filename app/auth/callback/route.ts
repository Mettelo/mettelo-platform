...
function signinTarget(url:URL,flow:string,error:string,next:string){const target=new URL('/signin',url.origin);target.searchParams.set('error',error);if(flow==='signup')target.searchParams.set('mode','signup');if(flow==='recovery')target.searchParams.set('mode','reset');if(next)target.searchParams.set('next',next);return target}

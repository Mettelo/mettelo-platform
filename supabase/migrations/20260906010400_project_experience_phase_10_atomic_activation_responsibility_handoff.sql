-- Project Experience Phase 10 -> Phase 11 readiness handoff hardening.
--
-- Keep the existing Phase 9 atomic activation boundary. Only replace its obsolete
-- single project_members.project_role_id coverage check with Phase 10's canonical
-- normalized delivery-responsibility assignments. Project Lead remains the
-- established project_members.team_role relationship and must already have been
-- confirmed through authorized Phase 10 logic.
...
begin
  ...
  if coalesce(run_row.has_started,false)=true or run_row.status='active' then
    return jsonb_build_object(
      'started',false,'already_started',true,'run_number',run_row.run_number,
      'filled',filled,'required_team_size',required_members
    );
  end if;

  if run_row.status<>'forming' then
    return jsonb_build_object(
      'started',false,'not_ready',true,'blockers',jsonb_build_array('run_lifecycle'),
      'run_number',run_row.run_number,'filled',filled,'required_team_size',required_members
    );
  end if;

  if project_row.status in ('cancelled','completed','archived') then
    return jsonb_build_object(
      'started',false,'not_ready',true,'blockers',jsonb_build_array('project_lifecycle'),
      'run_number',run_row.run_number,'filled',filled,'required_team_size',required_members
    );
  end if;
  ...
  if p_source='auto_scheduler' and effective_admission<>'auto' then
    return jsonb_build_object(
      'started',false,'not_ready',true,'blockers',jsonb_build_array('admission_mode'),
      'run_number',run_row.run_number,'filled',filled,'required_team_size',required_members
    );
  end if;
  ...
  if run_row.auto_start_blocked_at is not null then
    return jsonb_build_object(
      'started',false,'blocked',true,'blockers',jsonb_build_array('auto_start_blocked'),
      'run_number',run_row.run_number,'filled',filled,'required_team_size',required_members
    );
  end if;
  ...
  if p_source='auto_scheduler'
    ...
    return jsonb_build_object(
      'started',false,'paused',true,'run_number',run_row.run_number,
      'filled',filled,'required_team_size',required_members
    );
  end if;
  ...
  if p_source='auto_scheduler'
    ...
    return jsonb_build_object(
      'started',false,'not_ready',true,'blockers',jsonb_build_array('schedule_not_due'),
      'run_number',run_row.run_number,'filled',filled,'required_team_size',required_members
    );
  end if;
  ...
  if filled<required_members then
    return jsonb_build_object(
      'started',false,'not_ready',true,'blockers',jsonb_build_array('team_size'),
      'run_number',run_row.run_number,'filled',filled,'required_team_size',required_members
    );
  end if;
  ...
  if filled>maximum_members then
    return jsonb_build_object(
      'started',false,'not_ready',true,'blockers',jsonb_build_array('capacity'),
      'run_number',run_row.run_number,'filled',filled,'required_team_size',required_members,
  ...
  if not lab_ready then
    return jsonb_build_object(
      'started',false,'not_ready',true,'blockers',jsonb_build_array('project_readiness'),
      'run_number',run_row.run_number,'filled',filled,'required_team_size',required_members
    );
  end if;

  -- Independent threshold-1 delivery does not receive artificial Team-only Lead
  -- or responsibility gates. Multi-member delivery requires every live member to
  -- own at least one active responsibility from Phase 10's normalized relation.
  if required_members>1 then
    select count(*)::integer into missing_responsibilities
    from public.project_members m
    where m.project_run_id=run_row.id
      and m.membership_status in ('waiting','active')
      and not exists (
        select 1
        from public.project_member_responsibilities r
        where r.project_member_id=m.id
          and r.project_run_id=run_row.id
          and r.assignment_status='active'
      );

    if missing_responsibilities>0 then
      return jsonb_build_object(
        'started',false,'not_ready',true,'blockers',jsonb_build_array('responsibility_coverage'),
        'run_number',run_row.run_number,'filled',filled,'required_team_size',required_members
      );
    end if;
    ...
    if lead_count=0 then
      return jsonb_build_object(
        'started',false,'not_ready',true,'blockers',jsonb_build_array('project_lead'),
        'run_number',run_row.run_number,'filled',filled,'required_team_size',required_members
      );
    end if;
    if lead_count>1 then
      return jsonb_build_object(
        'started',false,'not_ready',true,'blockers',jsonb_build_array('multiple_project_leads'),
        'run_number',run_row.run_number,'filled',filled,'required_team_size',required_members
      );
    end if;
  ...
  set status='active',has_started=true,started_at=now_at,kickoff_at=now_at,
      scheduled_start_at=null,start_scheduled_at=null,start_ready_at=null,
      auto_start_failure=null,auto_start_paused_at=null,auto_start_pause_reason=null,
      auto_start_paused_by_user_id=null,updated_at=now_at
  ...
  set membership_status='active',activated_at=coalesce(activated_at,now_at)
  where project_run_id=run_row.id and membership_status='waiting';
  ...
  if project_row.project_type='partner' then
    ...
    set status='active',applications_open=false,kickoff_at=now_at,starts_at=now_at,updated_at=now_at
  ...
  insert into public.project_activity_log(
    ...
    project_row.id,run_row.id,event_type,
    case when p_source='auto_scheduler' then 'system' else 'user' end,
    p_actor_user_id,'forming','active',
    jsonb_build_object(
      'run_number',run_row.run_number,'filled',filled,'required_team_size',required_members,
      'maximum_team_size',maximum_members,'participation_mode',project_row.participation_mode,
      'lead_user_id',lead_user_id,'responsibility_coverage_ready',missing_responsibilities=0,
      'responsibility_contract','phase10_normalized_assignments','lab_ready',lab_ready,
      'admission_mode',effective_admission,'activation_contract','phase9_atomic'
    )
  );

  return jsonb_build_object(
    'started',true,'run_number',run_row.run_number,'filled',filled,
    'required_team_size',required_members,'maximum_team_size',maximum_members,

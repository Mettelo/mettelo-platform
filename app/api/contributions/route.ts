...
export async function POST(request:Request){
  try{
    ...
    const {data:membership}=await supabase.from('project_members').select('id,team_role').eq('project_id',projectId).eq('user_id',user.id).maybeSingle();
    ...
    if(taskId){
      ...
      if(!task||task.project_id!==projectId) return NextResponse.json({error:'Choose a task from this project.'},{status:400});
    ...
    const {data,error}=await supabase.from('contributions').insert({user_id:user.id,project_id:projectId,task_id:taskId,contribution_type:contributionType,title,description,evidence_url:evidenceUrl,verification_status:'pending',is_public:isPublic}).select('id,verification_status').single();

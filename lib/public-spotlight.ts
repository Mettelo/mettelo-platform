...
async function projectPublicSpotlights(id:string|null,limit:number){
  ...
  if(!publicDb)return [] as PublicSpotlightAward[];
  ...
  if(error||!data?.length)return [] as PublicSpotlightAward[];
  ...
  if(userIds.length){
    const {data:profiles}=await publicDb.from('profiles').select('id').in('id',userIds).eq('is_public',true).gte('profile_readiness',PROFILE_APPLICATION_READY);
  ...
  if(projectIds.length){
    ...
    const {data:projects}=await publicDb.from('projects').select('id,title').in('id',projectIds).eq('visibility','public');
  ...
  if(service){
    ...
    if(spotlightIds.length){
      const {data:links}=await service.from('spotlight_evidence').select('spotlight_id,contribution_id,is_primary').in('spotlight_id',spotlightIds).eq('is_primary',true);
      ...
      if(contributionIds.length){
        ...
        const {data:proof}=await publicDb.from('contributions').select('id,title,verified_at').in('id',contributionIds).eq('verification_status','verified').eq('visibility','public');

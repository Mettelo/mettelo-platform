...
export async function GET(request:Request){
 ...
 try{
  ...
  const {data,error}=await db.from('saved_opportunities').select('user_id,opportunity_id,reminders_enabled,opportunities!inner(id,title,organisation,status,closes_at)').eq('reminders_enabled',true).eq('opportunities.status','published').not('opportunities.closes_at','is',null).gte('opportunities.closes_at',now.toISOString()).lte('opportunities.closes_at',horizon.toISOString());if(error)throw error;
  let notified=0;

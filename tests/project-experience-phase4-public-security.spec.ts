...
test('public project detail uses the governed anon projection, not serviceDb helpers',()=>{
  ...
  for(const protectedName of ['external_url','storage_path','content_pointer','provider_url','licence_url','access_notes','review_agreement_accepted','preview_data','sample_rows','download_enabled','query_enabled'])expect(migration).not.toContain(`'${protectedName}'`);
});

test('anon rich projection is visible only while the project is public and omits protected resource fields',async()=>{
  ...
  for(const protectedName of ['external_url','storage_path','content_pointer','provider_url','licence_url','access_notes','review_agreement_accepted','preview_data','sample_rows','download_enabled','query_enabled'])expect(serialized).not.toContain(`\"${protectedName}\"`);

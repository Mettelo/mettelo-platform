...
async function assertResponsiveSurface(page:Page,url:string,label:string){
 for(const width of [320,375,390,414,768,1024,1440]){await page.setViewportSize({width,height:900});await page.goto(url,{waitUntil:'networkidle'});await noOverflow(page,`${label} overflowed at ${width}px`)}
 await page.setViewportSize({width:768,height:900});await page.goto(url,{waitUntil:'networkidle'});await page.evaluate(()=>{document.documentElement.style.fontSize='200%'});await noOverflow(page,`${label} overflowed at 200% text`);await page.evaluate(()=>{document.documentElement.style.fontSize=''})
}

test.describe('Capability Paths Phase 4 member lifecycle',()=>{
 test('follow, primary direction, governed completion reuse and separate Proof remain truthful',async({page})=>{
  ...
  try{
   ...
   await page.goto('/member/discover',{waitUntil:'networkidle'});const cardA=page.locator('article.mcpCard').filter({hasText:'E2E Member Data Analyst'});const cardB=page.locator('article.mcpCard').filter({hasText:'E2E Member BI Analyst'});await expect(cardA).toContainText('PRIMARY PATH');await expect(cardA).toContainText('0/1');await expect(cardA).toContainText('Verified Proof');await expect(cardA).toContainText('1');await expect(cardB).toContainText('0/1');await expect(cardA.getByRole('link',{name:'Explore next project'})).toHaveCount(0);await expect(cardA.getByRole('button',{name:'Unfollow Path'})).toBeVisible();

   const pauseB=await memberApi.post('/api/member/capability-paths',{data:{action:'pause',path_id:pathB.id}});expect(pauseB.status()).toBe(200);await page.reload({waitUntil:'networkidle'});const pausedB=page.locator('article.mcpCard').filter({hasText:'E2E Member BI Analyst'});await expect(pausedB).toContainText('Path paused');await expect(pausedB.getByRole('link',{name:'Explore next project'})).toHaveCount(0);await expect(pausedB.getByRole('button',{name:'Resume Path'})).toBeVisible();const resumeB=await memberApi.post('/api/member/capability-paths',{data:{action:'resume',path_id:pathB.id}});expect(resumeB.status()).toBe(200);

   await page.goto(`/member/projects/${projectId}?run=${runId}`,{waitUntil:'networkidle'});await expect(page.getByText('WHY THIS PROJECT MATTERS',{exact:true})).toBeVisible();await expect(page.getByText('E2E Member Data Analyst',{exact:true})).toBeVisible();

   const {error:projectModeError}=await service.from('projects').update({project_type:'partner',status:'active'}).eq('id',projectId);if(projectModeError)throw projectModeError;const {error:runModeError}=await service.from('project_runs').update({status:'review'}).eq('id',runId);if(runModeError)throw runModeError;
   const {data:completionRequest,error:completionRequestError}=await service.from('project_completion_requests').insert({project_id:projectId,project_run_id:runId,requested_by_user_id:memberId,status:'pending',readiness_snapshot:{source:'capability-path-phase-4-e2e'},review_notes:null}).select('id').single();if(completionRequestError)throw completionRequestError;completionRequestId=completionRequest.id;
   ...
   await page.context().clearCookies();await signIn(page,'member','/member/discover');await page.goto('/member/discover',{waitUntil:'networkidle'});await expect(page.locator('article.mcpCard').filter({hasText:'E2E Member Data Analyst'})).toContainText('1/1');await expect(page.locator('article.mcpCard').filter({hasText:'E2E Member BI Analyst'})).toContainText('1/1');
   const memberApiAfter=page.context().request;const primaryB=await memberApiAfter.post('/api/member/capability-paths',{data:{action:'set_primary',path_id:pathB.id}});expect(primaryB.status()).toBe(200);await page.reload({waitUntil:'networkidle'});await expect(page.locator('article.mcpCard').filter({hasText:'E2E Member BI Analyst'})).toContainText('PRIMARY PATH');await expect(page.locator('article.mcpCard').filter({hasText:'E2E Member Data Analyst'})).toContainText('1/1');

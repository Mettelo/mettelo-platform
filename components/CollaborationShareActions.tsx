...
export default function CollaborationShareActions({url,text,collaborationNeedId}:Props){
 ...
 function track(event_type:'share_linkedin'|'share_x'|'share_whatsapp'|'share_copy'){void fetch('/api/collaboration-analytics',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({event_type,collaboration_need_id:collaborationNeedId}),keepalive:true}).catch(()=>undefined)}

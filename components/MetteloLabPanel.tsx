...
export default async function MetteloLabPanel(props:Props){
 ...
 const canonicalRunId=current?.id||props.projectRunId;
 const visibleMembers=current?current.members.filter(member=>member.status==='active'||(props.runStatus==='completed'&&member.status==='completed')):[];
 ...
 return <section className={styles.metteloLab} id="mettelo-lab" aria-labelledby="mettelo-lab-title">
  ...
  <section className={styles.summary} data-lab-home-section aria-labelledby="lab-summary-title">
   ...
   <div className={styles.summaryGrid} aria-label="Current project delivery context"><Stat label="Project status" value={humanise(props.runStatus)}/><Stat label="Team" value={current?`Team ${current.run_number} · ${visibleMembers.length} member${visibleMembers.length===1?'':'s'}`:'Team forming'}/><Stat label="Current milestone" value={milestoneLabel}/><Stat label="Next meeting" value={meetingLabel}/><Stat label="Blockers" value={blockerLabel}/><Stat label="Upcoming work" value={upcomingLabel}/></div>
   <div className={styles.progressGrid}><ProgressCard label="Milestones" completed={props.completedMilestones} total={props.totalMilestones} percent={milestoneProgress}/><ProgressCard label="Tasks" completed={props.completedTasks} total={props.totalTasks} percent={taskProgress}/></div>
  ...
  <section className={styles.teamRoster} data-lab-team-section id="team" aria-labelledby="team-roster-title">
   <div className={styles.teamHero}><div><span className={styles.cardNumber}>YOUR TEAM · METTELO LAB</span><h3 id="team-roster-title">{current?`Team ${current.run_number}`:'Your team'}</h3><p className={styles.teamIntro}>See the active people working with you on this project, their responsibilities and the current team status.</p></div><div className={styles.teamSnapshot} aria-label="Team status"><span>{current?humanise(current.status):'Forming'}</span><strong>{current?`${visibleMembers.length}/${current.required_team_size??visibleMembers.length}`:'—'}</strong><small>{current?'active members':'team placement'}</small></div></div>
   {current?<><div className={styles.teamMeta}><span><strong>Your role</strong><b>{humanise(props.workspaceRole)}</b></span><span><strong>Project Lead</strong><b>{lead?.name||'Not assigned yet'}</b></span><span><strong>Project Architect</strong><b>{architect||'Not assigned yet'}</b></span></div>{canonicalRunId?<ProjectGrowTeamSection projectId={props.projectId} projectRunId={canonicalRunId} workspaceRole={props.workspaceRole} activeMemberCount={visibleMembers.length} isAdmin={isAdmin}/>:null}<div className={styles.teamGrid}>{visibleMembers.length?visibleMembers.map(member=><RosterMember key={member.id} member={member} currentUserId={props.currentUserId} canManageSubmissionPermissions={props.canManageSubmissionPermissions} completionHref={viewHref('proof')}/>):<div className={styles.labEmpty}><strong>No active team members are available.</strong><p>Only active participants in this run appear in the delivery roster.</p></div>}</div></>:<div className={styles.labEmpty}><strong>Your team is still being formed.</strong><p>Your working team will appear here once placement is complete.</p></div>}
  </section>
  ...
  {canonicalRunId?<MetteloLabSupportView projectId={props.projectId} projectRunId={canonicalRunId} runStatus={current?.status||props.runStatus} canCreateSupport={Boolean(current?.members.some(member=>member.id===props.currentUserId&&member.status==='active'))}/>:null}
  ...
  {props.projectRunId?<div data-lab-review-section id="lab-completion-review"><ProjectFinalProofPanel projectId={props.projectId} projectRunId={props.projectRunId} currentUserId={props.currentUserId} team={props.team}/>{props.reviewSlot}</div>:props.reviewSlot?<div data-lab-home-section>{props.reviewSlot}</div>:null}
 </section>;
}
...
function RosterMember({member,currentUserId,canManageSubmissionPermissions,completionHref}:{member:ProjectTeamOverviewMember;currentUserId:string;canManageSubmissionPermissions:boolean;completionHref:string}){const isCurrent=member.id===currentUserId;const responsibilityLabel=member.responsibilities.length?member.responsibilities.join(' · '):'Not assigned yet';return <article className={styles.teamMember} data-current-user={isCurrent?'true':undefined}>{member.avatar_url?<span className={`${styles.personAvatar} ${styles.personAvatarPhoto}`} style={{backgroundImage:`url(${member.avatar_url})`}} aria-label={`${member.name} profile photo`}/>:<span className={styles.personAvatar} aria-hidden="true">{initials(member.name)}</span>}<div className={styles.memberContent}><div className={styles.memberHeading}><div><strong>{member.name}</strong>{member.username&&<small>@{member.username}</small>}{isCurrent&&<span className={styles.youLabel}>You</span>}</div><span className={styles.rolePill}>{roleLabel(member.role)}</span></div>{member.headline&&<p className={styles.memberHeadline}>{member.headline}</p>}<div className={styles.memberMeta}><span>Status · {humanise(member.status)}</span><span>Responsibilities · {responsibilityLabel}</span>{member.can_submit_final_proof&&<span>Can submit final Proof</span>}</div>{canManageSubmissionPermissions&&!isCurrent&&<a className={styles.permissionLink} href={completionHref}>Manage submission permissions →</a>}</div></article>}

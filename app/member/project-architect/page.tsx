...
export default async function ProjectArchitectPage(){
  ...
  const [{data:identity},{data:application},{data:proof},{data:profile}]=await Promise.all([
    ...
    supabase.from('profiles').select('headline,bio,skills').eq('id',user.id).maybeSingle()
  ]);
  const [{data:evidence},{data:history},{data:credential}]=application?await Promise.all([
    supabase.from('project_architect_application_evidence').select('*').eq('application_id',application.id).order('created_at'),
    supabase.from('project_architect_application_history').select('id,from_status,to_status,note,created_at').eq('application_id',application.id).order('created_at',{ascending:false}),
  ...
  const eligibility={profile_ready:Boolean(profile?.headline&&profile?.bio&&(profile?.skills||[]).length>=3),has_verified_proof:Boolean((proof||[]).length),has_evidence:Boolean((evidence||[]).length)};
  return <section className="section softSection memberWorkspace"><div className="shell"><div className="sectionHead"><div><div className="eyebrow">Volunteer · Project leadership</div><h1>Become a Project Architect.</h1></div><p>Share your experience, work examples and project idea. If approved, you can help Mettelo teams shape and deliver Data &amp; AI projects while building a stronger professional track record grounded in reviewed contribution.</p></div><ProjectArchitectApplication initialApplication={application} initialEvidence={evidence||[]} history={history||[]} proof={(proof||[]) as never[]} eligibility={eligibility} identity={identity||{account_type:'member',show_project_architect_designation:false}} credential={credential}/></div></section>;
}
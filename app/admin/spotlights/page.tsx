...
export default async function AdminSpotlightsPage(){
  ...
  let loadError=false;
  if(db){
    const {data,error}=await db.from('spotlights').select('id,user_id,title,category,summary,award_month,score,score_breakdown,status,is_excluded,exclusion_reason,consent_status,publication_held,hold_reason,suppress_public_project,suppress_public_evidence,primary_project_id').order('award_month',{ascending:false}).order('created_at',{ascending:false}).limit(60);
    if(error){console.error('admin Spotlight list load failed',error);loadError=true;}
    else{
      const rows=(data||[]) as unknown as Row[];
      const ids=rows.map(row=>row.id);
      const userIds=[...new Set(rows.map(row=>row.user_id).filter(Boolean))];
      const projectIds=[...new Set(rows.map(row=>row.primary_project_id).filter(Boolean) as string[])];
      const [profilesResult,evidenceResult,projectsResult]=await Promise.all([
        userIds.length?db.from('profiles').select('id,full_name,headline').in('id',userIds):Promise.resolve({data:[],error:null}),

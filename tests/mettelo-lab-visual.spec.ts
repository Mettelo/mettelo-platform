...
test('Mettelo Lab matches the approved responsive composition screen by screen',async({page})=>{
 test.setTimeout(180_000);await page.emulateMedia({reducedMotion:'reduce'});await signIn(page,basePath);await mkdir('artifacts/mettelo-lab-visual',{recursive:true});
 for(const viewport of viewports){
  await page.setViewportSize({width:viewport.width,height:viewport.height});await page.goto(basePath,{waitUntil:'networkidle'});await assertNoHorizontalOverflow(page,viewport.name);
  const labRail=page.locator('aside[aria-label="Mettelo Lab workspace"]'),rightRail=page.locator('aside[aria-label="Mettelo Lab project context"]'),mobileNav=page.getByRole('navigation',{name:'Mettelo Lab mobile navigation'}),outerMemberNav=page.getByRole('navigation',{name:'Member mobile navigation'});await expect(outerMemberNav).toBeHidden();
  if(viewport.width<=480){await expect(labRail).toBeHidden();await expect(page.getByText('METTELO LAB',{exact:true}).first()).toBeVisible();await expect(mobileNav).toBeVisible();for(const link of await mobileNav.getByRole('link').all()){const box=await link.boundingBox();expect(box?.height||0,`${viewport.name}: mobile Lab navigation target`).toBeGreaterThanOrEqual(44)}}else{await expect(mobileNav).toBeHidden();await expect(labRail).toBeVisible();await expect(labRail.getByRole('heading',{name:'Mettelo Lab'})).toBeVisible()}
  if(viewport.width>=1181)await expect(rightRail).toBeVisible();else await expect(rightRail).toBeHidden();
  for(const [screen,selector] of screens){const target=page.locator(selector).first();await expect(target,`${viewport.name}/${screen}: target exists`).toBeVisible();await target.scrollIntoViewIfNeeded();await page.waitForTimeout(80);await assertNoHorizontalOverflow(page,`${viewport.name}/${screen}`);if(viewport.width<=480&&screen==='chat'){const composer=page.locator('.messageComposer');await expect(composer).toBeVisible();const[composerBox,navBox]=await Promise.all([composer.boundingBox(),mobileNav.boundingBox()]);expect(composerBox).not.toBeNull();expect(navBox).not.toBeNull();if(composerBox&&navBox)expect(composerBox.y+composerBox.height,`${viewport.name}: Chat composer must sit above nav`).toBeLessThanOrEqual(navBox.y+2);const fontSize=await composer.locator('textarea').evaluate(element=>Number.parseFloat(getComputedStyle(element).fontSize));expect(fontSize,`${viewport.name}: composer input avoids iOS focus zoom`).toBeGreaterThanOrEqual(16)}await page.screenshot({path:`artifacts/mettelo-lab-visual/${viewport.name}-${screen}.png`,fullPage:false,animations:'disabled'})}
 }
});

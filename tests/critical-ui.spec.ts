...
test.describe('critical public journeys',()=>{
  ...
  test('contact form sends the expected API contract and reaches confirmation',async({page})=>{
    ...
    await page.locator('[name="name"]').fill('Regression Contact');
    await page.locator('[name="email"]').fill('contact@example.test');
    await page.locator('[name="topic"]').selectOption({label:'Technical issue'});
    await page.locator('[name="subject"]').fill('Regression form contract');
    await page.locator('[name="message"]').fill('This verifies that the contact form still sends every required value.');
  ...
  test('partnership form sends the expected API contract and reaches confirmation',async({page})=>{
    ...
    await page.locator('[name="organisation"]').fill('Regression Organisation');
    await page.locator('[name="name"]').fill('Regression Partner');
    await page.locator('[name="email"]').fill('partner@example.test');
    await page.locator('[name="role"]').fill('Engineering lead');
    ...
    await page.locator('[name="objective"]').fill('Validate the complete partnership intake journey before every release.');
    await page.locator('[name="contribution"]').fill('Provide a safe staging scenario and a clear expected business outcome.');
    await page.locator('[name="consent"]').check();
  ...
  test('feedback form sends the expected API contract and reaches confirmation',async({page})=>{
    ...
    await page.locator('[name="email"]').fill('feedback@example.test');
    await page.locator('[name="area"]').selectOption({label:'Navigation / mobile'});
    await page.locator('[name="message"]').fill('The regression suite confirms this form remains wired after visual changes.');

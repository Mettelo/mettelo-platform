 ...
 * governance workflows. Public Project Detail therefore reads them server-side
 * and selects only presentation-safe planning fields. Do not add internal notes,
 * reviewer identities, task execution state, private links or governance evidence
 * to this projection.
 */
export async function getProjectExperiencePlanning(projectId:string):Promise<{brief:ProjectExperienceBrief|null;milestones:ProjectExperienceMilestone[]}>
{
  ...
  const [briefResult,milestonesResult]=await Promise.all([
    ...
    db.from('project_milestones').select('id,title,description,week_start,week_end,expected_output,sort_order').eq('project_id',projectId).order('sort_order',{ascending:true}).order('created_at',{ascending:true})
  ]);

-- Phase 9 final Offer response lock order.
-- Canonical ordering for capacity-changing project operations is:
--   project row -> Phase 9 project-capacity advisory lock -> Offer row.
-- This matches project/membership/Offer-capacity guards and prevents response
-- from holding the advisory lock while waiting for a project row already held by
-- another transaction that is itself waiting for the advisory lock.
...
begin
  ...
  where id=offer_hint.project_id
  for update;

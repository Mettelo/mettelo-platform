...
export async function PATCH(request:Request){
  try{
    ...
    if(status!=='approved'){
      const patch:{status:string;reviewer_notes:string|null;decision_reason?:string|null;decision_at?:string}={
        status,
    ...
    // Pausing intake stops new submissions; applications already received can
    // still complete review while the project remains eligible for placement.
    if(
      ['completed','cancelled','archived'].includes(project.status)||
      (project.project_type==='partner'&&['active','review'].includes(project.status))
    ){
      return NextResponse.json({error:'This project can no longer accept a new team placement.'},{status:409});
    }

    const {data:priorMembership,error:priorMembershipError}=await db
      .from('project_members')
      .select('id,project_run_id,membership_status')
    ...
    if(priorMembership){
      if(
        application.project_run_id&&
        priorMembership.project_run_id===application.project_run_id&&
        ['approved','waiting_for_team','team_complete','accepted'].includes(application.status)
      ){
        return NextResponse.json({
          ok:true,
          already_approved:true,
          application:{id:application.id,status:application.status},
          team:{id:priorMembership.project_run_id}
        });
      }
    ...
    const {data:role,error:roleError}=await db
      .from('project_roles')
      .select('id,title,openings')
      .eq('id',application.project_role_id)
      .eq('project_id',application.project_id)
      .maybeSingle();
    if(roleError)throw roleError;
    if(!role||Number(role.openings||0)<1){
      return NextResponse.json({error:'The selected project role is no longer available. Choose another role before approval.'},{status:409});
    }
    ...
    const roleFilled=usage.filled.get(role.id)||0;
    if(roleFilled>=role.openings){
      return NextResponse.json({error:`${role.title} is full for the current team. Choose another role or wait for the next Open Project cohort.`},{status:409});
    }

    const now=new Date().toISOString();
    let run:{id:string;run_number:number;status:string;required_team_size:number;has_started:boolean}|null=null;

    if(application.project_run_id){
      const {data}=await db
        .from('project_runs')
        .select('id,run_number,status,required_team_size,has_started')
        .eq('id',application.project_run_id)
        .maybeSingle();
      run=data;
    }
    ...
    if(!run&&project.project_type==='open'){
      // Fill the oldest not-started cohort before creating another one.
      const {data:forming}=await db
        .from('project_runs')
        .select('id,run_number,status,required_team_size,has_started')
        .eq('project_id',application.project_id)
        .eq('status','forming')
    ...
    if(!run){
      const {data:latest}=await db
        ...
        .limit(1)
        .maybeSingle();
      const nextNumber=project.project_type==='partner'?1:(latest?.run_number||0)+1;
      const required=Math.max(1,Math.min(50,Number(project.team_size_threshold||5)));
      const {data:created,error:createError}=await db
        ...
        .insert({
          ...
          status:'forming',
          team_size_threshold:required,
          required_team_size:required,
          has_started:false
        })
        .select('id,run_number,status,required_team_size,has_started')
        .single();
      ...
      if(createError){
        // Concurrent approvals may both observe no forming cohort. The unique
        // project/run-number constraint chooses the creator; the other request
        // reuses that newly-created cohort instead of returning a transient 500.
        const {data:concurrentRun}=await db
          .from('project_runs')
          .select('id,run_number,status,required_team_size,has_started')
          .eq('project_id',application.project_id)
          .eq('status','forming')
        ...
        await db.from('project_activity_log').insert({
          ...
          from_status:null,
          to_status:'forming',
          metadata:{project_type:project.project_type,run_number:run.run_number,required_team_size:required}
        });
      }
    }
    ...
    if(memberError){
      if(['23505','23514'].includes(memberError.code||'')){
        return NextResponse.json({error:'That team place changed while this approval was being processed. No existing cohort history was overwritten.'},{status:409});
      }
      throw memberError;
    }
    ...
    if(project.project_type==='partner'){
      await db.from('projects').update({
        status:['draft','pilot','recruiting','open'].includes(project.status)?'forming':project.status,
        updated_at:now
      }).eq('id',application.project_id);
    }else{
      await db.from('projects').update({status:'open',updated_at:now}).eq('id',application.project_id);
    }
    ...
    const threshold=Math.max(1,Number(run.required_team_size||project.team_size_threshold||5));
    ...
    const {data:updated,error}=await db
      .from('project_applications')
      .update({
        status:full?'team_complete':'waiting_for_team',
        reviewer_notes:reviewerNotes||null,
        approved_at:now,
        decision_at:now,
    ...
    await notifyUser(db,{
      ...
      payload:{project_title:project.title,team_number:run.run_number}
    });

    // Open cohorts start independently as soon as they fill. Partner teams never
    // auto-start. The compare-and-set transition makes one concurrent request the
    // kickoff winner, preventing duplicate activation and kickoff notifications.
    if(full&&project.project_type==='open'&&!run.has_started){
      const {data:startedRun,error:startError}=await db
        .from('project_runs')
        .update({status:'active',kickoff_at:now,started_at:now,has_started:true,updated_at:now})
        .eq('id',run.id)
        .eq('status','forming')
        .eq('has_started',false)
        .select('id')
      ...
      if(startedRun){
        await db
          .from('project_members')
          .update({membership_status:'active',activated_at:now})
          .eq('project_run_id',run.id)
          .eq('membership_status','waiting');
        await db
          .from('project_applications')
          .update({status:'team_complete',updated_at:now})
          .eq('project_run_id',run.id)
          .in('status',['approved','waiting_for_team','accepted']);
        await db.from('project_activity_log').insert({
          project_id:application.project_id,
          project_run_id:run.id,
          event_type:'cohort_auto_started',
          actor_type:'system',
          from_status:'forming',
          to_status:'active',
          metadata:{run_number:run.run_number,filled,required_team_size:threshold}
        });
        ...
        await Promise.all((members||[]).map(async member=>notifyUser(db,{
          userId:member.user_id,
          email:await memberEmail(db,member.user_id),
          projectId:application.project_id,
          type:'project_kickoff',
          title:'Your project is starting',
          body:`Team ${run!.run_number} for ${project.title} is ready. Open the workspace to meet the team and start work.`,
          actionUrl:`/member/projects/${application.project_id}?run=${run!.id}`,
    ...
    return NextResponse.json({
      ...
      team:{
        ...
        status:full&&project.project_type==='open'?'active':'forming'
      },
      project_status:project.project_type==='open'?'open':'forming',
      communication:{body:memberMessage}
    });
  }catch(error){
    console.error('project request review error',error);
    return NextResponse.json({error:'Unable to update this project application.'},{status:500});
  }
}

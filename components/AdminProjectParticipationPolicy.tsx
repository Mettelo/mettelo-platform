...
type Props={
  ...
  maxTeamSize:number;
  currentRun?:{id:string;status:string;has_started:boolean;recruitment_open:boolean}|null;
};
...
export default function AdminProjectParticipationPolicy({projectId,participationMode,minTeamSize,targetTeamSize,maxTeamSize,currentRun=null}:Props){
  ...
  function chooseMode(next:Mode){
    setMode(next);
    if(next==='solo'){
      setMinimum(1);setTarget(1);setMaximum(1);
      return;
    }
    if(mode==='solo'){
      setMinimum(2);setTarget(5);setMaximum(5);
    }
  }
  ...
  async function save(){
    ...
    try{
      const response=await fetch('/api/admin/project-participation',{method:'PATCH',headers:{'content-type':'application/json'},body:JSON.stringify({project_id:projectId,participation_mode:mode,min_team_size:minimum,target_team_size:target,max_team_size:maximum})});
      const body=await response.json().catch(()=>({}));
      if(!response.ok)throw new Error(body.error||'Unable to update project participation policy.');
  ...
  async function setRecruitment(open:boolean){
    if(!currentRun)return;
    setBusy(true);setMessage(open?'Reopening recruitment…':'Closing recruitment…');
    try{
      const response=await fetch('/api/admin/project-participation',{method:'PATCH',headers:{'content-type':'application/json'},body:JSON.stringify({project_id:projectId,action:'set_recruitment',project_run_id:currentRun.id,recruitment_open:open})});
      const body=await response.json().catch(()=>({}));if(!response.ok)throw new Error(body.error||'Unable to update recruitment.');
      setRecruitmentOpen(Boolean(body.run?.recruitment_open));setMessage(open?'Recruitment reopened for the same active run.':'Recruitment closed for the active run.');router.refresh();
    }catch(error){setMessage(error instanceof Error?error.message:'Unable to update recruitment.')}finally{setBusy(false)}
  }

  const solo=mode==='solo';
  const activeRun=Boolean(currentRun?.has_started&&currentRun?.status==='active');
  return <section className="phase9Policy" aria-labelledby="phase9-policy-title">
    ...
    <fieldset disabled={busy}><legend>Participation mode</legend><div className="modeGrid">{(['team','solo','flexible'] as Mode[]).map(item=><label key={item}><input type="radio" name="admin-project-participation-mode" checked={mode===item} onChange={()=>chooseMode(item)}/><span><strong>{item.toUpperCase()}</strong><small>{item==='team'?'Configured minimum required before the start process.':item==='solo'?'One valid member can satisfy participation readiness.':'Solo/Either can work independently; Team preference uses the configured minimum.'}</small></span></label>)}</div></fieldset>
    <div className="capacityGrid">
      <label>{mode==='flexible'?'Team minimum':'Minimum to start'}<input type="number" min="1" max="50" value={minimum} disabled={busy||solo} onChange={event=>setMinimum(Math.max(1,Math.min(50,Number(event.target.value)||1)))}/></label>
      <label>Target team<input type="number" min="1" max="50" value={target} disabled={busy||solo} onChange={event=>setTarget(Math.max(1,Math.min(50,Number(event.target.value)||1)))}/></label>
      <label>Maximum places<input type="number" min="1" max="50" value={maximum} disabled={busy||solo} onChange={event=>setMaximum(Math.max(1,Math.min(50,Number(event.target.value)||1)))}/></label>
    </div>

...
export async function POST(request:Request){
  try{
    ...
    if(resource==='project'){
      ...
      const archetype=text(body.project_archetype,40);if(!['analytics','data_engineering','machine_learning','generative_ai','research','visualisation','data_governance'].includes(archetype))return NextResponse.json({error:'Choose a valid Data & AI project archetype.'},{status:400});
    ...
    if(resource==='opportunity'){
      const status=text(body.status,30)||'draft';const opportunityType=text(body.opportunity_type,30);if(!['job','referral','volunteer','fellowship','freelance','consulting','project','other'].includes(opportunityType)) return NextResponse.json({error:'Choose a valid opportunity type.'},{status:400});if(!['draft','published','closed','archived'].includes(status)) return NextResponse.json({error:'Invalid opportunity status.'},{status:400});const sourceUrl=text(body.source_url,500);const organisation=text(body.organisation,180);if(status==='published'&&!sourceUrl)return NextResponse.json({error:'Published opportunities need a source or application URL.'},{status:400});if(status==='published'&&!organisation)return NextResponse.json({error:'Published opportunities need an identifiable organisation.'},{status:400});const now=new Date().toISOString();
      const {data,error}=await db.from('opportunities').upsert({slug,title,organisation:organisation||null,opportunity_type:opportunityType,summary:text(body.summary,900)||null,location:text(body.location,180)||null,eligibility:text(body.eligibility,900)||null,source_url:sourceUrl||null,official_application_url:sourceUrl||null,source_type:'admin_verified_source',source_organisation:organisation||null,access_level:body.access_level==='members'?'members':'public',status,published_at:status==='published'?now:null,closes_at:body.closes_at||null,publication_mode:'manual',verification_status:status==='published'?'verified':'needs_review',verification_score:status==='published'?100:null,verification_reasons:status==='published'?['Published manually by a Mettelo admin after source review']:[],review_required:status!=='published',last_verified_at:status==='published'?now:null,updated_at:now},{onConflict:'slug'}).select('*').single();if(error)throw error;if(status==='published')await db.from('opportunity_verification_checks').insert({opportunity_id:data.id,check_type:'human_review',result:'pass',score:100,detail:'Published manually from the Mettelo Admin publisher.',checked_by:user.email||user.id});return NextResponse.json({ok:true,item:data});
    }

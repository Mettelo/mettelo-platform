...
type Opportunity={id:string;title:string;organisation:string|null;opportunity_type:string;summary:string|null;location:string|null;eligibility:string|null;source_url:string|null;official_application_url:string|null;closes_at:string|null;published_at:string|null;data_ai_relevance_score:number|null;remote_scope:string|null;source_organisation:string|null;country_code:string|null;applicant_scope:string;work_arrangement:string|null;sponsorship_status:string};

const PAGE_SIZE=10;
const countryLabels:Record<string,string>={GB:'United Kingdom',NG:'Nigeria',IN:'India',US:'United States',CA:'Canada',AU:'Australia',GLOBAL:'Remote / Global'};
...
export default function OpportunityBoard({items}:{items:Opportunity[]}){
  const [query,setQuery]=useState('');const [type,setType]=useState('all');const [country,setCountry]=useState('all');const [arrangement,setArrangement]=useState('all');const [sponsorship,setSponsorship]=useState('all');const [scope,setScope]=useState('all');const [sort,setSort]=useState('newest');const [page,setPage]=useState(1);
  ...
  const filtered=useMemo(()=>{
    ...
    const rows=items.filter(item=>{
      const hay=[item.title,item.organisation,item.location,item.summary,item.country_code?countryLabels[item.country_code]:null].filter(Boolean).join(' ').toLowerCase();
      ...
      return (!q||hay.includes(q))&&typeOk&&countryOk&&arrangementOk&&sponsorshipOk&&scopeOk;
    });
  ...
  },[items,query,type,country,arrangement,sponsorship,scope,sort]);
  ...
  function resetPage(){setPage(1);}function clearFilters(){setQuery('');setType('all');setCountry('all');setArrangement('all');setSponsorship('all');setScope('all');setSort('newest');setPage(1);}
  const hasFilters=Boolean(query||type!=='all'||country!=='all'||arrangement!=='all'||sponsorship!=='all'||scope!=='all');

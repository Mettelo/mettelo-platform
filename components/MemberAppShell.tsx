...
export default function MemberAppShell({children}:{children:React.ReactNode}){
  ...
  const isActive=(href:string)=>href==='/member'?pathname==='/member':href!=='/'&&(pathname===href||pathname.startsWith(`${href}/`));
  ...
  return <div className={styles.appShell}>
    <header className={styles.appHeader}><div className={styles.headerInner}><Link className={styles.brand} href="/member" aria-label="Mettelo member dashboard"><Mark/><strong>Mettelo</strong></Link><a className={styles.search} href="/search"><span aria-hidden="true">⌕</span><span>Search projects, people, opportunities...</span></a><div className={styles.headerActions}>{account?.isAdmin&&<Link className={styles.modeSwitch} href="/admin">Admin mode</Link>}<NotificationMenu/><details className={styles.accountMenu}><summary aria-label="Open account menu"><span className={styles.avatar} style={account?.avatarUrl?{backgroundImage:`url(${account.avatarUrl})`}:undefined}>{account?.avatarUrl?'':(account?.name?.[0]||'M').toUpperCase()}</span><span className={styles.accountCopy}><strong>{account?.name?.split(' ')[0]||'Member'}</strong><small>{account?.isAdmin?'Member mode':'Account'}</small></span><span aria-hidden="true">⌄</span></summary><div className={styles.accountPanel}>{account&&<div className={styles.accountIdentity}><strong>{account.name}</strong><small>{account.email}</small></div>}<Link href="/member">Dashboard <span>→</span></Link><Link href="/member/profile">Profile <span>→</span></Link>{account?.isAdmin&&<Link href="/admin">Switch to Admin <span>→</span></Link>}<Link href="/feedback">Give feedback <span>→</span></Link><button type="button" onClick={signOut}>Sign out <span>→</span></button></div></details><details className={styles.mobileMenu}><summary aria-label="Open member navigation"><span aria-hidden="true">☰</span><span>Menu</span></summary><div className={styles.mobilePanel}>{account?.isAdmin&&<Link href="/admin">Switch to Admin →</Link>}{nav}<div className={styles.mobileAccount}><Link href="/member/profile">Profile</Link><button type="button" onClick={signOut}>Sign out</button></div></div></details></div></div></header>
    <div className={styles.appBody}><aside className={styles.sidebar} aria-label="Mettelo member navigation"><div className={styles.sidebarIntro}><span>Workspace</span><strong>My Mettelo</strong></div>{account?.isAdmin&&<Link className={styles.modeCard} href="/admin"><small>MODE</small><strong>Switch to Admin</strong><span>Manage Mettelo →</span></Link>}<nav>{nav}</nav><div className={styles.sidebarHelp}><Link href="/feedback">Give feedback →</Link><Link href="/contact">Help & support →</Link></div></aside><main className={styles.content}>{children}</main></div>

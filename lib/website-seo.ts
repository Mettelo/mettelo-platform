...
export function validateGlobalSeo(value:unknown){
 ...
 if(google===null||bing===null)return{ok:false as const,error:'Search-engine verification values contain unsupported characters.'};
...
export function absoluteSeoUrl(value:string){try{return new URL(value,'https://mettelo.com').toString()}catch{return'https://mettelo.com'}}

export async function buildGlobalMetadata():Promise<Metadata>{
 const seo=await getPublicGlobalSeo();
 return{metadataBase:new URL('https://mettelo.com'),title:{default:seo.default_title,template:seo.title_template},description:seo.default_description,openGraph:{title:seo.default_og_title,description:seo.default_og_description,url:'https://mettelo.com/',siteName:seo.site_name,images:[{url:seo.default_og_image,alt:seo.default_og_title}],type:'website'},twitter:{card:'summary_large_image',title:seo.twitter_title,description:seo.twitter_description,images:[seo.twitter_image]},verification:{google:seo.google_site_verification||undefined,other:seo.bing_site_verification?{'msvalidate.01':[seo.bing_site_verification]}:undefined}};
}
export async function buildPageMetadata(page:WebsiteSeoPageKey):Promise<Metadata>{
 const [global,pageSeo]=await Promise.all([getPublicGlobalSeo(),getPublicPageSeo(page)]);const canonical=absoluteSeoUrl(pageSeo.canonical);
 return{title:page==='home'?{absolute:pageSeo.title}:pageSeo.title,description:pageSeo.description,alternates:{canonical},robots:{index:pageSeo.index,follow:pageSeo.follow,googleBot:{index:pageSeo.index,follow:pageSeo.follow}},openGraph:{title:pageSeo.og_title,description:pageSeo.og_description,url:canonical,siteName:global.site_name,images:[{url:pageSeo.og_image,alt:pageSeo.og_title}],type:'website'},twitter:{card:'summary_large_image',title:pageSeo.og_title,description:pageSeo.og_description,images:[pageSeo.og_image]}};
}
export async function buildOrganisationJsonLd(){const seo=await getPublicGlobalSeo();return{'@context':'https://schema.org','@type':'Organization',name:seo.organisation_name,url:'https://mettelo.com',description:seo.organisation_description,logo:absoluteSeoUrl(seo.organisation_logo_url)};}

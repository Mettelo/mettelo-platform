...
type Draft={project_role_id:string;portfolio_url:string;availability:string;contribution_statement:string};
const emptyDraft:Draft={project_role_id:'',portfolio_url:'',availability:'',contribution_statement:''};
...
export default function ProjectApplicationForm({projects,selectedProjectId,profileMissing=[]}:{projects:Project[];selectedProjectId?:string;profileMissing?:string[]}){
  ...
  const [step,setStep]=useState<'application'|'review'>('application');
  const [draft,setDraft]=useState<Draft>(emptyDraft);
  const [restored,setRestored]=useState(false);
  const selected=projects.find(project=>project.id===selectedProjectId)||null;
  const storageKey=selected?`mettelo:project-application:${selected.id}`:'';
  const selectedRole=useMemo(()=>selected?.roles.find(role=>role.id===draft.project_role_id)||null,[selected,draft.project_role_id]);

  useEffect(()=>{
    if(!storageKey)return;
    try{const raw=window.localStorage.getItem(storageKey);if(raw){const parsed=JSON.parse(raw) as Partial<Draft>;setDraft({...emptyDraft,...parsed});setRestored(true);}}catch{}
  },[storageKey]);
  useEffect(()=>{if(!storageKey)return;try{window.localStorage.setItem(storageKey,JSON.stringify(draft));}catch{}},[storageKey,draft]);
  ...
  if(!selected){
    return <div className="formCard projectSelectionRequired">
      ...
      <p style={{color:'var(--slate)',margin:0}}>Start from a project card or project detail page so your application stays linked to the correct brief.</p>
  ...
  if(profileMissing.length){
    ...
    return <div className="formCard applicationEligibilityBlock">
      ...
      <div className="eligibilityList">{profileMissing.map(item=><div key={item}><span>→</span><strong>{item}</strong></div>)}</div>
      <a className="button dark" href={`/member/profile?next=${encodeURIComponent(returnTo)}`}>Complete profile →</a>
      <style jsx>{`.applicationEligibilityBlock{display:grid;gap:15px}.applicationEligibilityBlock h3{font-size:1.55rem;margin:0}.applicationEligibilityBlock p{margin:0;color:var(--slate);line-height:1.65}.eligibilityList{display:grid;gap:8px;padding:14px;border-radius:12px;background:#faf7f0}.eligibilityList div{display:grid;grid-template-columns:auto 1fr;gap:9px}.eligibilityList span{color:#8b5a17;font-weight:800}`}</style>
    </div>;
  }
  ...
  if(step==='review')return <form className="formCard applicationReviewStep" onSubmit={submit}>
    ...
    <style jsx>{`.applicationReviewStep{display:grid;gap:18px}.applicationReviewStep h3{font-size:1.65rem;margin:0}.reviewProject,.reviewGrid>div,.reviewNotice{padding:14px;border:1px solid var(--line);border-radius:12px;background:#fff}.reviewProject small,.reviewGrid small{display:block;color:var(--bronze-deep);font-size:.65rem;font-weight:850;letter-spacing:.08em;margin-bottom:6px}.reviewGrid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.reviewWide{grid-column:1/-1}.reviewWide p,.reviewNotice p{margin:5px 0 0;color:var(--slate);line-height:1.65}.reviewActions{display:flex;justify-content:space-between;gap:10px}@media(max-width:600px){.reviewGrid{grid-template-columns:1fr}.reviewWide{grid-column:auto}.reviewActions{display:grid}.reviewActions .button{width:100%}}`}</style>
  </form>;
  ...
  return <form className="formCard" onSubmit={review}>
    ...
    <button className="button dark" type="submit" style={{width:'100%',marginTop:20}}>Review application →</button>
    <div className={`formStatus ${status}`} role="status" aria-live="polite">{message}</div>
    <style jsx>{`.applicationSaveNote{margin:8px 0 0;color:#66707e;font-size:.78rem}.selectedProjectBlock{display:grid;grid-template-columns:1fr auto;gap:5px 14px;margin:20px 0;padding:15px 16px;border:1px solid var(--line);border-radius:14px;background:var(--sand-2)}.selectedProjectBlock small{grid-column:1/-1;color:var(--bronze-deep);font-size:.65rem;font-weight:850;letter-spacing:.08em}.selectedProjectBlock strong{font-size:1rem}.selectedProjectBlock a{align-self:center;color:var(--bronze-deep);font-size:.73rem;font-weight:750}@media(max-width:560px){.selectedProjectBlock{grid-template-columns:1fr}.selectedProjectBlock a{margin-top:5px}.formCard{padding:20px}}`}</style>
  </form>;
}

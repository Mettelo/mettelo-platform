...
const structuredVisibilityValues=new Set(['public','members','internal']);
const dependencyTypes=new Set(['data','access','tool','stakeholder','project','other']);
...
function rpcErrorMessage(message:string){
  ...
  if(message.includes('DELIVERABLE_NOT_IN_PROJECT')||message.includes('SUCCESS_CRITERION_NOT_IN_PROJECT')||message.includes('ACCEPTANCE_CRITERION_NOT_IN_PROJECT')||message.includes('DEPENDENCY_NOT_IN_PROJECT')||message.includes('MILESTONE_NOT_IN_PROJECT')||message.includes('ROLE_NOT_IN_PROJECT'))return 'The project changed while this draft was open. Reload it before saving so existing record IDs are preserved.';
...
export async function PATCH(request:Request,{params}:Context){
  try{
    ...
    const acceptanceCriteria=items(body.acceptance_criteria,40).map((item,index)=>({id:uuid(item.id)||null,criterion:clean(item.criterion,1200),is_required:item.is_required!==false,visibility:structuredVisibilityValues.has(clean(item.visibility,20))?clean(item.visibility,20):'public',sort_order:index}));
    if(acceptanceCriteria.some(item=>item.criterion.length<3))return NextResponse.json({error:'Every structured acceptance criterion must contain at least three characters.'},{status:400});
    const dependencies=items(body.dependencies,40).map((item,index)=>({id:uuid(item.id)||null,title:clean(item.title,180),description:clean(item.description,2400)||null,dependency_type:dependencyTypes.has(clean(item.dependency_type,30))?clean(item.dependency_type,30):'other',is_required:item.is_required!==false,visibility:structuredVisibilityValues.has(clean(item.visibility,20))?clean(item.visibility,20):'public',sort_order:index}));
    if(dependencies.some(item=>item.title.length<2))return NextResponse.json({error:'Every project dependency must have a clear title.'},{status:400});

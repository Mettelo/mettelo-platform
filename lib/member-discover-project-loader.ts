...
function projectQuery(db:MemberServerDb,select:string,from:number,to:number){
...
async function loadAllProjectBatches(db:MemberServerDb,select:string){
  let from=0;
  const rows:unknown[]=[];
  let lastResult:Awaited<ReturnType<typeof projectQuery>>|null=null;

  while(true){
    const to=from+DISCOVER_BATCH_SIZE-1;
    const result=await projectQuery(db,select,from,to);
    lastResult=result;
    if(result.error)return result;

    const batch=(result.data||[]) as unknown[];
    rows.push(...batch);
    if(batch.length<DISCOVER_BATCH_SIZE)break;
    from+=DISCOVER_BATCH_SIZE;
  }

  if(!lastResult)return projectQuery(db,select,0,DISCOVER_BATCH_SIZE-1);
  return {...lastResult,data:rows};
}

export async function loadMemberDiscoverProjects(db:MemberServerDb){
  const primary=await loadAllProjectBatches(db,PRIMARY_SELECT);
  ...
  const core=await loadAllProjectBatches(db,CORE_FACET_SELECT);
  ...
  return loadAllProjectBatches(db,MINIMAL_SELECT);
}

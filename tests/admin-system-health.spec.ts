...
test.describe('Admin System aggregate health',()=>{
 test('health API is capability-gated and returns aggregate operational evidence only',async({page,request})=>{
  ...
  expect(body.audit.state).toBe('available');expect(body.delivery.state).toBe('available');expect(body.backend_contract.state).toBe('available');expect(body.backend_contract.missing).toEqual([]);expect(typeof body.backend_contract.checked).toBe('number');expect(typeof body.audit.events_24h).toBe('number');expect(typeof body.delivery.queued).toBe('number');expect(typeof body.can_manage_delivery).toBe('boolean');
  ...
  expect(Object.keys(body.delivery).sort()).toEqual(['dead_letter','failed','latest_delivery_at','queued','retrying','sent_24h','state'].sort());expect(Object.keys(body.backend_contract).sort()).toEqual(['checked','missing','state'].sort());
 });
 ...
 test('System health is discoverable and responsive',async({page})=>{
  ...
  for(const width of [390,768,1440]){await page.setViewportSize({width,height:900});await page.goto('/admin/system/health',{waitUntil:'networkidle'});await expect(page.getByRole('heading',{level:1,name:'System health'})).toBeVisible();await expect(page.getByRole('heading',{name:'Admin audit activity'})).toBeVisible();await expect(page.getByRole('heading',{name:'Backend contract integrity'})).toBeVisible();await expect(page.getByRole('heading',{name:'Transactional email delivery'})).toBeVisible();await expect(page.getByText('Unknown is not healthy.')).toBeVisible();await expect(page.getByRole('button',{name:'Refresh status'})).toBeVisible();await noOverflow(page,`System health overflowed at ${width}px`);}
 });
});

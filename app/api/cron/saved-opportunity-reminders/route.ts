...
export async function GET(request:Request){
 ...
 try{
  ...
  const {data,error}=await db.from('saved_opportunities').select('user_id,opportunity_id,reminders_enabled,opportunities(id,title,organisation,status,closes_at)').eq('reminders_enabled',true).gte('opportunities.closes_at',now.toISOString()).lte('opportunities.closes_at',horizon.toISOString()).eq('opportunities.status','published');if(error)throw error;
  let notified=0;

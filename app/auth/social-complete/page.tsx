...
export default function SocialAccountCompletePage(){
  ...
  async function submit(event:FormEvent<HTMLFormElement>){
    ...
    try{
      ...
      setStatus('success');setMessage(`@${identityPayload.identity?.username||usernameResult.username} is ready. Continuing to professional profile setup…`);
      window.setTimeout(()=>window.location.assign(`/auth/verified?next=${encodeURIComponent(safeNext())}`),400);
    }catch(error){
      setStatus('error');
  ...
  const locked=status==='working'||status==='success';
  return <section className="section softSection"><div className="shell" style={{maxWidth:760}}><div className="panel" style={{padding:'clamp(24px,5vw,52px)'}}><div className="eyebrow">Complete account setup</div><h1 style={{fontSize:'clamp(2.35rem,7vw,4.25rem)',margin:'0 0 14px'}}>Secure your Mettelo account.</h1><p className="lead">{provider} has verified {email?<strong>{email}</strong>:'your email address'}. Choose your Mettelo username and create a password for an additional sign-in and recovery method.</p><form onSubmit={submit} style={{marginTop:24}} aria-busy={status==='working'}><label htmlFor="username">Username *</label><input id="username" name="username" required minLength={3} maxLength={30} autoComplete="username" autoCapitalize="none" autoCorrect="off" spellCheck={false} pattern="[A-Za-z][A-Za-z0-9_]{2,29}" aria-describedby="username-help oauth-complete-status" disabled={locked}/><p id="username-help" style={{fontSize:'.76rem',color:'var(--slate)',marginTop:4}}>Start with a letter and use 3–30 letters, numbers or underscores. It will be stored in lowercase.</p><label htmlFor="password">Create password *</label><input id="password" name="password" type="password" minLength={8} required autoComplete="new-password" aria-describedby="password-help oauth-complete-status" disabled={locked}/><p id="password-help" style={{fontSize:'.76rem',color:'var(--slate)',marginTop:4}}>Use at least 8 characters. For stronger security, use a unique password that you do not use elsewhere.</p><label htmlFor="confirmPassword">Confirm password *</label><input id="confirmPassword" name="confirmPassword" type="password" minLength={8} required autoComplete="new-password" aria-describedby="oauth-complete-status" disabled={locked}/><button className="button dark" type="submit" disabled={locked} style={{width:'100%',minHeight:44,marginTop:20}}>{status==='working'?'Saving account…':status==='success'?'Account ready ✓':'Save account and continue →'}</button><div id="oauth-complete-status" className={`formStatus ${status}`} role="status" aria-live="polite">{message}</div></form><p style={{color:'var(--slate)',marginBottom:0}}>You can continue to sign in with {provider}. Your Mettelo password provides an additional secure way to access and recover your account.</p></div></div></section>;
}

...
async function notifyAdminsSafely(db:NonNullable<ReturnType<typeof serviceDb>>,input:Parameters<typeof notifyAdmins>[1]){try{await notifyAdmins(db,input);}catch(error){console.warn('Spotlight admin notification will not block member state',error);}}

export async function PATCH(request:Request){
  try{
    const auth=await createServerSupabaseClient();const {data:{user}}=await auth.auth.getUser();
    if(!user)return NextResponse.json({error:'Authentication required.'},{status:401});
    const body=await request.json().catch(()=>({}));const id=String(body.id||''),action=String(body.action||'');
    ...
    const {data:item,error:readError}=await db.from('spotlights').select('id,user_id,title,status,consent_status,award_month,is_excluded,publication_held').eq('id',id).eq('user_id',user.id).maybeSingle();
    if(readError)throw readError;if(!item)return NextResponse.json({error:'Spotlight recognition not found.'},{status:404});
    const now=new Date().toISOString();

    if(action==='grant'){
      if(item.is_excluded||item.status==='archived')return NextResponse.json({error:'This recognition is not available for public publication.'},{status:409});
      if(item.consent_status==='granted'){
      ...
      const publication=await publishSpotlightIfReady(db,id,user.id);
      await notifyAdminsSafely(db,{type:'spotlight_consent',eventKey:'spotlight_published',title:'Spotlight consent granted',body:`A member granted publication consent for ${item.title}. Publication is automatic unless the award is held or excluded.`,actionUrl:'/admin/spotlights',subject:'Spotlight consent granted',dedupeKey:`spotlight:${id}:consent-granted`});
      const current=publication.item||updated;
      return NextResponse.json({ok:true,item:current,publicUrl:current.status==='published'?`/spotlight/${id}`:null,message:current.status==='published'?'Consent granted. Your Spotlight recognition is now public and ready to share.':'Consent granted. Publication will happen automatically when any active publication hold is cleared.'});
    }

    if(action==='decline'){
      if(item.consent_status==='declined')return NextResponse.json({ok:true,item,publicUrl:null,message:'This recognition is already private by your choice.'});
      ...
      const {data:updated,error}=await db.from('spotlights').update({consent_status:'declined',consented_at:null,consent_withdrawn_at:now,public_display_name:null,public_headline:null}).eq('id',id).eq('user_id',user.id).select('id,consent_status,status,publication_held').single();if(error)throw error;
      await recordSpotlightEvent(db,id,'consent_declined',user.id,{award_month:item.award_month});
      await notifyAdminsSafely(db,{type:'spotlight_consent',eventKey:'spotlight_published',title:'Spotlight publication declined',body:`A member declined public publication for ${item.title}. The recognition remains in their private Mettelo history.`,actionUrl:'/admin/spotlights',subject:'Spotlight publication declined',dedupeKey:`spotlight:${id}:consent-declined`});
    ...
    if(item.consent_status==='withdrawn'||(item.status==='archived'&&!item.is_excluded))return NextResponse.json({ok:true,item,publicUrl:null,message:'Publication permission is already withdrawn and the public recognition is unavailable.'});
    ...
    const changes=item.status==='published'?{consent_status:'withdrawn',consented_at:null,consent_withdrawn_at:now,status:'archived',published_at:null,public_display_name:null,public_headline:null}:{consent_status:'withdrawn',consented_at:null,consent_withdrawn_at:now,public_display_name:null,public_headline:null};
    const {data:updated,error}=await db.from('spotlights').update(changes).eq('id',id).eq('user_id',user.id).select('id,consent_status,status,publication_held').single();if(error)throw error;
    await recordSpotlightEvent(db,id,'consent_withdrawn',user.id,{award_month:item.award_month,was_public:item.status==='published'});
    await notifyAdminsSafely(db,{type:'spotlight_consent',eventKey:'spotlight_published',title:'Spotlight consent withdrawn',body:`A member withdrew publication consent for ${item.title}. Any public Spotlight URL is no longer available.`,actionUrl:'/admin/spotlights',subject:'Spotlight consent withdrawn',dedupeKey:`spotlight:${id}:consent-withdrawn`});

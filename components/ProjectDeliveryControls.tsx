...
export default function ProjectDeliveryControls({projectId,milestones}:{projectId:string;milestones:Milestone[]}){
  ...
  async function submit(event:FormEvent<HTMLFormElement>){
    ...
    try{
      const response=await fetch('/api/project-delivery',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({...values,resource,project_id:projectId})});
  ...
  return <section className="panel"><div className="panelHead"><div><span className="cardNumber">PROJECT LEAD CONTROLS</span><h3 style={{marginTop:8}}>Plan delivery</h3></div><div className="filterBar" style={{margin:0}}><button type="button" className={`filter ${resource==='milestone'?'active':''}`} onClick={()=>setResource('milestone')}>Milestone</button><button type="button" className={`filter ${resource==='task'?'active':''}`} onClick={()=>setResource('task')}>Task</button></div></div><form className="formCard" onSubmit={submit}>
    ...
    {resource==='task'&&<><label htmlFor="delivery-milestone">Milestone</label><select id="delivery-milestone" name="milestone_id" defaultValue=""><option value="">No milestone</option>{milestones.map(item=><option key={item.id} value={item.id}>{item.title}</option>)}</select></>}

...
type Application={id:string;status:string;submitted_at:string;updated_at:string;project_id:string;project_run_id?:string|null;application_kind?:string;requested_role?:string|null;admission_decision?:string|null;reviewer_notes?:string|null;clarification_requested_at?:string|null;projects:{title:string;status?:string;project_type?:string}|null;project_roles:{title:string}|null;formation?:Formation|null;events?:AppEvent[]};
type ViewState='current'|'needs'|'closed'|'all';
type CardState='review'|'forming'|'confirmed'|'paused'|'closed';

const labels:Record<string,string>={submitted:'Submitted',in_review:'In review',clarification_requested:'Clarification requested',action_required:'Action required',needs_changes:'Action required',shortlisted:'Shortlisted',offered:'Offer received',approved:'Team forming',accepted:'Place accepted',waiting_for_team:'Team forming',team_complete:'Ready / starting',declined:'Not selected',expired:'Offer expired',withdrawn:'Withdrawn'};
...
const closedStates=new Set(['declined','expired','withdrawn']);
const ordinaryWithdrawable=new Set(['submitted','in_review','shortlisted','approved','waiting_for_team']);
...
function canWithdraw(item:Application){return ordinaryWithdrawable.has(item.status)||(item.status==='accepted'&&item.admission_decision==='auto_qualified')}
...
function statusLabel(item:Application){if(item.projects?.status==='cancelled')return'Project cancelled';if(item.status==='offered')return'→ Offer received';if(actionStates.has(item.status))return'! Action required';if(item.status==='accepted')return'✓ Place accepted';const state=cardState(item);if(state==='confirmed')return'✓ Ready / starting';if(state==='forming')return'◷ Team forming';if(state==='paused')return'◷ Team paused';if(state==='closed')return timelineLabel(item.status);return item.status==='submitted'?'● Submitted':'● In review'}
function requestCopy(item:Application){
  const state=cardState(item);const noun=requestNoun(item);
  if(item.status==='offered')return{body:'Mettelo has offered you a place on this project. You need to review the offer and respond before its deadline.',sideLabel:'ACTION REQUIRED',sideTitle:'Review your offer',sideBody:'Accept or decline the Project Place Offer before it expires.'};
  if(['clarification_requested','action_required','needs_changes'].includes(item.status))return{body:item.reviewer_notes||'Mettelo needs additional information before this project request can continue.',sideLabel:'ACTION REQUIRED',sideTitle:'Send clarification',sideBody:'Respond to the clarification request so review can continue.'};
  if(item.status==='accepted'&&item.admission_decision==='review_required')return{body:'You accepted this project place. Your explicit commitment is recorded, but project membership and private workspace access still depend on the governed team-formation and start-readiness journey.',sideLabel:'PLACE ACCEPTED',sideTitle:'Your commitment is recorded',sideBody:'Mettelo will move the accepted place into team formation and readiness. Acceptance does not mean the project has started.'};
  if(item.status==='expired')return{body:'This project place Offer expired before an acceptance was recorded. The reservation has been released and the Offer remains in your history.',sideLabel:'OFFER HISTORY',sideTitle:'Offer expired',sideBody:'No further action is available on this Offer. You can return to Discover for another project.'};
...
export default function MemberApplicationTracker({applications}:{applications:Application[]}){
  ...
  const counts=useMemo(()=>({
    needs:items.filter(item=>actionStates.has(item.status)&&!isClosed(item)).length,
  ...
  const filtered=useMemo(()=>items.filter(item=>{
    ...
    const matchesView=view==='all'||(view==='needs'?actionStates.has(item.status)&&!isClosed(item):view==='closed'?isClosed(item):!isClosed(item));
  ...
  return <>
    <section className="mmaSummary" aria-label="Project request summary">
      ...
      <article className="mmaSummaryCard"><strong>{counts.review}</strong><span>In review</span><small>{counts.review===1?'One project request is being reviewed':'Project requests waiting on Mettelo review'}</small></article>
    ...
    <section className="mmaFilterbar" aria-label="Search and filter project requests">
      ...
      <div className="mmaSegment" role="tablist" aria-label="Project request lifecycle filter">{(['current','needs','closed','all'] as ViewState[]).map(value=><button key={value} type="button" role="tab" aria-selected={view===value} aria-controls="mma-applications-panel" className={view===value?'mmaActive':''} onClick={()=>setView(value)}>{value==='needs'?'Needs action':value.charAt(0).toUpperCase()+value.slice(1)}</button>)}</div>
    ...
    <div id="mma-applications-panel" role="tabpanel" aria-live="polite">
    {view==='needs'&&filtered.length===0&&<section className="mmaEmpty"><h3>Nothing needs your attention</h3><p>There are no project-request actions waiting for you right now.</p></section>}
...
function CurrentCard({item,onOpen}:{item:Application;onOpen:(item:Application)=>void}){
  const state=cardState(item);const copy=requestCopy(item);const confirmed=state==='confirmed';const forming=state==='forming';const paused=state==='paused';const offered=item.status==='offered';const clarification=['clarification_requested','action_required','needs_changes'].includes(item.status);const needsAction=offered||clarification;const accepted=item.status==='accepted'&&item.admission_decision==='review_required';const itemRole=roleOf(item);
  ...
  return <article className={cardClass}>
    <div className="mmaCardMain">
      ...
      <div className="mmaMeta">
        ...
        {forming&&!accepted&&<span>Team not yet confirmed</span>}
      </div>
      {!confirmed&&<div className={needsAction?'mmaNeedsAction':'mmaNoAction'}><span aria-hidden="true">{needsAction?'!':'✓'}</span> {offered?'Action required: review your project offer':clarification?'Action required: send the requested clarification':accepted?'Your place is accepted; team formation and start readiness come next':paused?'No action needed while the team is paused':forming?'No action needed right now':'No action needed'}</div>}
    ...
    <aside className="mmaActionbox" aria-label={`Actions for ${item.projects?.title||'project request'}`}>
      ...
      <div className="mmaActions">{offered&&<a className="mmaBtn mmaPrimary" href="#member-project-offers-title">Review offer</a>}{clarification&&<a className="mmaBtn mmaPrimary" href="#clarification-title">Respond</a>}{confirmed&&<a className="mmaBtn mmaPrimary" href="/member/projects">Open in Projects →</a>}<button className="mmaBtn" type="button" onClick={()=>onOpen(item)}>{confirmed?'View history':isInterest(item)?'View interest':'View application'}</button></div>
    </aside>
  </article>;
}
...
function HistoryCard({item,onOpen}:{item:Application;onOpen:(item:Application)=>void}){
  ...
  return <article className="mmaHistoryCard"><div><span className="mmaStatus">{statusLabel(item)}</span><h3>{item.projects?.title||'Mettelo project'}</h3><p>{itemRole?`${itemRole} · `:''}{item.status==='withdrawn'?'Withdrawn':item.status==='expired'?'Offer expired':cancelled?'Project cancelled':'Closed'} {date(item.updated_at)}</p></div><button className="mmaBtn" type="button" onClick={()=>onOpen(item)}>{isInterest(item)?'View interest':'View application'}</button></article>;
}

...
export default async function MemberProjectApplyPage({params,searchParams}:{params:Promise<{id:string}>;searchParams?:Promise<{role?:string|string[]}>}){
  ...
  const weeklyCapacity=typeof profile?.weekly_capacity==='string'?profile.weekly_capacity:null;
  if(compareWeeklyCapacity(weeklyCapacity,project.weekly_commitment)==='gap')redirect(`/member/discover/${id}#fit`);
  if(!requestedRole||!availableRoles.some(role=>role.id===requestedRole))redirect(`/member/discover/${id}#roles`);
  const initialRoleId=requestedRole;const initialAvailability=weeklyCapacity||'';const initialPortfolioUrl=[profile?.portfolio_url,profile?.github_url,profile?.linkedin_url].find(value=>typeof value==='string'&&value.trim()) as string|undefined;

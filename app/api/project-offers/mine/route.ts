...
export async function GET(){
  ...
  const {data,error}=await auth
    .from('project_offers')
    .select('id,application_id,project_id,project_run_id,status,offered_at,expires_at,accepted_at,declined_at,expired_at,capacity_reserved_at,capacity_released_at,projects(title,project_type,partner_name,weekly_commitment,duration_weeks,participation_mode,kickoff_at,min_team_size,target_team_size,max_team_size)')
    .eq('user_id',user.id)
  ...
  if(service&&projectIds.length){
    const [{data:members,error:memberError},{data:reservations,error:reservationError}]=await Promise.all([
      ...
      service.from('project_offers').select('project_id,status,capacity_released_at').in('project_id',projectIds).in('status',['pending','accepted']).is('capacity_released_at',null)
    ]);
  ...
  const enriched=offers.map(offer=>{
    ...
    const counts=aggregate.get(offer.project_id)||{confirmed:0,reserved:offer.status==='pending'||offer.status==='accepted'?1:0};
    return{
      ...offer,
      team_state:{

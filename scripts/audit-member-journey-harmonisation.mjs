...
const checks=[
  ...
  ['application lifecycle states still route to Applications',journey.includes("href:'/member/applications'")&&journey.includes("'application_submitted','application_action_required','application_in_review','team_forming'")],
  ...
  ['Profile save uses the canonical atomic authenticated-owner RPC',profileApi.includes("supabase.rpc('save_member_profile'")&&profileApi.includes('p_profile:profilePayload')&&profileApi.includes('p_expected_updated_at:expectedUpdatedAt')],
  ['Profile save keeps owner identity server-derived',profileApi.includes("supabase.auth.getUser()")&&!profileApi.includes('id:body.id')&&!profileApi.includes('user_id:body.user_id')],
  ['Profile save surfaces RPC/schema mismatch without exposing database detail',profileApi.includes("schemaMismatch=error.code==='42883'||error.code==='PGRST202'")&&profileApi.includes('Profile saving is temporarily unavailable while the profile schema is updated.')],

...
test.beforeAll(async()=>{
  ...
  const {error:profileError}=await db.from('profiles').update({profile_readiness:100,skills:['Data Analysis','Research'],preferred_roles:['Data Analyst']}).eq('id',memberId);if(profileError)throw profileError;
  for(let index=0;index<projectIds.length;index++){
    const {error:projectError}=await db.from('projects').upsert({id:projectIds[index],slug:`e2e-recommended-project-${index+1}`,title:projectTitles[index],summary:'A deterministic member project using Data Analysis to test truthful relevance, internal routing and responsive recommendation cards.',status:'open',visibility:'public',project_type:'open',applications_open:true,location:'Remote',location_type:'remote',duration_weeks:6,weekly_commitment:'4–6 hours/week',application_deadline:'2099-08-24T23:59:59.000Z'},{onConflict:'id'});if(projectError)throw projectError;
...
test('Recommended matches the approved simple responsive relevance hierarchy',async({page})=>{
  ...
  for(const width of widths){
    ...
    await expect(page.getByRole('heading',{level:1,name:'Recommended for you'})).toBeVisible();await expect(page.getByText('PERSONALISED · RELEVANCE',{exact:true})).toBeVisible();await expect(page.getByText("Your recommendations use what you've shared with Mettelo",{exact:true})).toBeVisible();await expect(page.getByRole('link',{name:'Browse Discover'})).toHaveAttribute('href','/member/discover');
    ...
    const projects=page.getByRole('region',{name:'Projects for you'});await expect(projects).toBeVisible();const projectCards=projects.locator('article');await expect(projectCards).toHaveCount(2);expect(await topAligned(projectCards),`${width}px project card alignment`).toBe(width>=1025);
    await expect(projects.getByRole('link',{name:'View project'}).first()).toHaveAttribute('href',/\/member\/discover\//);await expect(projects.getByRole('button',{name:/Save project|Saved/}).first()).toBeVisible();
    const events=page.getByRole('region',{name:'Events you may like'});await expect(events).toBeVisible();await expect(events.getByRole('link',{name:'View event'})).toHaveAttribute('href','/events/e2e-data-analysis-lab');
    const spotlight=page.getByRole('region',{name:'Worth a look'});await expect(spotlight).toBeVisible();await expect(spotlight.getByRole('link',{name:'View Spotlight'})).toHaveAttribute('href',`/spotlight/${spotlightId}`);
...
test('Recommended Save is independent of application lifecycle',async({page})=>{
  test.setTimeout(120_000);await signIn(page);await page.goto('/member/recommended',{waitUntil:'networkidle'});const card=page.getByRole('region',{name:'Projects for you'}).locator('article').filter({hasText:projectTitles[2]});const save=card.getByRole('button',{name:/Save project|Saved/});await expect(save).toBeVisible();await save.click();await expect(save).toHaveText(/Saved/);
  const db=localDb();const {count,error}=await db.from('project_applications').select('id',{count:'exact',head:true}).eq('project_id',projectIds[2]).eq('user_id',memberId);if(error)throw error;expect(count||0).toBe(0);
});

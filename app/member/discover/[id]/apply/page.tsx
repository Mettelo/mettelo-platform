...
export default async function MemberProjectApplyPage({params,searchParams}:{params:Promise<{id:string}>;searchParams?:Promise<{role?:string|string[]}>}){
  ...
  const initialRoleId=requestedRole&&availableRoles.some(role=>role.id===requestedRole)?requestedRole:'';const initialAvailability=typeof profile?.weekly_capacity==='string'?profile.weekly_capacity:'';const initialPortfolioUrl=[profile?.portfolio_url,profile?.github_url,profile?.linkedin_url].find(value=>typeof value==='string'&&value.trim()) as string|undefined;

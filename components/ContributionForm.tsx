...
export default function ContributionForm({projects,tasks=[],projectRunId,evidenceOptions=[]}:{projects:Project[];tasks?:Task[];projectRunId?:string|null;evidenceOptions?:EvidenceOption[]}){
  ...
  if(!projects.length) return <div className="emptyState"><h3>No active project membership.</h3><p>You can record a project contribution after you are accepted into a Mettelo Lab team.</p></div>;
  return <form className="formCard" onSubmit={submit}>
    <div className="panelHead"><div><span className="cardNumber">YOUR CONTRIBUTION</span><h3 style={{marginTop:8}}>Record what you contributed</h3><p>Describe the work you completed and link the project records or evidence that help show your contribution. Your submission will be reviewed before it can become Mettelo Proof.</p></div><span className="chip">EVIDENCE</span></div>
    ...
    <label htmlFor="proof-url">Additional evidence URL</label><input id="proof-url" name="evidence_url" type="url" placeholder="GitHub PR, dashboard, document or published output"/><small>Add a relevant link if the project records above do not fully show your contribution.</small>
    <label className="consent"><input name="is_public" type="checkbox"/><span>If this contribution is verified, allow the resulting Mettelo Proof record to be visible publicly. Leave unchecked to keep the verified record private. Spotlight publication remains a separate consent process.</span></label>
    <button className="button dark" type="submit" disabled={status==='submitting'} style={{width:'100%',marginTop:20}}>{status==='submitting'?'Submitting…':'Submit for review →'}</button>

...
export default async function AdminCareerCandidatesPage({searchParams}:{searchParams:Promise<{role?:string}>}){
 ...
 const active=items.filter(x=>x.status!=='withdrawn'&&!['hired','rejected'].includes(x.final_outcome));
 const finalStage=items.filter(x=>['interview','offer','hired','rejected'].includes(x.status));
 return <section className="section softSection"><div className="shell"><AdminSectionTabs label="Career sections" tabs={[{label:'Roles',href:'/admin/careers/roles'},{label:'Candidates',href:'/admin/careers/applications'},{label:'Pipeline overview',href:'/admin/careers/pipeline'}]}/><div className="metricGrid" style={{marginBottom:18}}><div className="metric"><strong>{active.length}</strong><span>Active candidates</span></div><div className="metric"><strong>{items.filter(x=>x.status==='shortlisted').length}</strong><span>Shortlisted</span></div><div className="metric"><strong>{finalStage.length}</strong><span>Interview &amp; final decision</span></div><div className="metric"><strong>{items.filter(x=>x.final_outcome==='hired').length}</strong><span>Hired</span></div><div className="metric"><strong>{items.filter(x=>x.offer_status==='sent').length}</strong><span>Offers sent</span></div></div><div className="adminPageHeader"><div><div className="eyebrow">Admin / Recruiting / Careers / Candidates</div><h1>Candidates &amp; applications</h1><p>Manage four recruitment stages, then control interviews, final outcomes and candidate communications independently.</p></div></div><AdminCareerApplicationQueueV2 initialItems={items} initialRoleFilter={params.role||''}/></div></section>;
}

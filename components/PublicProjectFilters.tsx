...
export default function PublicProjectFilters({values,activeCount,resultCount,roles,experiences,formats,capabilities,domains,tools,commitments,workingModels,types,availabilities,stages,durations,paths}:Props){
  ...
  function openFilters(event:React.MouseEvent<HTMLButtonElement>){const node=dialog.current;if(!node||node.open)return;lastTrigger.current=event.currentTarget;node.showModal();document.body.style.overflow='hidden';emitCatalogueAnalytics({event:'filter_opened',surface:'public',active_count:activeCount,result_count:resultCount});requestAnimationFrame(()=>closeButton.current?.focus())}
  function closeFilters(){setCapabilityOpen(false);const node=dialog.current;if(!node?.open)return;node.close();document.body.style.overflow='';lastTrigger.current?.focus()}
  function cancelFilters(event:React.SyntheticEvent<HTMLDialogElement>){event.preventDefault();closeFilters()}
  ...
  return <form className="pfShell" method="get" action="/projects#projects" onSubmit={trackSubmit}>
    ...
    <dialog ref={dialog} className="pfDrawer" onCancel={cancelFilters} onClose={()=>{document.body.style.overflow='';setCapabilityOpen(false)}} onClick={event=>{if(event.target===dialog.current)closeFilters()}} aria-labelledby="pf-title" aria-describedby="pf-description">
      <div className="pfDrawerInner">
        ...
        <div className="pfDrawerBody">
          <fieldset><legend>Career fit</legend><div className="pfGrid">
            ...
            <label><span>Capability Path</span><select id="path-filter" name="path" defaultValue={values.path}><option value="all">All paths</option>{paths.map(item=><option key={item.slug} value={item.slug}>{item.label}</option>)}</select></label>

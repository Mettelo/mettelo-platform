...
import {normaliseOptionalWebUrl} from '@/lib/normalise-web-url';
...
export async function POST(request:Request){
  try{
    ...
    const form=await request.formData();const roleId=clean(form.get('role_id'),80),fullName=clean(form.get('full_name'),140),email=clean(form.get('email'),254).toLowerCase(),phone=clean(form.get('phone'),50),location=clean(form.get('location'),160),linkedinResult=normaliseOptionalWebUrl(clean(form.get('linkedin_url'),800)),portfolioResult=normaliseOptionalWebUrl(clean(form.get('portfolio_url'),800)),workAuth=clean(form.get('work_authorisation'),300),motivation=clean(form.get('motivation'),3000),experience=clean(form.get('relevant_experience'),4000);const submittedFile=form.get('cv');
    ...
    if(!linkedinResult.ok||!portfolioResult.ok)return NextResponse.json({error:'Enter a valid LinkedIn or portfolio web address with a domain.'},{status:400});
    const linkedin=linkedinResult.value,portfolio=portfolioResult.value;

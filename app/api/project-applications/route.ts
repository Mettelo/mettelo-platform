...
import {canonicalParticipationMode,resolveParticipationPreference} from '@/lib/project-admission';
...
const ACTIVE_STATUSES=['submitted','in_review','shortlisted','offered','approved','accepted','waiting_for_team','team_complete'];

function rpcError(error:DbError){
 const message=String(error.message||'');
 if(message.includes('DUPLICATE_APPLICATION'))return fail('You already have active interest in this project.',409,'DUPLICATE_APPLICATION');
 if(message.includes('PROJECT_NOT_FOUND'))return fail('Project not found.',404,'PROJECT_NOT_FOUND');
 if(message.includes('PROJECT_CLOSED'))return fail('This project is not currently accepting interest.',409,'PROJECT_CLOSED');
 if(message.includes('DEADLINE_PASSED'))return fail('The interest window for this project has closed.',409,'DEADLINE_PASSED');
 if(message.includes('PARTICIPATION_NOT_SUPPORTED')||message.includes('INVALID_PARTICIPATION_PREFERENCE'))return fail('Choose a participation option supported by this project.',400,'INVALID_PARTICIPATION_PREFERENCE','participation_preference');
 if(message.includes('PRIMARY_ROLE_REQUIRED'))return fail('Choose your primary project role.',400,'PRIMARY_ROLE_REQUIRED','project_role_id');
 if(message.includes('INVALID_PRIMARY_ROLE')||message.includes('INVALID_SECONDARY_ROLE')||error.code==='23503')return fail('That project role is no longer available. Please choose another role.',409,'ROLE_UNAVAILABLE','project_role_id');
 if(message.includes('SOLO_ROLE_NOT_ALLOWED'))return fail('Solo participation owns the complete project outcome and does not use a team role.',400,'SOLO_ROLE_NOT_ALLOWED','project_role_id');
 if(message.includes('FLEXIBLE_PREFERENCE_REQUIRED'))return fail('Tell us how you would prefer to work if the project can form either way.',400,'FLEXIBLE_PREFERENCE_REQUIRED','flexible_preference');
 if(message.includes('COMMITMENT_REQUIRED'))return fail('Confirm whether you can meet the published project commitment.',400,'COMMITMENT_REQUIRED','commitment_response');
 if(message.includes('MOTIVATION_REQUIRED'))return fail('Tell us why you want to work on this project.',400,'MOTIVATION_REQUIRED','motivation_statement');
 if(message.includes('CONTRIBUTION_REQUIRED'))return fail('Tell us what you would contribute in at least 40 characters.',400,'CONTRIBUTION_REQUIRED','contribution_statement');
 if(error.code==='42501')return fail('Your session has expired. Please sign in again.',401,'AUTH_REQUIRED');
 return fail('We could not submit your interest right now. Your responses are still available; please try again.',503,'SUBMISSION_FAILED');
}

...
export default async function OpportunitiesPage(){
  ...
  return <>
    ...
    <section className="opportunityTrustSection" aria-labelledby="opportunity-trust-title"><div className="shell opportunityTrustGrid"><div className="opportunityTrustIntro"><div className="eyebrow">WHY METTELO</div><h2 id="opportunity-trust-title">Useful routes forward, with clearer context.</h2><p>Opportunities are one part of Mettelo: a way to explore possible next moves alongside the experience, contribution evidence and professional context you build through real work.</p></div><div className="opportunityTrustItems"><article><span>01</span><h3>Relevant</h3><p>This public feed currently focuses on Data &amp; AI opportunities rather than claiming to cover every technology discipline.</p></article><article><span>02</span><h3>Current</h3><p>Listings leave the public feed when their closing date has passed.</p></article><article><span>03</span><h3>Clearer</h3><p>Location, work arrangement, applicant scope and sponsorship are surfaced when known so you can assess fit yourself.</p></article></div></div></section>
    <section className="opportunityAlertCta"><div className="shell opportunityAlertBand"><div><div className="eyebrow">STAY CLOSE TO RELEVANT OPPORTUNITIES</div><h2>Don&apos;t want to keep checking?</h2><p>Get new Data &amp; AI opportunities and Mettelo updates in your inbox. Listings are routes to explore, not guarantees of fit, selection or outcome.</p></div><div className="actions"><a className="button dark" href={copy.final_cta_href}>{copy.final_cta_label}</a></div></div></section>
  </>
}

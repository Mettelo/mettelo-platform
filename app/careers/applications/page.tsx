...
import CareerApplicationTrackerV2,{type CareerApplicationItemV2} from '@/components/CareerApplicationTrackerV2';

type CareerRow=Omit<CareerApplicationItemV2,'events'|'offer_documents'|'onboarding_items'>;
...
export default async function CareerApplicationsPage(){
  ...
  const {data:careerData,error}=await auth.from('career_applications')
    .select('id,status,final_outcome,offer_status,offer_sent_at,submitted_at,updated_at,withdrawn_at,interview_at,interview_timezone,interview_format,interview_url,interviewer,interview_instructions,offer_salary_rate,offer_start_date,offer_employment_type,offer_manager,offer_working_arrangement,offer_conditions,offer_acceptance_deadline,offer_personal_message,career_roles(title)')
    .eq('user_id',user.id)
  ...
  const applications:CareerApplicationItemV2[]=rows.map(item=>({...item,final_outcome:item.final_outcome||'pending',offer_status:item.offer_status||'not_prepared',events:events.filter(event=>event.application_id===item.id),offer_documents:docs.filter(doc=>doc.application_id===item.id).map(doc=>({id:doc.id,file_name:doc.file_name,size_bytes:doc.size_bytes,created_at:doc.created_at})),onboarding_items:onboarding.filter(row=>row.application_id===item.id).map(row=>({id:row.id,item_key:row.item_key,title:row.title,description:row.description,status:row.status,due_at:row.due_at,completed_at:row.completed_at}))}));

  return <main className="careerApplicationsPage" aria-labelledby="career-applications-title"><div className="careerApplicationsShell">
    ...
    <header className="careerHero"><div><span>METTELO CAREERS</span><h1 id="career-applications-title">Your career applications</h1><p>Recruitment applications, interview details, offers and onboarding live here in Careers. Follow your application through four recruitment stages, with final outcomes and formal offers shown separately so each update is clear.</p></div><a className="careerButton" href="/careers">View open roles</a></header>
    {error?<section className="careerError" role="alert"><h2>We could not load your career applications</h2><p>Refresh the page to try again. No application data has been changed.</p></section>:applications.length?<CareerApplicationTrackerV2 applications={applications}/>:<section className="careerEmpty"><h2>No career applications yet</h2><p>When you apply for a Mettelo role while signed in, you can follow your recruitment progress here.</p><a className="careerButton" href="/careers">View open roles</a></section>}
  </div><style>{`

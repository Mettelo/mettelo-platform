...
type Phase11ReadinessRpc={ready?:boolean;blockers?:string[];project?:{ready?:boolean;blockers?:string[]};team?:{ready?:boolean;blockers?:string[];filled?:number;required_team_size?:number;maximum_team_size?:number};system?:{ready?:boolean;blockers?:string[]}};
...
export async function startProjectRun({db,projectId,runId,source,actorUserId=null}:{db:Db;projectId:string;runId:string;source:StartSource;actorUserId?:string|null}):Promise<StartResult>{
 ...
 if(!phase11.ready){
  ...
  return{started:false,notReady:true,blockers,projectId,runId,runNumber:run.run_number,filled:readinessFilled,requiredTeamSize:readinessRequired};
 }
 ...
 const filled=Number(result.filled??readinessFilled);
 const requiredTeamSize=Number(result.required_team_size??readinessRequired);
 if(!result.started){

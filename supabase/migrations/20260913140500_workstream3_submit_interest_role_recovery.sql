-- Workstream 3: restore Role & Contribution persistence at the canonical interest RPC.
-- This retains the Workstream 2 readiness/capacity authority and restores the
-- Team/Flexible role validation that was lost in the later RPC replacement.
...
begin
  ...
  if p_participation_preference='solo' then
    ...
    if p_primary_project_role_id is not null then
      ...
      where id=p_primary_project_role_id and project_id=p_project_id and role_status in ('open','limited') for update;
      if not found then raise exception 'INVALID_PRIMARY_ROLE' using errcode='23503'; end if;
      select count(*)::integer into v_primary_filled from public.project_members
      where project_id=p_project_id and project_role_id=p_primary_project_role_id and membership_status in ('waiting','active');
      if v_primary_filled>=greatest(1,v_primary.openings) then raise exception 'PRIMARY_ROLE_FULL' using errcode='23514'; end if;
    end if;
    if p_secondary_project_role_id is not null then
      if p_secondary_project_role_id=p_primary_project_role_id then raise exception 'INVALID_SECONDARY_ROLE' using errcode='22023'; end if;
      select * into v_secondary from public.project_roles
      where id=p_secondary_project_role_id and project_id=p_project_id and role_status in ('open','limited') for update;
      if not found then raise exception 'INVALID_SECONDARY_ROLE' using errcode='23503'; end if;
      select count(*)::integer into v_secondary_filled from public.project_members
      where project_id=p_project_id and project_role_id=p_secondary_project_role_id and membership_status in ('waiting','active');
      if v_secondary_filled>=greatest(1,v_secondary.openings) then raise exception 'SECONDARY_ROLE_FULL' using errcode='23514'; end if;
    end if;
  end if;
  ...
  insert into public.project_applications(
    ...
    p_project_id,
    case when p_participation_preference='solo' then null else p_primary_project_role_id end,

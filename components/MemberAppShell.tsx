...
export default function MemberAppShell({children}:{children:React.ReactNode}){
  ...
  return <div className={styles.appShell}>
    ...
    <div className={styles.appBody}><aside className={styles.sidebar} aria-label="Mettelo member navigation"><div className={styles.sidebarIntro}><span>Workspace</span><strong>My Mettelo</strong></div>{account?.isAdmin&&<Link className={styles.modeCard} href="/admin/project-operations"><small>MODE</small><strong>Switch to Admin</strong><span>Manage projects & access →</span></Link>}<nav>{nav}</nav><div className={styles.sidebarHelp}><Link href="/feedback">Give feedback →</Link><Link href="/contact">Help & support →</Link></div></aside><main className={styles.content}><WorkspaceRouteTabs/>{children}</main></div>

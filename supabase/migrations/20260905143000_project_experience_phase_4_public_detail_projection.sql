...
as $$
  select case
    when not exists (
      ...
      'data_sources', coalesce((select jsonb_agg(jsonb_build_object('id',s.id,'name',s.name,'description',s.description,'source_type',s.source_type,'provider_name',s.provider_name,'licence_name',s.licence_name,'required_subset',s.required_subset,'approximate_size',s.approximate_size,'data_period',s.data_period,'data_format',s.data_format,'known_limitations',s.known_limitations,'provenance',s.provenance,'governance_status',s.governance_status,'governance_verified_at',s.governance_verified_at,'retention_policy',s.retention_policy) order by s.created_at) from public.project_data_sources s where s.project_id=p_project_id and s.project_run_id is null and s.sensitivity='public' and s.publish_policy='permitted' and s.governance_status='green'),'[]'::jsonb),
      ...
      'import_origin', (select jsonb_build_object('source_project_key',origin.source_project_key,'normalized',import_row.normalized) from public.capability_path_import_project_origins origin left join public.capability_path_import_rows import_row on import_row.batch_id=origin.batch_id and import_row.row_kind='project' and import_row.source_key=origin.source_project_key where origin.project_id=p_project_id order by origin.created_at desc limit 1),
      ...
      'role_details', coalesce((select jsonb_agg(jsonb_build_object('id',r.id,'responsibilities',r.responsibilities,'recommended_skills',r.recommended_skills,'experience_expectation',r.experience_expectation,'weekly_commitment',r.weekly_commitment,'role_status',r.role_status,'application_requirements',r.application_requirements) order by r.created_at) from public.project_roles r where r.project_id=p_project_id),'[]'::jsonb)
    )
  end;
$$;
...
comment on function public.get_public_project_experience_detail(uuid) is 'Phase 4 public-safe Project Experience projection. Returns rich detail only for public projects and deliberately excludes protected resource URLs, storage paths, access/review evidence, and run-scoped execution data.';

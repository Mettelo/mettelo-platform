...
function openStatus(state:MemberProjectState,deadline:string|null){
  ...
  return days>0&&days<=7?`Closing soon · ${days} ${days===1?'day':'days'}`:'Open for applications';
}

export default function MemberProjectDetailV2({model,state,stateLabel,stateCopy,primaryAction,applicationReady,profileCompletion,applicationMissing,roleAvailabilityKnown,saved,roles}:Props){
  const project=model.project;
  const available=useMemo(()=>roles.filter(role=>role.available),[roles]);
  const [selectedRole,setSelectedRole]=useState(available[0]?.id||'');
  const selected=roles.find(role=>role.id===selectedRole)||null;
  const applyHref=selected?`/member/discover/${project.id}/apply?role=${encodeURIComponent(selected.id)}`:`/member/discover/${project.id}/apply`;
  const status=openStatus(state,project.applicationDeadline)||stateLabel;
  const action=state==='open_eligible'?{label:'Choose a role',href:'#roles'}:primaryAction;
  ...
  return <div className={styles.page}>
    ...
    <header className={styles.hero} aria-labelledby="member-project-title">
      <div className={styles.heroMain}>
        <div className={styles.eyebrow}>MEMBER PROJECT DETAIL</div>
        <h1 id="member-project-title">{project.title}</h1>
        <p>{project.summary}</p>
        <div className={styles.pills} aria-label="Project characteristics">
          <span className={`${styles.pill} ${state==='open_eligible'?styles.open:''}`}>{status}</span>
          {project.durationWeeks&&<span className={styles.pill}>{project.durationWeeks} {project.durationWeeks===1?'week':'weeks'}</span>}
          {project.weeklyCommitment&&<span className={styles.pill}>{project.weeklyCommitment}</span>}
          <span className={styles.pill}>{workingModel}</span>
        </div>
      </div>

      <aside className={styles.decision} aria-labelledby="member-decision-title">
        <div className={styles.eyebrow}>YOUR DECISION</div>
        <h2 id="member-decision-title">{state==='open_eligible'?'Ready to contribute?':stateLabel}</h2>
        <p>{stateCopy}</p>
        <dl className={styles.meta}>
          <div><dt>Duration</dt><dd>{project.durationWeeks?`${project.durationWeeks} ${project.durationWeeks===1?'week':'weeks'}`:'Not published'}</dd></div>
          <div><dt>Commitment</dt><dd>{project.weeklyCommitment||'Not published'}</dd></div>
          <div><dt>Open roles</dt><dd>{roleAvailabilityKnown?available.length:'Checking availability'}</dd></div>
          <div><dt>Applications close</dt><dd>{date(project.applicationDeadline)}</dd></div>
        </dl>
        {state==='ineligible'&&!applicationReady&&<div className={styles.readiness} role="status"><strong>{applicationMissing.length} profile requirement{applicationMissing.length===1?'':'s'} remaining</strong><span>Profile completion: {profileCompletion}%</span></div>}
        {action&&<Link className={styles.primaryButton} href={action.href}>{action.label}</Link>}
        <SaveProjectButton projectId={project.id} initialSaved={saved}/>
        {state==='ineligible'&&!applicationReady&&<Link className={styles.secondaryButton} href={`/member/profile?next=${encodeURIComponent(`/member/discover/${project.id}`)}`}>Complete profile requirements</Link>}
      </aside>
    </header>

    <main id="member-project-main" className={styles.layout}>
      <div className={styles.content}>
        <ProjectMemberCanonicalSections model={model}/>

        <section className="pdv2Section" id="roles" aria-labelledby="member-roles-heading">
        ...
        <section className="pdv2Section" aria-labelledby="member-next-heading">
          <div className="pdv2Eyebrow">09 · WHAT HAPPENS NEXT</div>
          <h2 id="member-next-heading">Your project journey</h2>
          <div className="pdv2Timeline"><div><span>1</span><p><strong>Review the canonical brief</strong>Understand the problem, objectives, resources, deliverables and quality bar.</p></div><div><span>2</span><p><strong>Choose one available role</strong>Select the responsibility you can credibly own.</p></div><div><span>3</span><p><strong>Submit your application</strong>Your profile readiness, evidence and role selection remain governed by the existing application workflow.</p></div><div><span>4</span><p><strong>Track the decision</strong>Your status and next action remain visible in My Mettelo.</p></div><div><span>5</span><p><strong>Deliver in Mettelo Lab</strong>Accepted project members move into the authorised workspace with the same canonical project definition.</p></div></div>
        </section>
      </div>

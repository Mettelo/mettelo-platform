...
export default function MemberProjectOffers(){
  ...
  async function respond(offer:Offer,action:Action){
    ...
    try{
      ...
      closeDialog(false);
      const active=body.participation?.project_active===true;
      const formed=body.participation?.creates_membership===true;
      const blockers=Array.isArray(body.participation?.start_blockers)?body.participation.start_blockers.map((item:string)=>item.replaceAll('_',' ')).join(', '):'';
      setMessage(next==='accepted'
        ?active
          ?`Place accepted. ${project} is active and ready in My Projects.`
          :formed
            ?`Place accepted and confirmed. ${blockers?`Start readiness still needs: ${blockers}.`:'Team formation/readiness is continuing.'}`
            :`Place accepted. Your acceptance is recorded, but formation needs operational attention.`
        :`Place declined. The reserved capacity for ${project} has been released.`);
    ...
    <section className="mpoPanel" aria-labelledby="member-project-offers-title">
      ...
      <div className="mpoList">
        {offers.map(offer=>{const project=projectOf(offer);const state=displayStatus(offer,now);const pending=state==='pending';const partner=project?.project_type==='partner';return <article className={`mpoCard mpo-${state}`} key={offer.id} aria-label={`${statusLabel(state)} for ${project?.title||'Mettelo project'}`}>
          ...
          {pending?<><p className="mpoDecisionCopy"><strong>If you accept:</strong> your reserved place becomes confirmed membership. Solo and Flexible-independent participation can start immediately when readiness is clear; Team participation continues until its governed minimum and readiness requirements are met. <strong>If you decline:</strong> the offer closes and the reserved place is released.</p><div className="mpoActions"><button className="mpoButton mpoDark" type="button" disabled={working===offer.id} onClick={event=>openAction(offer,'accept',event.currentTarget)}>Accept place</button><button className="mpoButton" type="button" disabled={working===offer.id} onClick={event=>openAction(offer,'decline',event.currentTarget)}>Decline</button></div></>:<p className="mpoResolved">{state==='accepted'?'Place accepted. Your place is confirmed when formation succeeds; check My Projects for an active workspace or Applications for any remaining readiness step.':state==='declined'?'This offer is closed and its reserved capacity has been released.':'Offer expired. This offer is read-only and its reserved capacity is released.'} <a href={state==='accepted'?'/member/projects':'/member/discover'}>{state==='accepted'?'Open My Projects':'Discover projects'}</a></p>}
        </article>})}
      </div>

...
export async function POST(request:Request){
  try{
    ...
    const message=careerMessage('submitted',role.title);const emailResult=await sendCareerEmail(db,{email,subject:message.subject,body:message.body,templateKey:'career_application_submitted',userId:user?.id||null,actionUrl:user?'/member':null});

...
export default async function MetteloLabPanel(props:Props){
 ...
 return <section className={styles.metteloLab} id="mettelo-lab" aria-labelledby="mettelo-lab-title">
  <header className={styles.labHeader} data-lab-home-section><span className={styles.labEyebrow}>METTELO LAB / HOME</span><h2 id="mettelo-lab-title">{props.projectTitle}</h2><p>{props.projectSummary||'Your project workspace for planning, delivery, collaboration, data, events and proof.'}</p><div className={styles.identityChips}><span>{humanise(props.runStatus)}</span><span>{humanise(props.workspaceRole)}</span>{current&&<span>Team {current.run_number}</span>}</div></header>
  <section className={styles.nextAction} data-lab-home-section aria-labelledby="lab-next-action"><div><span className={styles.labLabel}>UP NEXT</span><h3 id="lab-next-action">{actionTitle}</h3><p>{actionCopy}</p></div><a className={styles.labButton} href={actionHref}>{actionLabel} →</a></section>
  <section className={styles.summary} data-lab-home-section aria-labelledby="lab-summary-title"><div className={styles.sectionTitle}><span className={styles.labLabel}>PROJECT PROGRESS</span><h3 id="lab-summary-title">At a glance</h3></div><div className={styles.summaryGrid}><Stat label="Your role" value={humanise(props.workspaceRole)}/><Stat label="Project Lead" value={lead?.name||'Not assigned yet'}/><Stat label="Project Architect" value={architect||'Not assigned yet'}/><Stat label="Team status" value={humanise(props.runStatus)}/><Stat label="Milestones" value={`${props.completedMilestones}/${props.totalMilestones}`}/><Stat label="Tasks done" value={`${props.completedTasks}/${props.totalTasks}`}/></div></section>

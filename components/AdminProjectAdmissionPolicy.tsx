...
export default function AdminProjectAdmissionPolicy({projectId,projectType,partnerName=null,admissionMode,autoStartDelayMinutes,autoStartPaused,lateJoiningEnabled=true,lateJoiningCutoffAt=null,projectSharingEnabled=true,memberInvitesEnabled=false}:Props){
 ...
 const[delay,setDelay]=useState(autoStartDelayMinutes||360);
 ...
 const load=useCallback(async()=>{
  ...
  setMode(effective);
  setDelay(Number(item.auto_start_delay_minutes??360));
 ...
 async function save(){
  ...
  try{
   const response=await fetch('/api/admin/project-admission',{method:'PATCH',headers:{'content-type':'application/json'},body:JSON.stringify({project_id:projectId,admission_mode:mode,auto_start_delay_minutes:delay,auto_start_paused:paused,late_joining_enabled:lateJoining,late_joining_cutoff_at:lateJoiningCutoff,project_sharing_enabled:sharing,member_invites_enabled:invites})});
 ...
 return <section className="admissionPolicy" aria-labelledby="admission-policy-title">
  ...
  <div className="admissionGrid">
   ...
   <label>Auto-start delay (minutes)<input type="number" min={0} max={10080} value={delay} disabled={partner||mode!=='auto'} onChange={event=>setDelay(Math.max(0,Math.min(10080,Number(event.target.value)||0)))}/><small>{mode==='auto'&&delay===360?'Standard intervention window: 6 hours.':'Per-project Open AUTO delay.'}</small></label>

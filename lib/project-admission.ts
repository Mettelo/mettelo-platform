...
export type ParticipationPreference='solo'|'team'|'flexible'|'either';
export type CanonicalParticipationPreference='solo'|'team'|'flexible';
...
/** Applicant-facing choices. `either` remains a supported legacy admission value but is never emitted by the new Submit Interest journey. */
export function participationOptions(mode:ProjectParticipationMode):CanonicalParticipationPreference[]{
  if(mode==='solo')return['solo'];
  if(mode==='team')return['team','flexible'];
  return['solo','team','flexible'];
}

export function resolveParticipationPreference(mode:ProjectParticipationMode,value:unknown):{ok:true;preference:CanonicalParticipationPreference}|{ok:false;error:string}{
  const normalized=value==='either'?'flexible':value;
  const options=participationOptions(mode);
  if(typeof normalized==='string'&&options.includes(normalized as CanonicalParticipationPreference))return{ok:true,preference:normalized as CanonicalParticipationPreference};
...
export function requiredMembersToScheduleStart(input:{participationMode:ProjectParticipationMode;preference:ParticipationPreference;minimum:number|null|undefined}){
  ...
  if(input.participationMode==='flexible'&&input.preference==='either')return 1;

  ...
  return <div className={styles.page}>
    ...
    <div id="project-content" className={styles.content}>
      <section className={styles.section} aria-labelledby="challenge-title">
        ...
        {(challenge.problemStatement||challenge.businessContext||challenge.useCase||challenge.constraintsTradeOffs.length||challenge.assumptions.length||challenge.responsibleUseRisks.length)&&<details className={styles.specDetails}><summary>View full challenge and scope</summary><div className={styles.specBody}>{challenge.problemStatement&&<><h3>Full challenge</h3><p>{challenge.problemStatement}</p></>}{challenge.businessContext&&<><h3>Business context</h3><p>{challenge.businessContext}</p></>}{challenge.useCase&&<><h3>Primary use case</h3><p>{challenge.useCase}</p></>}{challenge.keyQuestions.length>0&&<><h3>Questions the team must answer</h3><ul>{challenge.keyQuestions.map(item=><li key={item}>{item}</li>)}</ul></>}{challenge.inScope.length>0&&<><h3>In scope</h3><ul>{challenge.inScope.map(item=><li key={item}>{item}</li>)}</ul></>}{challenge.outOfScope.length>0&&<><h3>Out of scope</h3><ul>{challenge.outOfScope.map(item=><li key={item}>{item}</li>)}</ul></>}{challenge.constraintsTradeOffs.length>0&&<><h3>Constraints & trade-offs</h3><ul>{challenge.constraintsTradeOffs.map(item=><li key={item}>{item}</li>)}</ul></>}{challenge.assumptions.length>0&&<><h3>Explicit assumptions</h3><ul>{challenge.assumptions.map(item=><li key={item}>{item}</li>)}</ul></>}{challenge.responsibleUseRisks.length>0&&<><h3>Responsible use & risks</h3><ul>{challenge.responsibleUseRisks.map(item=><li key={item}>{item}</li>)}</ul></>}</div></details>}
      </section>

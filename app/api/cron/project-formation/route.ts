...
export async function GET(request:Request){
  ...
  try{
    ...
    let cancelled=0;let notified=0;let started=0;let paused=0;let blocked=0;let policySkipped=0;let deferred=0;let failed=0;
    ...
    return NextResponse.json({ok:true,cancelled,notified,scheduled:{due:(dueRuns||[]).length,started,paused,blocked,policySkipped,deferred,failed}});
  }catch(error){

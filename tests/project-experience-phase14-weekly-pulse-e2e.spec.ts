...
test.describe('Project Experience Phase 14 weekly pulse',()=>{
 test('member pulse, private RLS and Lead/Admin aggregate health work end to end',async({page,browser})=>{
  ...
  try{
   ...
   const own=await memberClient.from('project_weekly_pulses').select('user_id,progress,note').eq('project_run_id',runId).eq('period_start',periodStart);if(own.error)throw own.error;expect(own.data).toHaveLength(1);expect(own.data?.[0].user_id).toBe(memberId);expect(own.data?.[0].note).toContain('Private context');

   const leadClient=await signInClient(anon(),leadEmail,password);
   const hiddenRaw=await leadClient.from('project_weekly_pulses').select('user_id,progress,note').eq('project_run_id',runId).eq('period_start',periodStart);if(hiddenRaw.error)throw hiddenRaw.error;expect(hiddenRaw.data).toEqual([]);
   const leadHealth=await leadClient.rpc('project_weekly_pulse_health',{target_project:projectId,target_run:runId,target_period:periodStart});if(leadHealth.error)throw leadHealth.error;const leadRow=Array.isArray(leadHealth.data)?leadHealth.data[0]:leadHealth.data;expect(Number(leadRow.submissions)).toBe(1);expect(Number(leadRow.some_risk)).toBe(1);expect(Number(leadRow.heavy)).toBe(1);expect(Number(leadRow.some_friction)).toBe(1);expect(Number(leadRow.support_maybe)).toBe(1);expect('note' in leadRow).toBeFalsy();expect('user_id' in leadRow).toBeFalsy();expect('health_score' in leadRow).toBeFalsy();

   const outsiderClient=await signInClient(anon(),outsiderEmail,password);
   const outsiderInsert=await outsiderClient.from('project_weekly_pulses').insert({project_id:projectId,project_run_id:runId,user_id:outsiderId,period_start:periodStart,progress:'on_track',workload:'manageable',team_state:'working_well',support_need:'no'});expect(outsiderInsert.error).toBeTruthy();

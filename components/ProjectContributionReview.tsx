...
type Item={id:string;name:string;title:string;type:string;description:string|null;evidence_url:string|null;status:string;task:string|null;created_at:string};
...
function ReviewItem({item,working,onUpdate}:{item:Item;working:boolean;onUpdate:(id:string,status:string,notes:string)=>void}){
  ...
  return <article className="applicationReview"><div className="panelHead"><div><span className="chip">{item.status.replace('_',' ').toUpperCase()}</span><h3 style={{margin:'10px 0 4px'}}>{item.title}</h3><small>{item.name} · {item.type.replace('_',' ')}{item.task?` · ${item.task}`:''}</small></div><small>{new Date(item.created_at).toLocaleString('en-GB')}</small></div>{item.description&&<p>{item.description}</p>}{item.evidence_url&&<a className="linkArrow" href={item.evidence_url} target="_blank" rel="noopener noreferrer">Inspect evidence →</a>}<label htmlFor={`review-${item.id}`}>Reviewer notes</label><textarea id={`review-${item.id}`} value={notes} onChange={e=>setNotes(e.target.value)} placeholder="Specific feedback, decision or changes required."/><div className="reviewActions"><button type="button" className="button ghost" disabled={working||!notes.trim()} onClick={()=>onUpdate(item.id,'needs_changes',notes)}>Changes required</button><button type="button" className="button dark" disabled={working} onClick={()=>onUpdate(item.id,'verified',notes)}>Verify</button><button type="button" className="button ghost" disabled={working} onClick={()=>onUpdate(item.id,'rejected',notes)}>Reject</button></div></article>;
}

...
export default async function SpotlightDetailPage({params}:{params:Promise<{id:string}>}){
  ...
  return <section className="section softSection"><div className="shell spotlightDetailShell">
    <article className="spotlightDetailCard" aria-labelledby="spotlight-title">
      ...
      <dl className="spotlightDetailFacts"><div><dt>Award</dt><dd>{categoryLabel(item.category)}</dd></div><div><dt>Recognition month</dt><dd>{monthLabel(item.awardMonth)}</dd></div>{item.publishedAt&&<div><dt>Published</dt><dd>{new Date(item.publishedAt).toLocaleDateString('en-GB',{dateStyle:'long'})}</dd></div>}</dl>

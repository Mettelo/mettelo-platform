...
as $$
  select case
    when not exists (
      ...
      'role_details', coalesce((select jsonb_agg(jsonb_build_object('id',r.id,'responsibilities',r.responsibilities,'recommended_skills',r.recommended_skills,'experience_expectation',r.experience_expectation,'weekly_commitment',r.weekly_commitment,'role_status',r.role_status) order by r.created_at) from public.project_roles r where r.project_id=p_project_id),'[]'::jsonb)
    )
  end;
$$;
...
comment on function public.get_public_project_experience_detail(uuid) is 'Phase 4 public-safe Project Experience projection. Returns rich public detail only for public projects. Resource governance administration, retention decisions, protected URLs/storage/access/review evidence, detailed application requirements and run-scoped execution data are deliberately excluded.';

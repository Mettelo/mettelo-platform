...
expect('app/projects/[id]/page.tsx',["const memberProjectHref=`/member/discover/${project.id}`","get_public_project_capacity',{p_project_id:project.id}","`/signin?next=${encodeURIComponent(memberProjectHref)}`",'ctaHref={ctaHref}']);
...
expect('lib/member-project-journey.ts',["capacityKnown=input.capacityKnown??input.roleAvailabilityKnown??false","capacityAvailable=input.capacityAvailable??input.hasAvailableRole??false","return{state:'open_eligible',reason:'ELIGIBLE',eligible:true}",'Applications open','Your place is confirmed','Project in progress']);

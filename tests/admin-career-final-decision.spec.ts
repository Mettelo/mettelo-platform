...
test.describe('Admin career final decision',()=>{
 test('Hired outcome, hired message and formal offer remain independent actions',async({page})=>{
  ...
  try{
   await signIn(page,'/admin/careers/applications');await page.goto('/admin/careers/applications',{waitUntil:'networkidle'});await page.getByRole('searchbox',{name:'Search candidates'}).fill(marker);await page.getByRole('button',{name:'View →'}).click();const detail=page.locator('dialog.candidateDetail');await expect(detail).toBeVisible();
   await detail.getByRole('button',{name:'Hired',exact:true}).click();await expect(page.getByRole('status').first()).toContainText('No candidate communication was sent');

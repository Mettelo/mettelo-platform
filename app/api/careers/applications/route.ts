...
export async function PATCH(request:Request){
  try{
    ...
    const {data:application}=await db.from('career_applications').select('id,user_id,email,full_name,status,final_outcome,career_roles(title)').eq('id',id).eq('user_id',user.id).maybeSingle();if(!application)return NextResponse.json({error:'Application not found.'},{status:404});
    if(!WITHDRAWABLE.has(application.status)||(['hired','rejected'].includes(String(application.final_outcome||''))))return NextResponse.json({error:'This application can no longer be withdrawn online. Contact Mettelo if you need help.'},{status:409});
    ...
    await db.from('communication_audit_log').insert({actor_user_id:user.id,action:'career_application_withdrawn',entity_type:'career_application',entity_id:id,metadata:{previous_status:application.status,previous_final_outcome:application.final_outcome||'pending',email_sent:sent.sent}});

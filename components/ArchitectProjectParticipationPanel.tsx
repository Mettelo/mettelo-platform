...
export default function ArchitectProjectParticipationPanel({projectId}:Props){
  ...
  const capacityHelp=useMemo(()=>value.participation_mode==='solo'
    ...
    :value.participation_mode==='flexible'
      ?'Flexible members choosing Solo or Either can satisfy participation readiness independently. Members choosing Team use the configured minimum below. Target is desirable capacity and maximum is the hard limit.'
  ...
  function setMode(mode:Mode){
    ...
    if(mode==='flexible'){setValue(current=>{const min=Math.max(2,current.min_team_size);const target=Math.max(min,current.target_team_size);return{...current,participation_mode:'flexible',min_team_size:min,target_team_size:target,max_team_size:Math.max(current.max_team_size,target),team_size_threshold:min}});return}
  ...
  return <section className={styles.panel} aria-labelledby="project-participation-heading">
    ...
    <p className={styles.intro}>Choose the canonical participation model and capacity. For Team projects—and Flexible members choosing Team—the minimum is the participation threshold. Target is desirable planning capacity and never blocks start after the applicable minimum is reached. Maximum is the hard capacity limit.</p>
    {loading?<p className={styles.status}>Loading participation…</p>:<form onSubmit={save}>
      <fieldset disabled={!editable||busy} className={styles.fieldset}><legend>Participation mode</legend><div className={styles.modes}>
        {(['solo','team','flexible'] as Mode[]).map(mode=><label key={mode} className={styles.mode}><input type="radio" name="participation-mode" value={mode} checked={value.participation_mode===mode} onChange={()=>setMode(mode)}/><span><strong>{mode[0].toUpperCase()+mode.slice(1)}</strong><small>{mode==='solo'?'One participant throughout.':mode==='team'?'Starts when the minimum viable team is ready.':'Solo/Either can start independently; Team preference uses the configured minimum.'}</small></span></label>)}
      </div></fieldset>
      <div className={styles.capacity}>
        <label>{value.participation_mode==='flexible'?'Team minimum':'Minimum to start'}<input aria-describedby="participation-capacity-help" type="number" min="1" max="50" value={value.min_team_size} disabled={!editable||busy||value.participation_mode==='solo'} onChange={event=>setCapacity('min_team_size',event.target.value)}/></label>

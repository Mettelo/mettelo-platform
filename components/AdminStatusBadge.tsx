...
const iconByStatus:Record<string,string>={draft:'○',pilot:'◇',open:'●',recruiting:'●',forming:'◎',active:'✓',paused:'Ⅱ',review:'◷',completed:'✓',archived:'—',private:'◒',public:'●',submitted:'◷',in_review:'◷',under_review:'◷',shortlisted:'★',approved:'✓',declined:'×',failed:'!',inactive:'○',success:'✓',completed_sync:'✓',waiting:'◷',confirmed:'✓'};
...
export default function AdminStatusBadge({status,label}:{status:string;label?:string}){
  ...
  const tone=['active','approved','completed','public','success','confirmed'].includes(key)?'success':['forming','shortlisted','pilot','review','waiting'].includes(key)?'warning':['failed','declined'].includes(key)?'danger':['paused','archived','inactive','private'].includes(key)?'neutral':'info';

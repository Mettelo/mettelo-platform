...
export async function PATCH(request:Request){
  try{
    ...
    const {data:existing}=await supabase.from('contributions').select('id,user_id,verification_status').eq('id',id).eq('user_id',user.id).maybeSingle();
    ...
    const {data,error}=await supabase.from('contributions').update({visibility,visibility_reviewed_at:now}).eq('id',id).eq('user_id',user.id).select('id,visibility,visibility_reviewed_at').single();

...
async function loadOpportunity(id:string){
 ...
 const [{data:project},{data:run},{data:links},capacityResult]=await Promise.all([
  db.from('projects').select('id,slug,title,summary,status,visibility,project_type,weekly_commitment,late_joining_enabled,late_joining_cutoff_at,collaboration_marketplace_enabled,collaboration_social_sharing_enabled').eq('id',row.project_id).maybeSingle(),
 ...
 if(!project||!run||project.visibility!=='public'||project.collaboration_marketplace_enabled!==true)return null;
 ...
 return{need:row,project,run,role:roleResult.data?.title||null,domain:domainResult.data?.name||null,capabilities:(capabilityResult.data||[]).map(item=>String(item.name)),openPlaces:accepting?Number(capacity?.available||0):0,accepting,socialSharingEnabled:project.collaboration_social_sharing_enabled===true};
}
...
export async function generateMetadata({params}:{params:Promise<{id:string}>}):Promise<Metadata>{
 ...
 return{title,description,alternates:{canonical},robots:{index:item.accepting&&item.socialSharingEnabled,follow:true},openGraph:{title,description,url:canonical,siteName:'Mettelo',type:'website'},twitter:{card:'summary',title,description}};
}
...
export default async function CollaborationOpportunityPage({params,searchParams}:{params:Promise<{id:string}>;searchParams?:Promise<Search>}){
 ...
 const analyticsDb=serviceDb();if(analyticsDb){const {error}=await analyticsDb.rpc('phase18_record_collaboration_analytics',{p_event_type:'opportunity_viewed',p_collaboration_need_id:id,p_surface:'public_opportunity',p_actor_user_id:user?.id||null});if(error)console.error('collaboration opportunity view analytics failed',error.message)}
 const role=item.role||item.need.responsibility||'Project collaborator';const commitment=item.need.weekly_commitment||item.project.weekly_commitment||'See project details';const publicUrl=`https://mettelo.com/collaborate/${encodeURIComponent(id)}`;const responseTarget=`/collaborate/${encodeURIComponent(id)}/respond`;const inviteLanding=`/collaborate/${encodeURIComponent(id)}${inviteToken?`?invite=${encodeURIComponent(inviteToken)}`:''}`;const cta=user?responseTarget:`/signin?next=${encodeURIComponent(inviteToken?inviteLanding:responseTarget)}`;const projectHref=item.project.slug?`/projects/${encodeURIComponent(item.project.slug)}`:'/projects';const stage=String(item.project.status||'').replace(/[_-]+/g,' ').replace(/\b\w/g,letter=>letter.toUpperCase());const shareText=`We’re looking for ${role} to join the Mettelo project: ${item.project.title}.${item.capabilities.length?` Useful capabilities: ${item.capabilities.join(', ')}.`:''} Commitment: ${commitment}.`;
 return <section className="section softSection"><div className="shell formShell"><div className="eyebrow">COLLABORATOR NEEDED</div><h1>{item.project.title}</h1><p className="lead">{item.project.summary}</p><article className="formCard" aria-labelledby="collaboration-need-heading"><h2 id="collaboration-need-heading">What this project needs</h2><dl><div><dt>Looking for</dt><dd>{role}</dd></div>{item.capabilities.length>0&&<div><dt>Useful capabilities</dt><dd>{item.capabilities.join(' · ')}</dd></div>}{item.domain&&<div><dt>Domain</dt><dd>{item.domain}</dd></div>}{item.need.experience_level&&<div><dt>Experience level</dt><dd>{item.need.experience_level}</dd></div>}<div><dt>Commitment</dt><dd>{commitment}</dd></div><div><dt>Project stage</dt><dd>{stage}</dd></div><div><dt>Places available</dt><dd>{item.openPlaces}</dd></div>{item.project.late_joining_cutoff_at&&<div><dt>Joining deadline</dt><dd>{new Date(item.project.late_joining_cutoff_at).toLocaleDateString('en-GB',{day:'numeric',month:'long',year:'numeric'})}</dd></div>}</dl>{item.need.member_message&&<p>{item.need.member_message}</p>}{item.accepting?inviteToken?user?<><div className="eyebrow">DIRECT INVITATION</div><p>This invitation must be accepted by the account it was sent to. Accepting only continues to Mettelo’s canonical project-interest process; it does not add you to the project automatically.</p><ExternalCollaborationInviteActions token={inviteToken} fallbackHref={publicUrl}/></>:<div className="actions"><a className="button dark" href={cta}>Sign in or create account to respond</a><a className="button ghost" href={publicUrl}>View without invitation</a></div>:<div className="actions"><a className="button dark" href={cta}>I’m interested</a><a className="button ghost" href={projectHref}>View project</a></div>:<div role="status"><strong>This collaboration opportunity is no longer open</strong><p>The team may be full, recruitment may have closed, or the project may have moved into a non-joinable state. This page remains available for safe context, but it cannot be used to join.</p><a className="button ghost" href="/member/collaboration?view=teams">Explore open teams</a></div>}</article>{item.accepting&&item.socialSharingEnabled&&<section aria-labelledby="share-collaboration-title"><div className="eyebrow">SHARE OPPORTUNITY</div><h2 id="share-collaboration-title">Know someone who may fit?</h2><p>Share the public collaboration page. Private project workspace information is never included.</p><CollaborationShareActions url={publicUrl} text={shareText} collaborationNeedId={id}/></section>}</div></section>;
}
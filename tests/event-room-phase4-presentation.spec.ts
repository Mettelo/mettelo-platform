...
async function injectPresentation(page:Page,participants=6,owner:PresentationOwner='remote'){
 await page.evaluate(({count,presentationOwner,longName})=>{
  ...
  const stage=document.createElement('div');stage.className='lk-focus-layout-wrapper';stage.dataset.phase4Fixture='stage';
  const sharedTile=document.createElement('div');sharedTile.className='lk-participant-tile';sharedTile.dataset.source='screen_share';sharedTile.dataset.owner=presentationOwner;
  const video=document.createElement('video');video.dataset.phase4Fixture='screen-share';video.setAttribute('aria-label',presentationOwner==='local'?'Your shared screen':'Shared screen from another participant');sharedTile.appendChild(video);stage.appendChild(sharedTile);
...
async function exitPresentation(page:Page,participants=6){
 await page.evaluate(({count,longName})=>{
  const inner=document.querySelector('.lk-video-conference-inner');if(!(inner instanceof HTMLElement))throw new Error('LiveKit conference inner surface not found.');inner.innerHTML='';
  const grid=document.createElement('div');grid.className='lk-grid-layout';grid.dataset.phase4Fixture='normal-grid';
  for(let index=0;index<count;index+=1){const tile=document.createElement('div');tile.className='lk-participant-tile';const name=document.createElement('span');name.className='lk-participant-name';name.textContent=index===count-1?longName:`Participant ${index+1}`;tile.appendChild(name);grid.appendChild(tile);}inner.appendChild(grid);
...
test('presentation rail contains dense participant content without expanding the Event Room',async({page})=>{test.slow();await openRoom(page,390,844);await injectPresentation(page,14,'remote');const shell=page.locator('[data-event-room-shell]');const rail=page.locator('[data-phase4-fixture="rail"]');await expect(shell).toBeVisible();await expect(rail).toBeVisible();const metrics=await page.evaluate(()=>{const tile=document.querySelector('[data-phase4-fixture="rail"] .lk-participant-tile:last-child') as HTMLElement;const name=document.querySelector('[data-phase4-fixture="rail"] .lk-participant-tile:last-child .lk-participant-name') as HTMLElement;return{rootClient:document.documentElement.clientWidth,rootScroll:document.documentElement.scrollWidth,shell:(document.querySelector('[data-event-room-shell]') as HTMLElement).getBoundingClientRect(),rail:(document.querySelector('[data-phase4-fixture="rail"]') as HTMLElement).getBoundingClientRect(),tile:tile.getBoundingClientRect(),tileOverflow:getComputedStyle(tile).overflowX,nameOverflow:getComputedStyle(name).overflowX,textOverflow:getComputedStyle(name).textOverflow,whiteSpace:getComputedStyle(name).whiteSpace};});expect(metrics.rootScroll).toBeLessThanOrEqual(metrics.rootClient+1);expect(metrics.rail.left).toBeGreaterThanOrEqual(metrics.shell.left-1);expect(metrics.rail.right).toBeLessThanOrEqual(metrics.shell.right+1);expect(metrics.tile.left).toBeGreaterThanOrEqual(metrics.rail.left-1);expect(metrics.tile.right).toBeLessThanOrEqual(metrics.rail.right+1);expect(metrics.tileOverflow).toBe('hidden');expect(metrics.nameOverflow).toBe('hidden');expect(metrics.textOverflow).toBe('ellipsis');expect(metrics.whiteSpace).toBe('nowrap');});

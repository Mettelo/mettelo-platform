...
type Run={id:string;status:string;run_number:number;scheduled_start_at:string|null;started_at:string|null};
...
function dateTimeLabel(value:string|null){return value?new Intl.DateTimeFormat('en-GB',{day:'numeric',month:'short',year:'numeric',hour:'2-digit',minute:'2-digit'}).format(new Date(value)):null}
    ...
    {showCurrent&&<>
      <section className={styles.section} aria-labelledby="current-work-title"><div className={styles.sectionHead}><div><div className={styles.eyebrow}>ONGOING WORK</div><h2 id="current-work-title">Continue where you left off</h2><p>Active work gets priority because it is the project work you can act on now.</p></div><span className={styles.count}>{filteredActive.length} active</span></div>
        {primary&&primaryProject?<div className={styles.activeGrid}>
          <article className={styles.activeCard}><div><span className={styles.status}>● Project started</span><h3>{primaryProject.title}</h3><p>{typeLabel(primaryProject)} · {one(primary.project_runs)?.run_number?`Your team: Team ${one(primary.project_runs)?.run_number} · `:''}Your role: {humaniseProjectValue(primary.team_role)}</p><div className={styles.meta}>{one(primary.project_runs)?.started_at&&<span>Started {dateTimeLabel(one(primary.project_runs)?.started_at||null)}</span>}<span>{completedTaskCount} of {primaryTasks.length} assigned tasks complete</span>{primaryNext?.blocker_reason&&<span>Blocker recorded</span>}{primaryNext?.due_at&&<span>Due {dateLabel(primaryNext.due_at)}</span>}</div><div className={styles.actions}><Link className={`${styles.button} ${styles.buttonDark}`} href={labHref(primary)}>Open Mettelo Lab →</Link></div></div><aside className={styles.upNext}><small>UP NEXT{primaryNext?.due_at?` · DUE ${dateLabel(primaryNext.due_at)?.toUpperCase()}`:''}</small><strong>{primaryNext?.title||'Continue in Mettelo Lab'}</strong><p>{primaryNext?.blocker_reason?`Blocker: ${primaryNext.blocker_reason}`:'Project execution and full task context remain inside Mettelo Lab.'}</p></aside></article>
          {filteredActive.slice(1).map(item=>{const project=one(item.projects);const run=one(item.project_runs);const itemTasks=tasksByProject.get(item.project_id)||[];const next=nextPortfolioTask(itemTasks);return <article className={styles.activeCardSecondary} key={item.id}><div><span className={styles.status}>● Project started</span><h3>{project?.title}</h3><p>{humaniseProjectValue(item.team_role)}{run?.run_number?` · Team ${run.run_number}`:''}</p>{run?.started_at&&<div className={styles.meta}><span>Started {dateTimeLabel(run.started_at)}</span></div>}{next&&<div className={styles.meta}><span>Next: {next.title}</span>{next.due_at&&<span>Due {dateLabel(next.due_at)}</span>}</div>}</div><div className={styles.actions}><Link className={`${styles.button} ${styles.buttonDark}`} href={labHref(item)}>Open Mettelo Lab →</Link></div></article>})}
      ...
      <section className={styles.section} aria-labelledby="preparing-title"><div className={styles.sectionHead}><div><div className={styles.eyebrow}>PREPARING TO START</div><h2 id="preparing-title">Your confirmed projects</h2><p>These projects are visible in your portfolio but are not yet ready for active delivery.</p></div><span className={styles.count}>{filteredPreparing.length} preparing</span></div>
        {filteredPreparing.length?<div className={styles.grid}>{filteredPreparing.map(item=>{const project=one(item.projects);const run=one(item.project_runs);const threshold=project?.team_size_threshold||0;const count=filled.get(item.project_id)||0;const scheduled=Boolean(run?.scheduled_start_at);return <article className={styles.card} key={item.id}><div><span className={`${styles.status} ${styles.forming}`}>{scheduled?'◷ Team ready · Start scheduled':'◷ Team forming'}</span><h3>{project?.title}</h3><p>{scheduled?'Your team has reached participation readiness. Mettelo will run the final project and system checks at the scheduled start time.':'Your place is confirmed. Mettelo is preparing the project team and delivery run before work begins.'}</p></div><div className={styles.formingNote} role="status"><strong>{scheduled?`Scheduled start: ${dateTimeLabel(run?.scheduled_start_at||null)}`:'No action needed right now'}</strong><small>{scheduled?'Mettelo Lab remains closed until final readiness passes and the canonical start succeeds.':'We’re completing your project setup. We’ll notify you when it’s ready.'}</small></div><div className={styles.facts}><div><small>Your role</small><strong>{humaniseProjectValue(item.team_role)}</strong></div><div><small>Your team</small><strong>{run?.run_number?`Team ${run.run_number}`:'Not assigned yet'}</strong></div>{threshold>0&&<div><small>Formation</small><strong>{Math.min(count,threshold)} of {threshold} places filled</strong></div>}</div><div className={styles.actions}><Link className={`${styles.button} ${styles.buttonDark}`} href="/member/applications">View status</Link></div></article>})}</div>:<div className={styles.empty}><h3>No projects are preparing to start.</h3><p>Confirmed projects that are still forming will appear here without being treated as errors or failed applications.</p></div>}
      </section>
    </>}

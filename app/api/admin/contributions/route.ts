...
export async function PATCH(request:Request){
  try{
    ...
    if(status!=='pending'){
      ...
      const outcome=status==='verified'?'Your contribution has been verified and can strengthen your Mettelo Proof.':status==='needs_changes'?`Your contribution needs changes before verification.${reviewNotes?` ${reviewNotes}`:''}`:`Your contribution was not verified.${reviewNotes?` ${reviewNotes}`:''}`;

...
type Offer={id:string;status:string;user_id:string;offered_at:string;expires_at:string;accepted_at:string|null;declined_at:string|null;expired_at:string|null;capacity_reserved_at:string;capacity_released_at:string|null};
...
export default async function AdminProjectOfferSummary({projectId,maximum}:Props){
  ...
  const [{data:offers,error},{count:members}]=await Promise.all([
    db.from('project_offers').select('id,status,user_id,offered_at,expires_at,accepted_at,declined_at,expired_at,capacity_reserved_at,capacity_released_at').eq('project_id',projectId).order('offered_at',{ascending:false}),
  ...
  const reserved=rows.filter(item=>['pending','accepted'].includes(item.status)&&!item.capacity_released_at).length;
  ...
  return <section className="apoPanel" aria-labelledby="admin-project-offers-title">
    <div className="apoHead"><div><span className="eyebrow">PROJECT OFFERS</span><h2 id="admin-project-offers-title">Offer & reservation state</h2><p>Pending and accepted Offers reserve project capacity until decline, expiry or later team formation consumes the accepted place.</p></div><a href={`/admin/project-operations/applications?project=${projectId}`}>Review applications →</a></div>
    ...
    <div className="apoLifecycle"><span>{pending} pending</span><span>{accepted} accepted</span><span>{declined} declined</span><span>{expired} expired</span></div>
    {rows.length?<div className="apoTableWrap"><table className="apoTable"><thead><tr><th>Member</th><th>Status</th><th>Offered</th><th>Expires / resolved</th><th>Reservation</th></tr></thead><tbody>{rows.slice(0,12).map(offer=>{const profile=profileMap.get(offer.user_id);const resolved=offer.accepted_at||offer.declined_at||offer.expired_at;return <tr key={offer.id}><td><strong>{profile?.full_name||'Mettelo member'}</strong>{profile?.username&&<small>@{profile.username}</small>}{profile?.member_id&&<small>{profile.member_id}</small>}</td><td><span className={`apoStatus apo-${offer.status}`}>{offer.status}</span></td><td>{dateTime(offer.offered_at)}</td><td>{resolved?dateTime(resolved):dateTime(offer.expires_at)}</td><td>{offer.capacity_released_at?'Released':'Reserved'}</td></tr>})}</tbody></table></div>:<p className="apoEmpty">No project place has been offered yet.</p>}

...
test('Discover and member project detail preserve the approved responsive internal journey',async({page})=>{
  ...
  for(const width of widths){
    ...
    await page.goto(`/member/discover/${projectId}`,{waitUntil:'networkidle'});await expect(page.getByRole('heading',{level:1,name:title})).toBeVisible();await expect(page.getByText('MEMBER PROJECT DETAIL',{exact:true})).toBeVisible();await expect(page.getByRole('navigation',{name:'Project breadcrumb'})).toContainText(`My Mettelo/Discover/${title}`);await expect(page.getByText('Data Analyst',{exact:true}).first()).toBeVisible();const apply=page.getByRole('link',{name:'Apply now'}).first();await expect(apply).toHaveAttribute('href',`/member/discover/${projectId}/apply`);await expect(page.getByRole('link',{name:/Apply as /})).toHaveCount(0);await expect(page.getByRole('button',{name:/Select this role|Selected/})).toHaveCount(0);const detailBody=page.locator('#member-project-main');await expect(detailBody).toBeVisible();const detailBox=await detailBody.boundingBox();expect(detailBox,`${width}px V3 detail body`).not.toBeNull();if(detailBox)expect(detailBox.width).toBeLessThanOrEqual(width+1);await noOverflow(page,`${width}px detail`);
  }
});
...
test('signed-out project catalogue and detail remain public, then Submit interest preserves intent into the member flow',async({page})=>{
  test.setTimeout(120_000);await page.goto('/projects',{waitUntil:'networkidle'});expect(new URL(page.url()).pathname).toBe('/projects');
  await page.goto(`/projects/${projectId}#apply`,{waitUntil:'networkidle'});expect(new URL(page.url()).pathname).toBe(`/projects/${projectId}`);const signInLink=page.getByRole('link',{name:'Submit interest'}).first();await expect(signInLink).toBeVisible();await signInLink.click();await page.waitForURL(url=>url.pathname==='/signin'&&url.searchParams.get('next')===`/member/discover/${projectId}`,{timeout:20_000});const account=credentials();const main=page.locator('#main-content');await main.locator('input[type="email"]').fill(account.email);await main.locator('input[type="password"]').fill(account.password);await main.getByRole('button',{name:'Sign in →'}).click();await page.waitForURL(url=>url.pathname===`/member/discover/${projectId}`,{timeout:20_000});await expect(page.getByText('MEMBER PROJECT DETAIL',{exact:true})).toBeVisible();const apply=page.getByRole('link',{name:'Apply now'}).first();await expect(apply).toHaveAttribute('href',`/member/discover/${projectId}/apply`);await apply.click();await page.waitForURL(url=>url.pathname===`/member/discover/${projectId}/apply`&&!url.searchParams.has('role'),{timeout:20_000});await expect(page.getByRole('heading',{level:1,name:`Apply to ${title}`})).toBeVisible();await expect(page.locator(`input[type="radio"][value="${roleId}"]`)).not.toBeChecked();
});
...
test('internal application keeps responses between steps and Save does not create an application',async({page})=>{
  ...
  await page.goto(`/member/discover/${projectId}`,{waitUntil:'networkidle'});await page.getByRole('link',{name:'Apply now'}).first().click();await page.waitForURL(url=>url.pathname===`/member/discover/${projectId}/apply`&&!url.searchParams.has('role'),{timeout:20_000});await expect(page.getByRole('heading',{level:1,name:`Apply to ${title}`})).toBeVisible();for(const label of ['Role & fit','Availability','How you could contribute','Review'])await expect(page.getByText(label,{exact:true}).first()).toBeVisible();const role=page.locator(`input[type="radio"][value="${roleId}"]`);await expect(role).not.toBeChecked();await role.check();
...
test('profile editing remains available below readiness and saving the final requirement immediately unlocks application',async({page})=>{
  ...
  await page.goto(`/member/discover/${projectId}`,{waitUntil:'networkidle'});await expect(page.getByRole('link',{name:'Apply now'}).first()).toBeVisible();await expect(page.getByRole('link',{name:/Apply as /})).toHaveCount(0);await noOverflow(page,'390px profile-save-unlock');
});

...
const checks=[
  ...
  ['Home uses the canonical project-run membership relationship',home.includes('project_runs:project_runs!project_members_project_run_id_fkey(status,run_number)')],
  ['Home never turns a failed project-membership query into a truthful-looking zero',home.includes('membershipLoadFailed')&&home.includes('Project status temporarily unavailable')&&home.includes('Home is deliberately not showing “0 active”')],
  ['Up Next still derives from member state',home.includes('const upNext=nextTask?')&&home.includes('activeProject&&activeProjectData')&&home.includes('pendingSpotlight')&&home.includes('profileReady')&&home.includes('recommendationCount')],
  ...
  ['approved command-centre Up Next hierarchy is present',css.includes('border-radius:22px')&&css.includes('font:780 21px/1.18')&&css.includes('radial-gradient')&&css.includes('border-radius:20px')],
  ['four-column metric rail collapses responsively',css.includes('grid-template-columns:repeat(4,minmax(0,1fr))')&&css.includes('border-radius:16px')&&css.includes('grid-template-columns:repeat(2,minmax(0,1fr))')],
  ['desktop content uses a work-first senior split',css.includes('grid-template-columns:minmax(0,1.7fr) minmax(300px,.8fr)')],

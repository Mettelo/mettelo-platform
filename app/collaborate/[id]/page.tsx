...
async function loadOpportunity(id:string){
 ...
 const [{data:project},{data:run},{data:links},capacityResult]=await Promise.all([
  db.from('projects').select('id,title,summary,status,visibility,project_type,weekly_commitment,late_joining_enabled,late_joining_cutoff_at,collaboration_marketplace_enabled,collaboration_social_sharing_enabled').eq('id',row.project_id).maybeSingle(),

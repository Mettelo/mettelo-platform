...
import {CANONICAL_CAREER_ROLES} from '@/lib/project-catalogue-taxonomy';
...
export default function AdminProjectRoleManager({projects}:{projects:Project[]}){
  ...
  async function submit(event:FormEvent<HTMLFormElement>){
    ...
    try{
      ...
      setStatus('success');setMessage('Project role saved with its canonical Career / Role classification.');
      form.reset();
  ...
  return <section className="panel" id="project-roles">
    ...
    {projects.length?<form className="formCard" onSubmit={submit}>
      ...
      <div className="fieldRow"><div><label htmlFor="role-career">Career / Role classification *</label><select id="role-career" name="career_role" required defaultValue=""><option value="" disabled>Select canonical career</option>{CANONICAL_CAREER_ROLES.map(role=><option key={role.slug} value={role.slug}>{role.label}</option>)}</select></div><div><label htmlFor="role-title">Project role title *</label><input id="role-title" name="title" required placeholder="Agricultural/Extension Analyst"/></div></div>
      <p className="fieldHint">Career / Role powers discovery. The project role title can stay specific to the work without creating a new public career filter.</p>
      <label htmlFor="role-discipline">Discipline</label><input id="role-discipline" name="discipline" placeholder="Data Analysis"/>

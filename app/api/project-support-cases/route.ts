...
export async function POST(request:Request){
 try{
  ...
  try{await notifyAdmins(db,{projectId,type:'project_support_case',eventKey:'project_support_case',title:'A private project support case needs review',body:'A project member submitted a private support case. Open Mettelo Admin to review it securely.',actionUrl:`/admin/project-support?case=${encodeURIComponent(created.id)}`,dedupeKey:`phase17:support:${created.id}:created`});}
  catch(notificationError){console.error('project support notification error',{code:safeErrorCode(notificationError)})}
...
export async function PATCH(request:Request){
 try{
  ...
  const {data:current,error:loadError}=await db.from('project_support_cases').select('id,project_id,project_run_id,reporter_user_id,status,updated_at').eq('id',caseId).eq('reporter_user_id',user.id).maybeSingle();
  ...
  const {data:updated,error:updateError}=await db.from('project_support_cases').update({status:'under_review'}).eq('id',caseId).eq('reporter_user_id',user.id).eq('status','awaiting_member').eq('updated_at',current.updated_at).select('id,status,updated_at').maybeSingle();
  if(updateError)throw updateError;
  ...
  const {error:eventError}=await db.from('project_support_case_updates').insert({case_id:caseId,actor_user_id:user.id,action:'member_update',body:responseText,member_visible:true,metadata:{from_status:'awaiting_member',to_status:'under_review',reporter_response:true}});
  if(eventError)throw eventError;
  const {error:auditError}=await db.from('project_activity_log').insert({project_id:current.project_id,project_run_id:current.project_run_id,event_type:'support_case_member_update',actor_type:'user',actor_user_id:user.id,from_status:'awaiting_member',to_status:'under_review',metadata:{support_case_id:caseId,member_response_received:true}});
  if(auditError)throw auditError;

  try{await notifyAdmins(db,{projectId:current.project_id,type:'project_support_case_update',eventKey:'project_support_case',title:'A private support case has a member response',body:'A member responded to a secure support information request. Open Mettelo Admin to review it.',actionUrl:`/admin/project-support?case=${encodeURIComponent(caseId)}`,dedupeKey:`phase17:support:${caseId}:member-response:${updated.updated_at}`});}
  catch(notificationError){console.error('project support response notification error',{code:safeErrorCode(notificationError)})}

  return NextResponse.json({ok:true,case:updated},{headers:{'Cache-Control':'private, no-store'}});
 }catch(error){
  console.error('project support case response error',{code:safeErrorCode(error)});

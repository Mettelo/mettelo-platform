...
export default async function MemberProjectDetailPage({params}:{params:Promise<{id:string}>}){
  ...
  const [projectResult,applicationsResult,membershipResult,savedResult,profileResult,domainPrefs,toolPrefs,detail]=await Promise.all([
    supabase.from('projects').select('id,title,summary,problem_statement,status,project_type,visibility,location,location_type,duration_weeks,weekly_commitment,application_deadline,applications_open,starts_at,ends_at,project_roles(id,title,description,skills,openings),project_domains(domains(slug,name)),project_tools(tools(slug,name)),project_methods(methods(slug,name))').eq('id',id).in('visibility',['public','members']).maybeSingle(),

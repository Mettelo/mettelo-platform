...
type HomeUpdate={kind:'UPDATE'|'CONSENT';title:string;text:string;href:string;symbol:string};
...
export default async function MemberHome(){
  ...
  if(latestApplication&&latestApplicationMeta&&!latestApplicationMeta.actionRequired)updates.push({kind:'UPDATE',title:one(latestApplication.projects)?.title||'Project application',text:latestApplicationMeta.meaning,href:'/member/applications',symbol:'A'});
  ...
  const latestApplicationHomeLabel=latestApplication&&['rejected','not_selected'].includes(latestApplication.status)?'Not progressed':latestApplicationMeta?.label||'';
  const latestApplicationCanExplore=Boolean(latestApplication&&['rejected','not_selected'].includes(latestApplication.status));
  ...
  return <section className={`${styles.home} memberWorkspace`} aria-labelledby="member-home-title"><div className={styles.wrap}>
    ...
    <section className={styles.overview} aria-label="Member overview">{overview.map(item=><Link href={item.href} key={item.label} aria-label={`${item.value} ${item.value===1?item.label:item.plural}. ${item.helper}`}><span className={styles.metricTop}><strong>{item.value}</strong><b aria-hidden="true">→</b></span><span>{item.value===1?item.label:item.plural}</span><small>{item.helper}</small></Link>)}</section>
    ...
    <div className={styles.grid}>
      ...
      <aside className={styles.stack}>
        <section className={styles.panel} aria-labelledby="application-heading"><div className={styles.panelHead}><div><div className={styles.eyebrow}>APPLICATIONS</div><h2 id="application-heading">Latest status</h2></div><Link className={styles.panelLink} href="/member/applications">View all →</Link></div>{latestApplication&&latestApplicationMeta?<article className={styles.application}><span className={styles.applicationState}>{latestApplicationHomeLabel.toUpperCase()}</span><h3>{one(latestApplication.projects)?.title||'Project application'}</h3><p>{latestApplicationCanExplore?'This application will not move forward. You can review the details or explore similar work when you are ready.':latestApplicationMeta.meaning}</p><div className={styles.actions}><Link className={styles.button} href="/member/applications">View application</Link>{latestApplicationCanExplore&&<Link className={`${styles.button} ${styles.buttonDark}`} href={recommendationCount?'/member/recommended':'/member/discover'}>Explore similar</Link>}</div></article>:<div className={`${styles.empty} ${styles.emptyCompact}`}><h3>No applications yet.</h3><p>Discover a project and apply when the fit is right.</p><Link href="/member/discover">Discover projects →</Link></div>}</section>

        <section className={`${styles.valueCard} ${styles.proof}`} aria-labelledby="proof-heading"><div className={styles.eyebrow}>PROOF</div><h2 id="proof-heading">Evidence that travels with you</h2><div className={styles.proofValue}>{proofResult.count||0} <span>verified</span></div><p>Verified project contributions become a reusable record of what you have actually done.</p>{!(proofResult.count||0)&&activeMemberships.length>0&&<div className={styles.proofHint}>Your active project can become verified Proof as reviewed contributions are completed.</div>}<Link className={styles.button} href="/member/proof">Open Proof →</Link></section>

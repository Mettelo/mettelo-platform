  ...
  return <div className={styles.body} id="member-project-main">
    ...
    <section className={styles.section} id="overview" aria-labelledby="member-overview-title"><div className={styles.sectionHead}><div><span className={styles.eyebrow}>01 · Overview</span><h2 id="member-overview-title">Project overview</h2></div><p className={styles.sectionIntro}>The essential project context for making an application decision, with deeper specification available when needed.</p></div>
      ...
      <div className={styles.insights}>
        ...
        <article className={styles.insight}><span>Project outcome</span><h3>{short(challenge.primaryObjective||challenge.useCase,140)||'A decision-ready project outcome'}</h3><p>The outcome the team must deliver so the stakeholder can act responsibly.</p></article>
      </div>
    ...
    <section className={styles.section} id="deliverables" aria-labelledby="member-deliverables-title"><div className={styles.sectionHead}><div><span className={styles.eyebrow}>02 · Delivery</span><h2 id="member-deliverables-title">Project deliverables</h2></div><p className={styles.sectionIntro}>The primary outputs are easy to scan. Full acceptance evidence remains available on demand.</p></div>
    ...
    <section className={styles.section} id="quality" aria-labelledby="member-quality-title"><div className={styles.sectionHead}><div><span className={styles.eyebrow}>03 · Quality</span><h2 id="member-quality-title">Success standards</h2></div><p className={styles.sectionIntro}>Project-specific criteria are grouped for quick review while the complete quality bar stays available below.</p></div>
    ...
    <section className={styles.section} id="roles" aria-labelledby="member-roles-title"><div className={styles.sectionHead}><div><span className={styles.eyebrow}>04 · Your contribution</span><h2 id="member-roles-title">Choose your contribution area</h2></div><p className={styles.sectionIntro}>Role availability comes from the project backend. Choose explicitly before continuing to the application form.</p></div>
      ...
      {roles.length?<div className={styles.roles} role="group" aria-label="Available contribution areas">{roles.map(role=>{const selectable=role.available&&((role.remaining??role.openings)>0);const selected=selectable&&role.id===selectedRoleId;const mission=short(role.responsibilities[0]||role.description||`${role.title} owns a defined project workstream and the evidence produced from it.`,155);return <article key={role.id} className={`${styles.role} ${selected?styles.roleSelected:''} ${!selectable?styles.roleUnavailable:''}`}><div className={styles.roleTop}><h3>{role.title}</h3><span className={styles.capacity}>{selectable?`${role.remaining??role.openings} place${(role.remaining??role.openings)===1?'':'s'} available`:'At capacity'}</span></div><p>{mission}</p>{role.skills.length>0&&<div className={styles.chips}>{role.skills.slice(0,4).map(skill=><span key={skill}>{skill}</span>)}</div>}{selectable&&<button type="button" className={`${styles.roleSelectButton} ${selected?styles.roleSelectButtonSelected:''}`} aria-pressed={selected} onClick={()=>setSelectedRoleId(role.id)}>{selected?'Selected':'Choose this role'}</button>}{(role.responsibilities.length>1||role.recommendedSkills.length>0||role.experienceExpectation||role.weeklyCommitment||role.applicationRequirements)&&<details><summary>Role details</summary><div>{role.responsibilities.length>0&&<ul>{role.responsibilities.slice(0,5).map(item=><li key={item}>{item}</li>)}</ul>}{role.experienceExpectation&&<p>{role.experienceExpectation}</p>}{role.weeklyCommitment&&<p><strong>Commitment:</strong> {role.weeklyCommitment}</p>}{role.recommendedSkills.length>0&&<><h4>Recommended skills</h4><div className={styles.chips}>{role.recommendedSkills.slice(0,8).map(skill=><span key={skill}>{skill}</span>)}</div></>}{role.applicationRequirements&&<p><strong>Application:</strong> {role.applicationRequirements}</p>}</div></details>}</article>})}</div>:contributionAreas.length?<div className={styles.roles}>{contributionAreas.map(area=><article className={styles.role} key={area.slug}><div className={styles.roleTop}><h3>{area.title}</h3><span className={styles.capacity}>Contribution area</span></div><p>{short(area.description||'A project-specific contribution area used to organise ownership and delivery.',155)}</p></article>)}</div>:<div className={styles.empty} role="status">Contribution areas are still being prepared.</div>}
      <div className={styles.applyBand}><div><span className={styles.eyebrow}>Selected contribution area</span><h3>{selectedRole?.title||'Choose an available contribution area'}</h3><p>{selectedRole?'Review the role expectations, confirm your availability and continue to the application form.':'Select one available role before continuing.'}</p></div>{canApply&&selectedRole?<Link className={styles.primaryButton} href={`/member/discover/${projectId}/apply?role=${encodeURIComponent(selectedRole.id)}`}>Apply as {selectedRole.title}</Link>:<span className={styles.selectionHint}>Selection required</span>}</div>
    </section>

    <section className={styles.section} id="timeline" aria-labelledby="member-timeline-title"><div className={styles.sectionHead}><div><span className={styles.eyebrow}>05 · Journey</span><h2 id="member-timeline-title">Project delivery plan</h2></div><p className={styles.sectionIntro}>A compact sequence shows what happens after application while Mettelo Lab remains the authorised delivery workspace.</p></div>
      {timeline.length?<div className={styles.timeline}>{timeline.slice(0,6).map((item,index)=><div key={item.id}><span aria-hidden="true">{String(index+1).padStart(2,'0')}</span><div><strong>{item.title}</strong>{item.description&&<p>{short(item.description,180)}</p>}</div></div>)}</div>:<div className={styles.timeline}><div><span aria-hidden="true">01</span><div><strong>Apply</strong><p>Choose a contribution area and submit your application.</p></div></div><div><span aria-hidden="true">02</span><div><strong>Team formation</strong><p>Accepted members are assigned to the project workspace.</p></div></div><div><span aria-hidden="true">03</span><div><strong>Project delivery</strong><p>Work through milestones, reviews and assigned deliverables.</p></div></div><div><span aria-hidden="true">04</span><div><strong>Evidence review</strong><p>Your contribution is reviewed against project standards.</p></div></div><div><span aria-hidden="true">05</span><div><strong>Handover</strong><p>Complete the final recommendation, evidence pack and professional handover.</p></div></div></div>}
    </section>

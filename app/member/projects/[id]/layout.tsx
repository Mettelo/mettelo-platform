...
export default async function ProjectWorkspaceGate({children,params}:{children:React.ReactNode;params:Promise<{id:string}>}){
 const {id}=await params;const supabase=await createServerSupabaseClient();const {data:{user}}=await supabase.auth.getUser();if(!user)redirect(`/signin?next=${encodeURIComponent(`/member/projects/${id}`)}`);const isAdmin=user.app_metadata?.role==='admin';const service=serviceDb();const gateDb=isAdmin?(service||supabase):supabase;
 const [{data:project},{data:membershipResult}]=await Promise.all([gateDb.from('projects').select('id,title,summary,status').eq('id',id).maybeSingle(),supabase.from('project_members').select('membership_status,project_run_id,team_role').eq('project_id',id).eq('user_id',user.id).in('membership_status',['waiting','active','completed']).order('joined_at',{ascending:false}).limit(1).maybeSingle()]);const membership=membershipResult as Membership|null;if(!project||(!membership&&!isAdmin))notFound();
 let runId=membership?.project_run_id||null;let runStatus:string|null=null;
 if(!isAdmin&&membership&&['active','completed'].includes(membership.membership_status)&&runId){
  const {data:hasLabAccess,error:labAccessError}=await supabase.rpc('phase12_has_lab_access',{p_project_id:id,p_run_id:runId});
  if(labAccessError||hasLabAccess!==true)redirect('/member#applications');
  const {data:memberRun}=await (service||supabase).from('project_runs').select('status').eq('id',runId).eq('project_id',id).maybeSingle();runStatus=memberRun?.status||null;
 }
 ...
 if(!runId&&isAdmin){const {data:latestRun}=await gateDb.from('project_runs').select('id,status').eq('project_id',id).in('status',['active','review','completed']).order('run_number',{ascending:false}).limit(1).maybeSingle();runId=latestRun?.id||null;runStatus=latestRun?.status||null}else if(isAdmin&&runId){const {data:memberRun}=await gateDb.from('project_runs').select('status').eq('id',runId).eq('project_id',id).maybeSingle();runStatus=memberRun?.status||null}
 if(!isAdmin&&runId){const {error:labOpenError}=await supabase.rpc('phase12_record_lab_open',{p_project_id:id,p_run_id:runId});if(labOpenError)console.error('lab open analytics error',labOpenError)}
 let teamRows:TeamRow[]=[],dataSources:DataSourceRow[]=[],dataVersions:DataVersionRow[]=[],conversationRows:ConversationRow[]=[];let architectUserId:string|null=null,lastReadAt:string|null=null;let readiness:Readiness=emptyReadiness;let completionRequest:CompletionRequest|null=null;const deliveryMembershipStates=runStatus==='completed'?['active','completed']:['active'];
 if(runId){const [teamResult,dataResult,versionResult,architectResult,conversationResult,readResult,readinessResult,completionResult]=await Promise.all([
  supabase.from('project_members').select('user_id,team_role,membership_status').eq('project_id',id).eq('project_run_id',runId).in('membership_status',deliveryMembershipStates).order('joined_at'),

...
export default function AdminApplicationQueue({initialItems}:{initialItems:Item[]}){
  ...
  return <div className="adminApplications">
    ...
    {detail&&<div className="modalBackdrop" onMouseDown={event=>{if(event.target===event.currentTarget)setDetail(null)}}><section className="detailModal" role="dialog" aria-modal="true" aria-labelledby="application-detail-heading"><div className="modalHead"><div><span className="eyebrow">{partner(detail)?'PARTNER PROJECT REVIEW':detail.admission_lane==='auto'?'OPEN PROJECT · AUTO OVERSIGHT':'OPEN PROJECT REVIEW'}</span><h2 id="application-detail-heading">{detail.name}</h2><p>{detail.username?`@${detail.username} · `:''}{detail.member_id?`${detail.member_id} · `:''}{detail.project}{partner(detail)&&detail.partner_name?` · ${detail.partner_name}`:''}</p></div><button type="button" aria-label={`Close ${requestNoun(detail)} detail`} onClick={()=>setDetail(null)}>×</button></div>
      ...
      {nextActions(detail).length>0&&<div className="detailActions">{nextActions(detail).map(status=><button className={status==='declined'?'button ghost dangerAction':'button dark'} type="button" key={status} disabled={working} onClick={()=>status==='in_review'&&detail.status==='submitted'?void startReview(detail):(()=>{setDetail(null);openAction(status,[detail.id])})()}>{actionLabel(status,detail.status)}</button>)}</div>}
    </section></div>}

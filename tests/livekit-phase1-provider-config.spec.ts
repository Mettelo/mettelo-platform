...
test('missing LiveKit server configuration fails closed with a clear provider error',async({page})=>{
 ...
 const response=await page.context().request.post(`/api/project-events/${body.event_id}/token`);expect(response.status()).toBe(503);expect(await response.json()).toEqual({error:'Live video is awaiting provider configuration.'});
});

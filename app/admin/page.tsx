...
type ContributionRow={id:string;user_id:string;title:string;contribution_type:string;description:string|null;evidence_url:string|null;verification_status:string;review_notes:string|null;created_at:string;is_public:boolean;projects:{title:string}|null};
...
export default async function AdminDashboard(){
  ...
  let counts={members:0,projects:0,opportunities:0,events:0,submissions:0,applications:0,proof:0};
  ...
  let contributionItems:{id:string;name:string;project:string;title:string;type:string;description:string|null;evidence_url:string|null;status:string;review_notes:string|null;created_at:string;is_public:boolean}[]=[];

  if(url&&serviceKey){
    const db=createClient(url,serviceKey,{auth:{persistSession:false,autoRefreshToken:false}});
    const [members,projects,opportunities,events,submissions,applications,proof,projectRows,allProjectRows,submissionRows,applicationRows,contributionRows]=await Promise.all([
      ...
      db.from('contributions').select('id',{count:'exact',head:true}).in('verification_status',['pending','needs_changes']),
      ...
      db.from('contributions').select('id,user_id,title,contribution_type,description,evidence_url,verification_status,review_notes,created_at,is_public,projects(title)').in('verification_status',['pending','needs_changes']).order('created_at',{ascending:true}).limit(30)
    ]);
    counts={members:members.count||0,projects:projects.count||0,opportunities:opportunities.count||0,events:events.count||0,submissions:submissions.count||0,applications:applications.count||0,proof:proof.count||0};
    recentProjects=projectRows.data||[];allProjects=allProjectRows.data||[];recentSubmissions=submissionRows.data||[];
    const appRows=(applicationRows.data||[]) as unknown as ApplicationRow[];
    const proofRows=(contributionRows.data||[]) as unknown as ContributionRow[];
    const ids=[...new Set([...appRows.map(row=>row.user_id),...proofRows.map(row=>row.user_id)])];
    ...
    applicationItems=appRows.map(row=>({id:row.id,name:profileMap.get(row.user_id)?.full_name||'Mettelo member',email:emailMap.get(row.user_id)||'',project:row.projects?.title||'Project',role:row.project_roles?.title||'Role',status:row.status,submitted_at:row.submitted_at,statement:row.contribution_statement,portfolio_url:row.portfolio_url,availability:row.availability}));
    contributionItems=proofRows.map(row=>({id:row.id,name:profileMap.get(row.user_id)?.full_name||'Mettelo member',project:row.projects?.title||'Mettelo project',title:row.title,type:row.contribution_type,description:row.description,evidence_url:row.evidence_url,status:row.verification_status,review_notes:row.review_notes,created_at:row.created_at,is_public:row.is_public}));
  }

  return <section className="section softSection"><div className="shell dashboard">
    <aside className="sidebar"><div className="sidebarTop"><h3>Mettelo Admin</h3><small>{user.email}</small></div><a className="active" href="#overview">Overview <span>⌂</span></a><a href="#publisher">Publish <span>＋</span></a><a href="#project-roles">Project roles <span>＋</span></a><a href="#applications">Applications <span>{counts.applications}</span></a><a href="#proof-review">Proof review <span>{counts.proof}</span></a><a href="#projects">Projects <span>{counts.projects}</span></a><a href="#submissions">Intake <span>{counts.submissions}</span></a></aside>
    <main className="dashboardMain" id="overview"><div className="eyebrow">Operations console</div><h1>Run Mettelo from production records.</h1><p className="lead">Publish Labs briefs, define roles, review applications, build project teams and verify contribution evidence from one protected workspace.</p>
      <div className="metricGrid"><div className="metric"><strong>{counts.members}</strong><span>Member profiles</span></div><div className="metric"><strong>{counts.projects}</strong><span>Projects</span></div><div className="metric"><strong>{counts.applications}</strong><span>Active applications</span></div><div className="metric"><strong>{counts.proof}</strong><span>Proof awaiting review</span></div></div>
      <AdminContentManager/>
      ...
      <section className="panel" id="proof-review" style={{marginTop:18}}><div className="panelHead"><div><span className="cardNumber">PROOF VERIFICATION</span><h3 style={{marginTop:8}}>Contribution evidence review</h3></div><span className="chip">{counts.proof} WAITING</span></div><AdminContributionQueue initialItems={contributionItems}/></section>

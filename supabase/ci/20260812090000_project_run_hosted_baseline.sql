...
alter table public.project_discussions
  add column if not exists project_run_id uuid references public.project_runs(id) on delete cascade;
...
create index if not exists idx_project_members_run_user
  on public.project_members(project_run_id,user_id) where project_run_id is not null;
create index if not exists idx_project_discussions_run
  on public.project_discussions(project_run_id) where project_run_id is not null;
create index if not exists idx_project_resources_run
  on public.project_resources(project_run_id) where project_run_id is not null;
create index if not exists idx_project_meetings_run
  on public.project_meetings(project_run_id) where project_run_id is not null;
create index if not exists idx_project_milestones_run
  on public.project_milestones(project_run_id) where project_run_id is not null;
create index if not exists idx_project_tasks_run
  on public.project_tasks(project_run_id) where project_run_id is not null;
create index if not exists idx_project_presentations_run
  on public.project_presentations(project_run_id) where project_run_id is not null;
create index if not exists idx_contributions_run
  on public.contributions(project_run_id) where project_run_id is not null;
...
begin
  if actor is not null then
    ...
    where pm.project_id=target_project
      ...
      and pm.membership_status in ('waiting','active','completed')
    order by case pm.membership_status when 'active' then 1 when 'waiting' then 2 else 3 end,
             pr.run_number desc
    limit 1;
  end if;
  if result is null then
    ...
    order by case status when 'forming' then 1 when 'active' then 2 else 3 end,
             run_number desc
    limit 1;
  end if;
  return result;
...
begin
  if new.project_run_id is null and new.project_id is not null then
    if tg_table_name='project_discussions' then
      new.project_run_id:=resolve_project_run(new.project_id,new.author_user_id);
    elsif tg_table_name='project_resources' then
      new.project_run_id:=resolve_project_run(new.project_id,new.added_by);
    elsif tg_table_name='project_meetings' then
      new.project_run_id:=resolve_project_run(new.project_id,new.organiser_user_id);
    elsif tg_table_name='contributions' then
      new.project_run_id:=resolve_project_run(new.project_id,new.user_id);
    else
      new.project_run_id:=resolve_project_run(new.project_id,auth.uid());
    end if;
  end if;
  return new;
end
...
begin
  if old.project_type='open' and new.status in ('active','review','completed') then
    ...
    order by case status when 'review' then 1 when 'active' then 2 else 3 end,
             run_number desc
    limit 1;
    ...
    if new.status='completed' then
      select coalesce(max(run_number),0)+1 into next_no
      from public.project_runs where project_id=old.id;

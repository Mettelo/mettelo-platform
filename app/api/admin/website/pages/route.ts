...
export async function GET(request:Request){
 ...
 const [{data:draft,error:draftError},{data:published,error:publishedError}]=await Promise.all([
  ctx.db.from('website_page_drafts').select('page_key,payload,updated_at,updated_by').eq('page_key',ctx.page).maybeSingle(),
...
export async function PATCH(request:Request){
 try{
  ...
  const {data,error}=await ctx.db.from('website_page_drafts').upsert({page_key:ctx.page,payload:validated.payload,updated_at:updatedAt,updated_by:ctx.user.id},{onConflict:'page_key'}).select('page_key,payload,updated_at,updated_by').single();
  if(error)throw error;
  const audit=await recordAdminAudit({actorUserId:ctx.user.id,actorEmail:ctx.user.email,capability:'website.content.edit',action:'website.page.draft.updated',resourceType:'website.page',resourceId:ctx.page,beforeState:before?.payload||null,afterState:validated.payload,metadata:{page:ctx.page}});
...
export async function POST(request:Request){
 try{
  ...
  const [{data:draft,error:draftError},{data:before}]=await Promise.all([
   ctx.db.from('website_page_drafts').select('payload').eq('page_key',ctx.page).maybeSingle(),
  ...
  const {data,error}=await ctx.db.from('website_page_public').upsert({page_key:ctx.page,payload:validated.payload,published_at:publishedAt,published_by:ctx.user.id},{onConflict:'page_key'}).select('page_key,payload,published_at,published_by').single();
  if(error)throw error;
  const audit=await recordAdminAudit({actorUserId:ctx.user.id,actorEmail:ctx.user.email,capability:'website.content.publish',action:'website.page.published',resourceType:'website.page',resourceId:ctx.page,beforeState:before?.payload||null,afterState:validated.payload,metadata:{page:ctx.page}});
  return NextResponse.json({ok:true,item:data,audit_recorded:audit.ok});

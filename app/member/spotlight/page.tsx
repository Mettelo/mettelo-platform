...
export default async function MemberSpotlightPage(){
  ...
  return <section className="memberSpotlightPage" aria-labelledby="member-spotlight-title">
    ...
    <aside className="memberSpotlightPrinciple"><div><p className="cardNumber">HOW SPOTLIGHT WORKS</p><h2><span>System selects.</span> <span>Admin safeguards.</span> <span>You consent.</span></h2></div><p>You do not nominate yourself and Admin does not routinely pick winners. Verified contribution is required for automatic recognition. Admin can only govern exceptions, and no personally identifying Spotlight is public without your permission.</p></aside>
    <style>{`
      .memberSpotlightPage{display:grid;gap:24px;padding:4px 0 36px}.memberSpotlightHero{display:flex;justify-content:space-between;gap:30px;align-items:end;padding:28px 0 8px;border-bottom:1px solid #e7e1d6}.memberSpotlightHero h1{margin:7px 0 8px;font-size:clamp(2.6rem,5vw,4.6rem);line-height:1;letter-spacing:-.045em}.memberSpotlightHero p{max-width:780px;margin:0;color:#596473;line-height:1.65}.memberSpotlightHeroActions{display:flex;gap:8px;flex:0 0 auto}.memberSpotlightPrinciple{display:grid;grid-template-columns:minmax(240px,.7fr) minmax(0,1.3fr);gap:24px;padding:22px;border:1px solid #ddd6ca;border-radius:18px;background:#fbf7ee}.memberSpotlightPrinciple h2{display:flex;flex-wrap:wrap;column-gap:.28em;row-gap:.05em;margin:6px 0 0;font-size:1.35rem}.memberSpotlightPrinciple>p{margin:0;line-height:1.65}.memberSpotlightPage :focus-visible{outline:3px solid #173f8f;outline-offset:3px}

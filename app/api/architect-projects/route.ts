...
export async function POST(request:Request){
  ...
  try{
    ...
    if(capabilityInputs.length){const capabilityIds=capabilityInputs.map(item=>item.capability_id);const {data:validCapabilities,error:capabilityError}=await db.from('capabilities').select('id').in('id',capabilityIds).eq('is_active',true);if(capabilityError)throw capabilityError;if((validCapabilities||[]).length!==capabilityIds.length)return NextResponse.json({error:'Choose only active governed capabilities for this project.'},{status:400})}
    ...
    if(capabilityInputs.length){const rows=capabilityInputs.map(item=>({project_id:project.id,capability_id:item.capability_id,importance:item.importance,evidence_expected:item.evidence_expected}));const {error:projectCapabilityError}=await db.from('project_capabilities').insert(rows);if(projectCapabilityError)throw projectCapabilityError}

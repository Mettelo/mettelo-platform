...
test('Discover and member project detail preserve the approved responsive internal journey',async({page})=>{
  ...
  for(const width of widths){
    ...
    await page.goto(`/member/discover/${projectId}`,{waitUntil:'networkidle'});await expect(page.getByRole('heading',{level:1,name:title})).toBeVisible();await expect(page.getByText('MEMBER PROJECT DETAIL',{exact:true})).toBeVisible();await expect(page.getByRole('navigation',{name:'Project breadcrumb'})).toContainText(`My Mettelo/Discover/${title}`);await expect(page.getByText('Data Analyst',{exact:true}).first()).toBeVisible();const apply=page.getByRole('link',{name:'Apply now'}).first();await expect(apply).toHaveAttribute('href',`/member/discover/${projectId}/apply`);await expect(page.getByRole('link',{name:/Apply as /})).toHaveCount(0);await expect(page.getByRole('button',{name:/Select this role|Selected/})).toHaveCount(0);expect(await columns(page,'.pdv2Layout'),`${width}px detail layout`).toBe(1);await noOverflow(page,`${width}px detail`);
  }
});

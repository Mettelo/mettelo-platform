...
const checks=[
 ...
 [hardening,"raise exception 'PROJECT_FULL'",'interest capacity is server-authoritative'],
 [hardening,"raise exception 'DEADLINE_PASSED'",'interest deadline is server-authoritative'],
 [hardening,"raise exception 'DUPLICATE_APPLICATION'",'duplicate interest is server protected'],
...
for(const [path,source] of [['public catalogue loader',publicLoader],['member discover loader',memberLoader]])if(source.includes('LEGACY_')||source.includes('retrying legacy'))failures.push(`${path} still contains legacy projection fallback`);

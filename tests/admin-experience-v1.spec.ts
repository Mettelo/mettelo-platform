...
test.describe('Admin operational experience',()=>{
  test('platform Settings is discoverable and governs social and logo configuration',async({page})=>{
    ...
    for(const width of [390,768,1440]){await page.setViewportSize({width,height:900});await page.reload({waitUntil:'networkidle'});await noOverflow(page);}
  });
  ...
  test('Admin can book an interview and the selected timezone is persisted correctly',async({page})=>{
    ...
    await expect(page.getByRole('heading',{level:1,name:'Career Applications'})).toBeVisible();
    await page.getByRole('button',{name:'View →'}).first().click();await expect(page.getByRole('heading',{name:'E2E Career Candidate'})).toBeVisible();
    ...
    await page.reload({waitUntil:'networkidle'});await page.getByRole('button',{name:'View →'}).first().click();await expect(page.getByLabel('Booked interview')).toContainText('20 Aug 2026, 10:00');await page.getByRole('button',{name:'Reschedule'}).click();await expect(page.getByLabel('Date & time')).toHaveValue('2026-08-20T10:00');
    for(const width of [390,768,1440]){await page.setViewportSize({width,height:900});await page.keyboard.press('Escape');await page.reload({waitUntil:'networkidle'});await noOverflow(page);}
  });
});

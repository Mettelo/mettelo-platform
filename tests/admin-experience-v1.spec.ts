...
test.describe('Admin operational experience',()=>{
  ...
  test('Admin can book an interview and the selected timezone is persisted correctly',async({page})=>{
    ...
    await page.reload({waitUntil:'networkidle'});await candidateRow(page).getByRole('button',{name:'View →'}).click();await expect(page.getByLabel('Booked interview')).toContainText('20 Aug 2026, 10:00');await page.getByRole('button',{name:'Reschedule'}).click();await expect(page.getByLabel('Date & time')).toHaveValue('2026-08-20T10:00');await capture(page,'admin-career-reschedule-desktop-1440');

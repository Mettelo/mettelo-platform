...
declare
  ...
  codes text;
begin
  if not ((new.status='active' or coalesce(new.has_started,false)=true) and not (old.status='active' or coalesce(old.has_started,false)=true)) then return new; end if;
  if new.project_id is distinct from old.project_id then raise exception using errcode='23514',message='PROJECT_RUN_PROJECT_IMMUTABLE_AT_START'; end if;
  select * into project_row from public.projects where id=old.project_id;
  if project_row.id is null then raise exception using errcode='23503',message='PROJECT_NOT_FOUND'; end if;
  ...
  if effective_admission='auto' then
    if old.scheduled_start_at is null or old.scheduled_start_at>now() then raise exception using errcode='23514',message='SCHEDULE_NOT_DUE'; end if;
    if project_row.auto_start_paused_at is not null or old.auto_start_paused_at is not null then raise exception using errcode='23514',message='PROJECT_PAUSED'; end if;
    if old.auto_start_blocked_at is not null then raise exception using errcode='23514',message='PROJECT_BLOCKED'; end if;
  end if;
  return new;
end;
$$;

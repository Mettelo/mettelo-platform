...
type ProjectRow={id:string;title:string;status:string;project_type:string;application_deadline:string|null;project_roles:{id:string;openings:number}[]|null};
...
export async function getMemberCapabilityPathProgress(db:Db,userId:string):Promise<MemberCapabilityPathProgress[]>{
  ...
  const [{data:projectData},{data:completedData},{data:verifiedData}]=projectIds.length?await Promise.all([
    db.from('projects').select('id,title,status,project_type,application_deadline,project_roles(id,openings)').in('id',projectIds),
  ...
  return follows.flatMap(follow=>{
    ...
    const pathPlacements=placements.filter(row=>row.path_id===path.id).sort((a,b)=>a.position-b.position).map(row=>{
      const project=projectMap.get(row.project_id)||null;const stage=stageMap.get(row.stage_id)||null;const roleCount=(project?.project_roles||[]).reduce((sum,role)=>sum+Math.max(0,Number(role.openings)||0),0);const availability=project?resolveProjectPublicAvailability({status:project.status,project_type:project.project_type,application_deadline:project.application_deadline,role_count:roleCount}):{available:false,label:'Not currently available'};
    ...
    const incomplete=pathPlacements.filter(item=>!item.completed);const nextProject=incomplete[0]||null;const nextAvailableProject=incomplete.find(item=>item.available)||null;const completedProjects=pathPlacements.filter(item=>item.completed).length;const verifiedProjects=pathPlacements.filter(item=>item.verified).length;

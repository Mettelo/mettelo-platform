...
export async function GET(request:Request){
 ...
 const [{data:project},{data:run},{data:member},{data:architect},{data:delegation},{data:submission},{data:latestRequest},{data:permissions},{data:history},readinessResult]=await Promise.all([
  ...
  db.from('project_runs').select('id,run_number,status,recruitment_open,completion_requested_at,completed_at').eq('id',runId).eq('project_id',projectId).maybeSingle(),

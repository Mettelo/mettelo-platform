...
export default async function ProjectTeamRecoverySection({projectId,projectRunId,currentUserId,runStatus}:{projectId:string;projectRunId:string;currentUserId:string;runStatus:string}){
 ...
 const [{data:run},{data:membership},{data:handovers}]=await Promise.all([
 ...
 return <section className={styles.panel} aria-labelledby="team-recovery-title">
  ...
  <div className={styles.statuses} aria-label="Recovery status">
   {run.replacement_needed&&<span>REPLACEMENT NEEDED</span>}
   {run.lead_replacement_needed&&<span>PROJECT LEAD NEEDED</span>}
   {run.recruitment_open&&<span>RECRUITMENT OPEN</span>}
   {run.replacement_requested_at&&<span>REPLACEMENT REQUESTED</span>}
  </div>

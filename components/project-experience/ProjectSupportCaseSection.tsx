...
export default function ProjectSupportCaseSection({projectId,projectRunId,runStatus}:Props){
 ...
 async function submit(event:FormEvent){
  ...
  try{
   ...
   setCategory('');setDescription('');setSuccess('Your private support case has been submitted. Only authorized Mettelo administrators can review the confidential case details.');
   await load();
 ...
 return <section className={styles.section} id="support" data-lab-support-section aria-labelledby="project-support-title">
  ...
  <div className={styles.history} aria-live="polite"><h4>Your support cases</h4>{loading?<p>Loading your private cases…</p>:cases.length?cases.map(item=><article key={item.id} className={styles.caseCard}><div className={styles.caseHeader}><div><span>{humanise(item.category)}</span><strong>{humanise(item.status)}</strong></div><small>Opened {date(item.created_at)}</small></div><p className={styles.caseDescription}>{item.description}</p>{item.recovery_plan?<div className={styles.secureUpdate}><strong>Recovery plan</strong><p>{item.recovery_plan}</p></div>:null}{item.resolution?<div className={styles.secureUpdate}><strong>Resolution</strong><p>{item.resolution}</p></div>:null}{updates.filter(update=>update.case_id===item.id).length?<div className={styles.timeline}><strong>Secure updates</strong>{updates.filter(update=>update.case_id===item.id).map(update=><div key={update.id}><span>{humanise(update.action)} · {date(update.created_at)}</span>{update.body?<p>{update.body}</p>:null}</div>)}</div>:null}</article>):<p>No support cases for this project run yet.</p>}</div>
 </section>
}

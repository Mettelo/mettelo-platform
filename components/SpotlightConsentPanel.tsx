...
export default function SpotlightConsentPanel({initialItems,publicationName,publicationHeadline,detail=false}:Props){
  ...
  const actionCount=items.filter(item=>item.status==='draft'&&!item.publicationHeld&&!['granted','declined'].includes(item.consentStatus)).length;

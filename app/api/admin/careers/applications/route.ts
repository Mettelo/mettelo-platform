...
export async function PATCH(request:Request){
  try{
    ...
    const eventNote=status==='interview'?`Interview scheduled${interviewAt?` for ${interviewAt}`:''}.`:status==='offer'?'Offer prepared and sent.':status==='hired'?'Candidate marked hired; onboarding started.':note||null;
    await db.from('career_application_events').insert({application_id:id,from_status:application.status,to_status:status,note:eventNote,actor_user_id:user.id});
    if(status==='hired')await db.from('career_onboarding_items').upsert(onboardingDefaults(id),{onConflict:'application_id,item_key',ignoreDuplicates:true});
    ...
    const message=await careerMessageForDb(db,status,roleTitle,{recipientName:application.full_name,interviewAt,interviewDetails:interviewSummary,offerDetails:offerSummary,stageNote:note,subjectOverride,bodyOverride});

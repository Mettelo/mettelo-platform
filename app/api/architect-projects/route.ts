...
import {DEFAULT_AUTO_START_DELAY_MINUTES,canonicalAdmissionMode} from '@/lib/project-admission';
...
function optionalTimestamp(value:unknown){if(value===null||value===undefined||value==='')return null;const parsed=new Date(String(value));return Number.isNaN(parsed.getTime())?undefined:parsed.toISOString()}
...
export async function POST(request:Request){
  ...
  try{
    ...
    const participation=parseProjectParticipation(body&&typeof body==='object'?body:{});const participationError=validateProjectParticipation(participation);if(participationError)return NextResponse.json({error:participationError},{status:400});
    ...
    const projectType=body.partner_project?'partner':'open';
    const requestedMode=projectType==='partner'?'review_required':canonicalAdmissionMode(body.admission_mode);
    const lateJoining=body.late_joining_enabled!==false;
    const lateJoiningCutoff=optionalTimestamp(body.late_joining_cutoff_at);if(lateJoiningCutoff===undefined)return NextResponse.json({error:'Choose a valid late-joining cutoff date and time.'},{status:400});
    const projectSharing=body.project_sharing_enabled!==false;
    const memberInvites=body.member_invites_enabled===true;
    const collaborationMarketplace=body.collaboration_marketplace_enabled!==false;
    const leadInvites=memberInvites&&body.project_lead_invites_enabled!==false;
    const teamInvites=memberInvites&&body.team_member_invites_enabled===true;
    const externalInvites=collaborationMarketplace&&body.external_collaboration_invites_enabled===true;
    const socialSharing=collaborationMarketplace&&projectSharing&&body.collaboration_social_sharing_enabled===true;
    const offerExpiryHours=boundedInt(body.offer_expiry_hours,1,720,72)??72;
    const offerReminders=body.offer_reminders_enabled!==false;
    ...
    const {data:project,error}=await db.from('projects').insert({slug,title,summary,problem_statement:question,project_archetype:archetype,status:'draft',visibility:'private',project_type:projectType,partner_name:clean(body.partner_name,180)||null,location:workingModel==='remote'?'Remote':clean(body.location,160)||null,location_type:workingModel,catalogue_working_model_source:'explicit',difficulty_level:clean(body.difficulty_level,30)||null,duration_weeks:Math.round(durationWeeks),weekly_commitment:weeklyCommitment,...participation,presentation_required:Boolean(body.presentation_required),admission_mode:requestedMode,auto_start_delay_minutes:DEFAULT_AUTO_START_DELAY_MINUTES,late_joining_enabled:lateJoining,late_joining_cutoff_at:lateJoiningCutoff,project_sharing_enabled:projectSharing,member_invites_enabled:memberInvites,collaboration_marketplace_enabled:collaborationMarketplace,project_lead_invites_enabled:leadInvites,team_member_invites_enabled:teamInvites,external_collaboration_invites_enabled:externalInvites,collaboration_social_sharing_enabled:socialSharing,offer_expiry_hours:offerExpiryHours,offer_reminders_enabled:offerReminders,applications_open:false,created_by_user_id:user.id,governance_status:'draft',risk_level:risk.level,risk_reasons:risk.reasons,admin_review_required:risk.adminReviewRequired,updated_at:now}).select('id,slug,title,governance_status,risk_level').single();if(error)throw error;createdId=project.id;
